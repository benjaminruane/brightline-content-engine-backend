...
import { houseVoicePromptGuidance } from "./prompt-library/house-voice.mjs";
...
export function getPromptGuidance(outputType, visibility) {
  ...
  if (ot === "INVESTOR_LETTER") {
    parts.push(
      "Format as an investor letter. Open with 'Dear valued investors,'. " +
        houseVoicePromptGuidance(ot) +
        " Do not produce a separate evidence or supporting detail section. Integrate all sourced detail into narrative prose. Include key metrics and figures from sources where they materially strengthen the narrative, prioritising quality over quantity. Narrative arc: context, decision, thesis, outlook."
    );
  } else if (ot === "PRESS_RELEASE") {
    parts.push(
      "Format as a press release. Open with FOR IMMEDIATE RELEASE. Include headline, dateline, lead paragraph (who/what/when). " +
        houseVoicePromptGuidance(ot) +
        " Include quote if attribution provided; otherwise insert placeholder. Close with boilerplate placeholder."
    );
  } else if (ot === "LINKEDIN_POST") {
    parts.push(
      "Format as a LinkedIn post. " +
        houseVoicePromptGuidance(ot) +
        " Strong opening hook. Short paragraphs or thesis bullet list. If a URL is provided, include it as the final line."
    );
  } else {
    parts.push(
      "Format as reporting commentary: clear, factual, investment-grade prose. " +
        houseVoicePromptGuidance(ot)
    );
  }
  if (vis === "PUBLIC") {
