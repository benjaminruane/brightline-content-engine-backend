 ...
 * WR1: When eventType × visibility matches a PG demo scaffold, that block replaces the generic scaffold.
 * @param {{
 *   outputType: string,
 *   visibility?: string,
 *   eventType?: string,
...
export function buildBasePrompt({
  ...
  const pgScaffold = buildPgWritingScaffold(eventType, visibility, {
    transactionDate,
    investment,
    specialInstructions,
    sources,
    authoringOrganisation: houseName,
  });
  ...
  if (guidance) {
    ...
    if (blocks.length > 0) {
      outputTypeBlock = "Output type: " + getOutputTypeLabel(ot) + "\n\n" + blocks.join("\n\n");
    }
  ...
  let basePromptText;
  if (pgScaffold) {
    // WR1: Generic output-type guidance first; PG scaffold (binding constraints) last in basePromptText.
    basePromptText = outputTypeBlock ? outputTypeBlock + "\n\n" + pgScaffold : pgScaffold;
  } else {
    basePromptText = styleGuideScaffold;
    if (outputTypeBlock) {
      basePromptText += "\n\n" + outputTypeBlock;
    }
  }
  ...
  return {
    ...
    baseGuidanceSummary:
      summary.length > 0 ? summary : ["Follow house rules and output-type conventions."],
  };
}
...
export function detectRewriteClash({ outputType, rewriteInstructions }) {
  ...
  // Voice clash: house voice is third-person; user asks for first person / we / our / my
  ...
  if (houseVoiceIsThirdPerson(ot) && firstPersonCues.test(lower)) {
    reasons.push("instructions request first-person or 'we'/'our' voice; house voice for this output type is third-person");
  }
