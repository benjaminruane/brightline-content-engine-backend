...
const HEDGE_WORD_RE =
  /\b(?:might|possibly|perhaps|likely|probably|uncertain|unclear|unknown|unspecified|unreliable|unsafe)\b/i;
...
/**
 * Hedge and caution phrasing in user-facing explanations.
 * Word-boundary, case-insensitive. Complements findBannedUserCopy.
 */
export function findHedgedUserCopy(text) {
  const raw = String(text ?? "");
  if (!raw.trim()) return [];
  const lowered = normalize(raw).toLowerCase();
  const hits = [];
  ...
  const word = raw.match(HEDGE_WORD_RE);
  if (word) add(word[0]);
  return hits;
}

export function findUnshippableUserCopy(text) {
  return [...findBannedUserCopy(text), ...findHedgedUserCopy(text)];
}
