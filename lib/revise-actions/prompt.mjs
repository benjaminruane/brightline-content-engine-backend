...
function licensedHouseName(authoringOrganisation, draftText, statement) {
  const resolved = resolveAuthoringOrganisationName(authoringOrganisation);
  const corpus =
    typeof draftText === "string" && draftText.trim()
      ? draftText
      : String(statement ?? "");
  return identifyAuthoringOrganisation(corpus, resolved);
}
...
export function buildFindingPrompt(finding, { authoringOrganisation, silenceOnCard, draftText } = {}) {
  const org = licensedHouseName(authoringOrganisation, draftText, finding?.statement);
  const noSourceSpeaks = silenceOnCard === true || finding?.sort?.silenceOnCard === true;
  const firstPersonOnly =
    Boolean(org) && (noSourceSpeaks || isFirstPersonActorRule(finding?.rule, finding?.rule));
  const thing1 = finding?.thing1?.quote ? finding.thing1.quote : "(none)";
  const firstPersonRules = firstPersonOnly
    ? [
  ...
  return [
    ...
    ...firstPersonRules,
    "",
    `Authoring organisation: ${org || "(none)"}`,
    noSourceSpeaks
