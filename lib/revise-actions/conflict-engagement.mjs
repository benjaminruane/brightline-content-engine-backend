...
function qualifierAllows(draftToken, sourceToken, excerpt) {
  const draftQ = extractQualifiers(draftToken.fragment);
  const sourceQ = extractQualifiers(sourceToken.fragment);
...
function r5Vetoes(statement, pair, licensed) {
  const tokens = annotateTokens(statement);
  const dest = fromToStart(statement, pair.from, tokens);
  if (dest && !licensed.has(dest.start)) return true;
  if (pair.from.total) {
    const hanging = hangingComponents(tokens, pair.from);
    if (hanging.length > 0 && hanging.some((token) => !licensed.has(token.start))) return true;
  }
  if (percentageOfBase(statement, pair.from, tokens)) {
    ...
    if (pct && !licensed.has(pct.start)) return true;
  }
  return false;
}
...
function buildReplaceProposal(statement, pairs) {
  ...
  const whyBits = ordered.map((pair) => `${pair.to.raw}, not ${pair.from.raw}`);
  const why =
    whyBits.length === 1
      ? `The source gives ${whyBits[0]}.`
      : `The source gives ${whyBits.join(", and ")}.`;
...
function unaddressed() {
  return { status: "unaddressed", pairs: [], proposal: null };
}
...
export function applyConflictProposal(finding, candidate = null) {
  ...
  if (pairs.length === 0) return unaddressed();

  const licensedStartsSet = licensedStarts(pairs);
  const writable = [];
  let blocked = false;
  for (const pair of pairs) {
    if (r1Vetoes(finding, excerpt, pair) || r5Vetoes(statement, pair, licensedStartsSet)) {
      blocked = true;
      break;
    }
    writable.push(pair);
  }
  if (blocked) return unaddressed();
  ...
  if (candidate && typeof candidate.resultingSentence === "string") {
    if (!proposalAddressesConflict(statement, candidate.resultingSentence, writable)) {
      ...
      return { status: "unengaged", pairs: writable, proposal: null };
    }
  }
  return { status: "replace", pairs: writable, proposal };
}
