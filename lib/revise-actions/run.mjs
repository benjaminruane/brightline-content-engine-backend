...
function explanationFromOutcome(explain) {
  ...
  if (!filled) return { explanation: "", explainCode: null };
  const swept = sweepPublicText(filled, GENERIC_CONTRADICTION);
  return { explanation: swept, explainCode: explain?.code || null };
}
...
function publicEntry(entry) {
  ...
  const explanation = sweepPublicText(entry.explanation, GENERIC_CONTRADICTION);
  if (explanation) out.explanation = explanation;
  if (entry.explainCode) out.explainCode = entry.explainCode;
  if (entry.governancePair && typeof entry.governancePair === "object") {
    out.governancePair = entry.governancePair;
  }
  if (Array.isArray(entry.disagreementPassages) && entry.disagreementPassages.length > 0) {
    const passages = [];
    for (const row of entry.disagreementPassages) {
  ...
  if (typeof entry.confirmingPassage === "string" && entry.confirmingPassage.trim()) {
    out.confirmingPassage = entry.confirmingPassage.trim();
    out.confirmingPassageLabel =
      typeof entry.confirmingPassageLabel === "string" && entry.confirmingPassageLabel.trim()
        ? entry.confirmingPassageLabel.trim()
        : "Confirmed excerpt";
  }
  if (entry.disposition === "ACKNOWLEDGE") {
    out.noProposalReason = sweepPublicText(entry.noProposalReason, GENERIC_CONTRADICTION) || entry.noProposalReason;
  }
...
function findingHasNoReviewText(entry) {
  const thing2 = typeof entry?.thing2 === "string" ? entry.thing2.trim() : "";
  const direction =
    typeof entry?.suggestedDirection === "string" ? entry.suggestedDirection.trim() : "";
  return !thing2 && !direction;
}
...
export async function fillAction(entry, { authoringOrganisation, callModel, draftText, candidateProposal, sourceRulings } = {}) {
  ...
  if (findingHasNoReviewText(entry)) {
    return publicEntry(asVisibleAcknowledge(entry, "visible_signal"));
  }
