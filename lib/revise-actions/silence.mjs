...
/**
 * Evidence badge is a conflict-free partial. Does not change whether the card
 * is silent. Used only to choose acknowledge copy.
 */
export function isConflictFreePartial(card) {
  if (!card || typeof card !== "object") return false;
  const displayVerdict = norm(card.displayVerdict);
  const supportState = norm(card.supportState);
  const isPartial =
    displayVerdict === "supported_partial" ||
    supportState === "partial" ||
    supportState === "partially_confirmed";
  if (!isPartial) return false;
  if (displayVerdict === "conflict" || supportState === "conflicting" || card.hasConflict === true) {
    return false;
  }
  return true;
}
