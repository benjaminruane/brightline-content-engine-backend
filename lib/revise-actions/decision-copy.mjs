...
export const EXPLAIN_CODES = Object.freeze({
  ...
  sources_disagree_neither: "sources_disagree_neither",
  sources_agreed_kept: "sources_agreed_kept",
});
...
function qualifierSilentLine(values) {
  ...
  return withConfirmedUnchanged(`${base} It does not say whether its ${value} is gross or net, so the word ${word} is left as written.`, values);
}

export function confirmedUnchangedLine(label) {
  const name = String(label ?? "").trim();
  if (!name) return "";
  return `It confirms the ${name}, which is left unchanged.`;
}

function withConfirmedUnchanged(base, values) {
  if (!base) return "";
  const extra = confirmedUnchangedLine(values?.confirmedLabel);
...
function sourcesDisagreeLine(values) {
  const docA = String(values?.docA ?? "").trim();
  const figA = String(values?.figA ?? "").trim();
  const docB = String(values?.docB ?? "").trim();
  const figB = String(values?.figB ?? "").trim();
  if (!docA || !figA || !docB || !figB) return "";
  return `Two of your sources disagree. ${docA} states ${figA}; ${docB} states ${figB}.`;
}

function sourcesDisagreeNeitherLine(values) {
  const docA = String(values?.docA ?? "").trim();
  const figA = String(values?.figA ?? "").trim();
  const docB = String(values?.docB ?? "").trim();
  const figB = String(values?.figB ?? "").trim();
  if (!docA || !figA || !docB || !figB) return "";
  return `Two of your sources disagree and neither supersedes the other. ${docA} states ${figA}; ${docB} states ${figB}.`;
}

function sourcesAgreedKeptLine(values) {
  const docGov = String(values?.docGov ?? "").trim();
  const fig = String(values?.fig ?? "").trim();
  const docOther = String(values?.docOther ?? "").trim();
  const figOther = String(values?.figOther ?? "").trim();
  if (!docGov || !fig || !docOther || !figOther) return "";
  return `${docGov} gives ${fig}, which is the figure in this sentence. ${docOther} states ${figOther}.`;
}
...
export function fillDecisionCopy(explain) {
  ...
  if (code === EXPLAIN_CODES.correction) return withConfirmedUnchanged(correctionLine(values), values);
  ...
  if (code === EXPLAIN_CODES.sources_disagree) return sourcesDisagreeLine(values);
  if (code === EXPLAIN_CODES.sources_disagree_neither) return sourcesDisagreeNeitherLine(values);
  if (code === EXPLAIN_CODES.sources_agreed_kept) return sourcesAgreedKeptLine(values);
  return "";
}
