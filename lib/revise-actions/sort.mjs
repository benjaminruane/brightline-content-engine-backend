...
import { GENERIC_CONTRADICTION } from "./decision-copy.mjs";
import { inventoryStatements } from "./inventory.mjs";
import { isConflictFreePartial, statementIsSilent } from "./silence.mjs";
...
export const NO_PROPOSAL = {
  ...
  silence_no_edit: "No supplied source speaks to this claim, either way. The wording is yours.",
  partial_no_edit: "A source supports part of this statement, not all of it. The wording is yours.",
  partial_policy: "This concern stands. The wording is yours.",
  visible_signal: "This concern is on the card so you can see it.",
  ...
  conflict_unaddressed: GENERIC_CONTRADICTION,
};
...
function sortedRecord(finding, { disposition, policyPermit, silenceOnCard, reasonCode }) {
  // card must stay on the internal entry because the proposal step reads
  // supportSpans from it. The public response is filtered separately by publicEntry.
  const entry = {
    ...finding,
    disposition,
    sort: {
...
export function sortFinding(finding, silenceOnCard) {
  ...
  if (kind === "evidence") {
    if (silenceOnCard) {
      if (isConflictFreePartial(finding.card)) {
        return acknowledge(finding, silenceOnCard, "partial_no_edit");
      }
      return acknowledge(finding, silenceOnCard, "silence_no_edit");
    }
  ...
  if (kind === "editorial" || kind === "compliance") {
    if (silenceOnCard) {
      ...
      if (isConflictFreePartial(finding.card)) {
        return acknowledge(finding, silenceOnCard, "partial_policy");
      }
