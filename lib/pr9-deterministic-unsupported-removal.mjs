...
import { isAuthoringOrganisationName } from "./qc/first-person-actor.mjs";
...
export function findCheckableParticulars(sentenceText) {
  ...
  while ((pm = properRe.exec(text)) !== null) {
    ...
    if (words.length === 1) {
      ...
      if (isAuthoringOrganisationName(w)) continue;
      ...
      // "We" is already dropped as the author by another name. The configured
      // organisation's own name is the same actor spelled out, so it is not a
      // particular an outside source has to corroborate either.
      if (isAuthoringOrganisationName(raw)) continue;
 ...
 * @param {{ enabled?: boolean, originalDraft?: string, authoringOrganisation?: string|null, traceId?: string, log?: Function }} [opts]
 * @returns {{
 *   revisedDraft: string,
...
export function applyDeterministicUnsupportedRemoval(parsed, concerns, opts = {}) {
  ...
  for (const concern of list) {
    ...
    // Silence about the author's own act is not grounds to delete it. The
    // statement is kept and quietly flagged; Review still reports it, so a
    // source that CONTRADICTS it is still caught on the ordinary path.
    const exemption = authorStatementExemption(concern, {
      authoringOrganisation: opts.authoringOrganisation,
      draftText: originalDraft || draft,
    });
    if (exemption.exempt) {
      const match = findStatementTextInDraft(draft, concern.statementText);
      if (match) {
        markers = keepAndFlagSentence(
          draft,
          markers,
          match.start,
          match.end,
      ...
      removalEvents.push({
        ...
        note: AUTHOR_STATEMENT_KEPT_NOTE,
      });
      continue;
    }
