...
export function applyDeterministicUnsupportedRemoval(parsed, concerns, opts = {}) {
  ...
  for (const { concern, match } of planned) {
    ...
    const removedText = draft.slice(live.start, range.sentenceEnd > live.start ? range.sentenceEnd : live.end);
    const note = buildDeterministicUnsupportedRemovalCutNote(
      concern.statementText || removedText
    );
    markers.push({
      start: remStart,
      end: remEnd,
    ...
    removalEvents.push({
      ...
      statementIndex: concern.statementIndex,
      statementText: concern.statementText,
      removedSentenceText: collapseWhitespace(
        stripMarkersFromQuotedText(concern.statementText || removedText)
      ),
      remnantSide,
      remnantText,
      note,
