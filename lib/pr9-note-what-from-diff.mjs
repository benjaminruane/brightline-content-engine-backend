 ...
 */
import { markerSpanAlignment, tokenizeWords } from "./pr9-marker-span-status.mjs";
import { findStatementTextInDraft } from "./pr9-deterministic-unsupported-removal.mjs";
import { flagRegister, findingAddressedByEdit, QUIET_NOTE } from "./revise-flag-register.mjs";
 ...
 * @param {Array<{ text: string, start: number, end: number }>} [revSpan]
 *        the aligned revised-span tokens. Defaults to tokenizing the slice,
 *        which is close but not identical; see spanWords below.
 * @returns {Array<{ text: string, start: number, end: number }>}
 */
const INTERNAL_SENTENCE_BREAK_RE = /[.!?]["'\u201d\u2019)]*\s+\S/;

export function confineRegionToOwnSentence(original, revised, start, end, origRegion, revSpan) {
  ...
  // Tokens as the ALIGNMENT sees them, which is not the same as splitting the
  // slice. Tokenizing happens over the whole draft, so the token at a sentence
  // end carries its full stop ("services.") while the slice usually stops
  // short of it ("services"). Comparing region tokens against the slice was
  // how a sentence's own last word came to look absent from its own span.
  const spanTokens = Array.isArray(revSpan) && revSpan.length > 0 ? revSpan : tokenizeWords(span);

  // A region the span reproduces token for token cannot contain a leak: there
  // is nothing outside the span to have leaked in. Clipping it can only invent
  // a difference, which is what happened on the Meridian date sentence — the
  // decimal point in "EUR 1.2 billion" reads as a sentence terminator, so the
  // bounds below stopped at "a EUR 1." and dropped the tail of a sentence
  // nothing had touched.
  if (
    spanTokens.length === region.length &&
    spanTokens.every((t, i) => t.text === region[i].text)
  ) {
    return region;
  }
  ...
  //
  // Matched on exact text. Bare, punctuation-insensitive matching rescues a
  // leaked "to" or "us" because some unrelated word in the span looks the
  // same, which was measured resurrecting a whole leaked clause onto the next
  // sentence.
  const spanWords = new Set(spanTokens.map((t) => t.text).filter(Boolean));
...
function ownRegion(original, revised, start, end, align) {
  return confineRegionToOwnSentence(
    original,
    revised,
    start,
    end,
...
/**
 * AN EDIT MUST NOT MASK A SILENCE FLAG.
 *
 * A statement can be edited for style AND still carry an evidence finding the
 * edit did nothing about. Before this, the register only fired on an unchanged
 * span, so the edit note won and the reader was never told inline that no
 * source speaks to the claim. Measured at 8 of 13 silence-flagged elements
 * across the committed artefacts.
 *
 * Returns the register sentence to append, or "" where there is nothing to add:
 * no concern, an ORDINARY finding (a source DID speak, so Revise is editing on
 * evidence and the edit note is the whole story), or a finding the edit
 * resolved.
 *
 * Both the register decision and the addressed test are made about the FLAGGED
 * ELEMENT — the tightest unsupported span where claim decomposition gave one,
 * the marker's own original region otherwise. Not about the marker geometry: a
 * marker wrapping a whole sentence whose unsupported clause was cut has
 * addressed its finding, and asking the question of the whole sentence would
 * say otherwise.
 *
 * @param {?object} concern
 * @param {string} originalRegionText
 * @param {string} revisedSpanText
 * @returns {string}
 */
export function unaddressedRegisterClause(concern, originalRegionText, revisedSpanText) {
  if (!concern) return "";
  const spans = tightestUnsupportedSpans(concern);
  const element = spans.length > 0 ? spans[0].text : originalRegionText;
  const decision = flagRegister(concern, null, element);
  if (!decision.note) return "";
  if (findingAddressedByEdit(element, revisedSpanText)) return "";
  return decision.note;
}

/**
 * Join an edit note to a register clause as two sentences.
 *
 * The edit clause leads because it describes what actually happened to the
 * text. The closer is left to normalizeMarkerNoteText, which recognises a note
 * ending on a register clause and appends nothing: LOUD already closes on its
 * own instruction, and QUIET asks nothing of the reader.
 */
function withRegisterClause(body, registerClause) {
  if (!registerClause) return body;
  const lead = String(body ?? "").trim().replace(/[.\s]+$/, "");
  return lead ? `${lead}. ${registerClause}` : registerClause;
}
...
export function buildNoteBodyFromDiff({ original, revised, start, end, note, concerns } = {}) {
  ...
  const registerClause = unaddressedRegisterClause(
    tracedConcern,
    ownAlign.origRegionText,
    spanText(revised, start, end)
  );

  const editorialReason = tracedConcern ? editorialReasonForEdits(tracedConcern, edits) : "";
  if (editorialReason) {
    return {
      body: withRegisterClause(`${clause} - ${editorialReason}`, registerClause),
      clause,
  ...
  // A class reason IS the evidence concern speaking ("no supplied source backs
  // this claim"), so a QUIET clause after it says the same thing twice. LOUD is
  // not a restatement: it carries an instruction the class reason does not, and
  // dropping it was measured losing "do not publish it without one" from a
  // softened causal claim.
  const applied =
    reasonSource === "concern" && registerClause === QUIET_NOTE ? "" : registerClause;

  return {
    body: withRegisterClause(reason ? `${clause} - ${reason}` : clause, applied),
    clause,
    reason,
    reasonSource,
    registerClause: applied,
    changed: edits.length > 0,
    edits,
  };
}
