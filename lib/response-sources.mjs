...
/**
 * True when prepareUploadedSourcesForPipeline flagged the row as scanned / image-only.
 * Key off extraction STATUS (not placeholder text).
 * @param {object|undefined} preparedRow
 */
export function isUnsupportedScannedExtraction(preparedRow) {
  return preparedRow?.meta?.extraction?.status === "unsupported_scanned";
}

/**
 * Whether a prepared source should be excluded from reviewed `sources` / pipeline kept set.
 * Empty text (existing) OR unsupported_scanned status (F13 — image-only placeholder text is non-empty).
 * @param {object|undefined} preparedRow
 */
export function shouldExcludePreparedSource(preparedRow) {
  const text = typeof preparedRow?.text === "string" ? preparedRow.text : "";
  if (!text.trim()) return true;
  return isUnsupportedScannedExtraction(preparedRow);
}
...
export function dropReasonAfterExtraction(original, preparedRow) {
  ...
  if (isUnsupportedScannedExtraction(preparedRow)) return "unsupported_scanned";
  const text = typeof preparedRow.text === "string" ? preparedRow.text : "";
...
/**
 * Split prepared sources into kept (reviewed / pipeline) vs dropped
 * (empty text OR unsupported_scanned — F13).
 * `kept` is contiguous and re-indexed by array position (sourceIndex for supportSpans).
 * `dropped` is separate — never merged into kept — so a middle empty/scanned source does not
...
export function splitSourcesForResponse(preparedSources, candidateSources) {
  ...
  for (let index = 0; index < prepared.length; index++) {
    ...
    if (shouldExcludePreparedSource(source)) {
      const reason =
