 ...
 */
export function splitDraftIntoCandidatesV2(normalizedDraftText, opts = {}) {
  ...
  // Step C - Secondary split on long segments (; then :), only outside parens.
  // Dashes (em, en, hyphen) are punctuation inside a sentence, not a boundary.
...
export function extractStatements({ mode, text, opts = {} }) {
  ...
  if (mode === "draft") {
    ...
    const candidates = normalizedDraftText.trim() ? [normalizedDraftText.trim()] : [];
    const metadata = {
      mode: "draft",
      draftChars: normalizedDraftText.length,
