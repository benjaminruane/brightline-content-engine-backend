...
export async function prepareUploadedSourcesForPipeline(sources, options = {}) {
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    if (hasText && !hasContent) {
      ...
      // X1.2 PART 3.1: If client sent inline text for a PDF (legacy), mark ingestionPath and add warning
      const looksLikePdf = /\.pdf$/i.test(String(s.name ?? s.title ?? s.filename ?? "")) ||
        (String(s.mimeType || "").toLowerCase() === "application/pdf");
      const ingestionPath = looksLikePdf ? "inline_text" : "inline_text";
      const warningsOut = [...(extraction.warnings || [])];
      if (looksLikePdf) warningsOut.push("pdf_inline_text_legacy");
      const x11f = buildExtractionTraceability(text, ingestionPath, s, { ...extraction, warnings: warningsOut });
      ...
      out.push({
        ...
        meta: { ...(s.meta || {}), extraction: { ...extraction, warnings: warningsOut, ...x11f } },
      });
      continue;
    }
    ...
    const x11f = buildExtractionTraceability(text, "raw_file", { filename: s.filename, name: s.name, title: s.title, mimeType: s.mimeType }, extraction);
    totalTextLength += text.length;
    ...
    out.push({
      ...
      name: s.name ?? s.title ?? "Untitled source",
      title: s.title ?? s.name ?? "Untitled source",
      meta: { ...(s.meta || {}), extraction: { ...extraction, ...x11f } },
    });
  }
