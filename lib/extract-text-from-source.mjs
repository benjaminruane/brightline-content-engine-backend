...
/** Extraction timeout (ms). Tracks vercel.json maxDuration (300s). */
export const EXTRACTION_TIMEOUT_MS = 300_000;

/**
 * B317: second officeparser convert ("chunks" -> extraction.structure).
 * Default OFF. Same true/1 read as DIAG_EXTRACT_SAMPLES in this file.
 * When off, skip the chunks convert and return the empty page/slide/sheet shape.
 */
function isExtractStructureEnabled() {
  const v = process.env.QC_EXTRACT_STRUCTURE;
  return v === "true" || v === "1";
}

/**
 * B321. PDF_ENGINE default "direct". Only "officeparser" falls back.
 * Per-call options.pdfEngine wins so extract-draft-text can compare engines
 * without flipping the process env for every concurrent Review.
 */
export function resolvePdfEngine(options = {}) {
  const fromOpt = typeof options.pdfEngine === "string" ? options.pdfEngine.trim() : "";
  if (fromOpt === "officeparser" || fromOpt === "direct") return fromOpt;
  if (fromOpt) {
    console.warn(
      `[extract-text-from-source] unrecognized options.pdfEngine=${JSON.stringify(fromOpt)}; using env`
    );
  }
  const fromEnv = typeof process.env.PDF_ENGINE === "string" ? process.env.PDF_ENGINE.trim() : "";
  if (fromEnv === "officeparser") return "officeparser";
  if (fromEnv && fromEnv !== "direct") {
    console.warn(
      `[extract-text-from-source] unrecognized PDF_ENGINE=${JSON.stringify(fromEnv)}; using direct`
    );
  }
  return "direct";
}
...
/**
 * Near-empty threshold for unsupported-scanned flag (born-digital scope; OCR deferred).
 * image_only.pdf / scanned pages that yield only image placeholders or tiny text trip this.
 */
export const SCANNED_NEAR_EMPTY_CHARS = 50;
...
/**
 * Meaningful char count for scanned detection: drop officeparser image placeholders.
 * @param {string} text
 * @returns {number}
 */
function meaningfulExtractedCharCount(text) {
  const str = text == null || typeof text !== "string" ? "" : text;
  return str
    .replace(/\[Image:[^\]]*\]/gi, "")
    .replace(/\s+/g, " ")
...
/**
 * Additive structure metadata from officeparser document-structure chunks (page/slide/sheet).
 * Not consumed by the matcher yet — present for drawer/navigation later.
 * @param {Array<{ text?: string, metadata?: Record<string, unknown>, startIndex?: number, endIndex?: number }>} chunks
 * @returns {{ pages: Array<{ page: number, text: string }>, slides: Array<{ slide: number, text: string }>, sheets: Array<{ sheet: string, text: string }> }}
 */
function buildStructureFromChunks(chunks) {
  const pages = [];
  const slides = [];
  const sheets = [];
  ...
  for (const c of list) {
    ...
    if (md.pageNumber != null && Number.isFinite(Number(md.pageNumber))) {
      pages.push({ page: Number(md.pageNumber), text });
    }
    if (md.slideNumber != null && Number.isFinite(Number(md.slideNumber))) {
      slides.push({ slide: Number(md.slideNumber), text });
    }
    if (typeof md.sheetName === "string" && md.sheetName.trim()) {
      sheets.push({ sheet: md.sheetName.trim(), text });
    }
  }
  return { pages, slides, sheets };
}

/**
 * Extract text from a file buffer by MIME type via officeparser (OCR disabled).
 * X1.1b: Returns extraction-health metadata (fileType, method, textLength, numLines, hasCurrencyToken, hasDigits, warnings).
 * Additive: `structure` (page/slide/sheet) and `status` (`ok` | `unsupported_scanned`).
 * @param {Buffer} fileBuffer
 * @param {string} mimeType - Normalised MIME (from detectFileType).
 * @param {{ timeoutMs?: number }} [options]
 * @returns {Promise<{ text: string, extraction: { fileType: string, method: string, textLength: number, numLines: number, hasCurrencyToken: boolean, hasDigits: boolean, warnings: string[], status: "ok"|"unsupported_scanned", structure: { pages: object[], slides: object[], sheets: object[] } } }>}
 */
export async function extractTextFromSource(fileBuffer, mimeType, options = {}) {
  ...
  if (!["pdf", "docx", "pptx", "xlsx"].includes(type)) {
  ...
  const engine = resolvePdfEngine(options);
  const useDirectPdf = type === "pdf" && engine === "direct";

  let raw = "";
  let chunks = [];
  let textConvertMs = 0;
  let chunkConvertMs = 0;
  let method = "officeparser";
  let structureFromDirect = null;

  if (useDirectPdf) {
    const { extractPdfDirect } = await import("./extract-pdf-direct.mjs");
    const direct = await extractPdfDirect(fileBuffer, { timeoutMs });
    raw = typeof direct?.text === "string" ? direct.text : "";
    textConvertMs = Number.isFinite(direct?.textConvertMs) ? direct.textConvertMs : 0;
    chunkConvertMs = 0;
    method = "pdfjs-direct";
    if (isExtractStructureEnabled()) {
      structureFromDirect = {
    ...
    const { OfficeConverter, terminateOcr } = await import("officeparser");
    const parseConfig = {
      ocr: false,
    ...
    try {
      ...
      try {
        textRes = await runWithTimeout(
          OfficeConverter.convert(fileBuffer, "text", { parseConfig })
        );
        textConvertMs = Date.now() - tText;
      } catch (e) {
        textConvertMs = Date.now() - tText;
      ...
      if (isExtractStructureEnabled()) {
        ...
        try {
          const chunkRes = await runWithTimeout(
            OfficeConverter.convert(fileBuffer, "chunks", {
              parseConfig,
              generatorConfig: {
                chunksConfig: {
          ...
          chunkConvertMs = Date.now() - tChunk;
          chunks = Array.isArray(chunkRes?.value) ? chunkRes.value : [];
        } catch (_) {
          chunkConvertMs = Date.now() - tChunk;
          chunks = [];
        }
      ...
      // OCR is disabled, but officeparser may still retain worker handles; release them.
      try {
        await terminateOcr();
      } catch (_) {
        /* ignore */
    ...
    const rawValue = textRes?.value;
    raw =
      typeof rawValue === "string"
        ? rawValue
        : String(rawValue?.text ?? rawValue ?? "");
  }

  const afterFormatCleanup =
    type === "pdf" ? normalisePdfExtractedText(raw.trim()) : raw.trim();
  const text = normaliseExtractedText(afterFormatCleanup);
  const structure = structureFromDirect || buildStructureFromChunks(chunks);
  const extraction = computeExtractionHealth(text, type, method);
  extraction.textConvertMs = textConvertMs;
  extraction.chunkConvertMs = chunkConvertMs;
  const meaningfulLen = meaningfulExtractedCharCount(text);
  const unsupportedScanned = meaningfulLen < SCANNED_NEAR_EMPTY_CHARS;
  if (unsupportedScanned) {
    if (!extraction.warnings.includes("likely_scanned_pdf") && type === "pdf") {
      extraction.warnings.push("likely_scanned_pdf");
    }
    if (!extraction.warnings.includes("unsupported_scanned")) {
      extraction.warnings.push("unsupported_scanned");
    }
  }

  extraction.status = unsupportedScanned ? "unsupported_scanned" : "ok";
  extraction.structure = structure;
  extraction.scannedNearEmptyChars = SCANNED_NEAR_EMPTY_CHARS;
  extraction.meaningfulTextLength = meaningfulLen;
 ...
 * @returns {boolean}
 */
export function looksLikeRawPdfBytes(text) {
  const str = typeof text === "string" ? text.trim() : "";
  return str.length >= 5 && str.slice(0, 5) === "%PDF-";
}

/**
 * B47: Inline text that looks like a ZIP / Office Open XML package (docx/pptx/xlsx).
 * True when text starts with the PK zip signature, or contains "[Content_Types].xml".
 * @param {string} text
 * @returns {boolean}
 */
export function looksLikeOfficeZipBytes(text) {
  const str = typeof text === "string" ? text : "";
  if (!str) return false;
  // ZIP local-file / empty-archive signatures (PK\x03\x04, PK\x05\x06, …)
  if (str.length >= 2 && str.charCodeAt(0) === 0x50 && str.charCodeAt(1) === 0x4b) return true;
  if (str.includes("[Content_Types].xml")) return true;
  return false;
}
...
export async function prepareUploadedSourcesForPipeline(sources, options = {}) {
  ...
  for (let i = 0; i < sources.length; i++) {
    ...
    if (hasText && !hasContent) {
      ...
      // B47: Office ZIP / OOXML bytes sniff — reject (must use contentBase64 → officeparser)
      if (looksLikeOfficeZipBytes(text)) {
        return {
          sources: out,
          error: {
            code: "OFFICE_INLINE_TEXT_NOT_ALLOWED",
            message:
              "DOCX/PPTX/XLSX sources must be uploaded as files (binary). Inline text payload is not accepted.",
          },
        };
      }
    ...
    let extracted;
    try {
      extracted = await extractTextFromSource(buffer, mimeType, {
        timeoutMs,
      ...
      return {
        ...
        error: {
          code: msg,
          message: `Source ${i + 1}: ${msg}`,
          textConvertMs: Number.isFinite(e?.textConvertMs) ? e.textConvertMs : 0,
          chunkConvertMs: Number.isFinite(e?.chunkConvertMs) ? e.chunkConvertMs : 0,
        },
      };
    }
