...
import {
  ...
  exampleAuthoringOrganisationName,
  resolveAuthoringOrganisationName,
  trimAuthoringOrganisationName,
} from "./first-person-actor.mjs";
import { houseVoiceStyleGuideProse } from "../prompt-library/house-voice.mjs";
import { EVALUATIVE_LANGUAGE_FIX_DIRECTION, EVALUATIVE_LANGUAGE_INSTRUCTION } from "./evaluative-language.mjs";

function firstPersonPluralCorrectExample(houseName) {
  const house = exampleAuthoringOrganisationName(houseName);
  return `${house} completed the investment in Q2. ${house} was attracted to Meridian. ${house} believes the fund should deliver returns broadly in line with its predecessor and ${house} recommends the commitment. Access that would not otherwise have been available to ${house}.`;
}
...
export const STYLE_GUIDE_LAYER_1 = [
  ...
  },
  {
    ...
    description:
      "Distinguish genuine hyperbole and distinction-claim language ('exceptional', 'genuinely exceptional', 'unparalleled', 'best-in-class', 'world-class', 'industry-leading', 'extraordinary', 'unmatched', 'revolutionary', 'game-changing', 'transformative', 'genuine differentiator') from standard qualitative descriptors. " +
    ...
    applies_to: ["*"],
    layer: 1,
    fixDirection: EVALUATIVE_LANGUAGE_FIX_DIRECTION,
  },
  {
...
export const STYLE_GUIDE_LAYER_2_CLIENT = [
  ...
  },
  {
    ...
    description:
      houseVoiceStyleGuideProse() + " " + FIRST_PERSON_ACTOR_INSTRUCTION,
    correct_example: firstPersonPluralCorrectExample(null),
    incorrect_example:
 ...
 * first_person_plural examples resolve per call from request/env, else placeholder.
 * @param {object} documentContext
 * @param {string} [documentContext.outputType]
 * @param {string|null} [documentContext.authoringOrganisation]
 * @param {string|null} [documentContext.promptHouseName] Presence-checked name. When this key is present, it is used as-is and env is not a fallback.
 * @returns {StyleGuideRule[]}
 */
export function resolveStyleGuide(documentContext = {}) {
  ...
  const house =
    documentContext.promptHouseName !== undefined
      ? trimAuthoringOrganisationName(documentContext.promptHouseName)
      : resolveAuthoringOrganisationName(documentContext.authoringOrganisation);
  return filterByOutputType(merged, outputSlug).map((rule) => {
    if (rule.id !== "first_person_plural") return rule;
    return {
      ...rule,
      description:
        ...
        buildFirstPersonActorInstruction(house),
      correct_example: firstPersonPluralCorrectExample(house),
    };
  });
}
