...
import { classifyEvidenceRelationship } from "./evidence-relationship.mjs";
...
export function buildClaimEvidenceAuthority(params) {
  ...
  if (mergedCandidates.length === 0) {
    ...
    const supportCandidates = mergedCandidates.filter((c) => {
      ...
      return rel.relationship === "support";
    });
  ...
  // Commentary: rule-based from classification only (2–3 sentences, ≤50 words)
  const commentaryText = buildCommentaryFromPayload(classification, claimText, null);
