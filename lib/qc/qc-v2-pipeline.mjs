...
/** A6.49j: Structured quantity mismatch only (spec type or inferQuantityTypeMismatch kind). */
function isQuantityMismatchStructured(supportMismatch) {
  ...
  switch (explanationType) {
    case "mismatch":
      if (conflictOpts?.valueA != null && conflictOpts?.valueB != null) {
        ...
        text = "The sources describe different readings of the material, not a single reading as stated.";
      }
      break;
    case "partial": {
    ...
    case "confirm":
      text = "The source confirms the entity, relationship, and material details reflected in the sources.";
      break;
    case "related":
      text = "The source discusses related subject matter, but does not confirm this claim.";
      break;
    case "absence":
    default:
      text = "No source directly confirms this claim.";
      break;
  }
...
/** A6.46: Per-citation popup sections (deterministic). A6.49e: supportMismatch shapes RELATED popup when partial path. */
function buildPopupSectionsForDisplayItem(item, claimText, claimLevelClassification, supportMismatch = null) {
  ...
  if (code === "RELATED_CONTEXT" || item.classification === "related") {
    if (supportMismatch?.kind != null && typeof supportMismatch.explanation === "string" && supportMismatch.explanation.trim()) {
      const explained = supportMismatch.explanation.trim().slice(0, 220);
      return {
        originalClaimText: ct,
        whatThisShows: explained,
        whatIsNotShown: null,
      };
    }
    return {
...
/** A6.49k: Bindings for the exact canonical claim (claimId must match when claim.id is set). */
function filterSupportBindingsForClaim(bindings, claim) {
...
/** A6.49k: Placeholder / synthetic bindings are never direct confirming support. */
function isBindingPlaceholderSynthetic(binding) {
...
/** A6.49k: Binding aligns with quantity-mismatch surface (not direct full support for this spec). */
function isBindingMismatchCompatible(binding, claim, claimText) {
  if (!binding || typeof binding !== "object") return false;
  const mt = String(binding.matchType || "").toLowerCase();
  if (mt === "partial_support" || mt === "paraphrase") return true;
  const rc = String(binding.reasonCode || "").toLowerCase();
  if (rc.includes("quantity") && rc.includes("mismatch")) return true;
  const ex = String(binding.excerpt || binding.snippet || binding.boundExcerpt || "").toLowerCase();
  const SOURCE_CUES = ["investing", "investment", "evaluating", "up to"];
  if (!SOURCE_CUES.some((c) => ex.includes(c))) return false;
  const moneyTypes = new Set(["investment_amount", "metric_amount", "valuation"]);
  const ct = (claim?.type && String(claim.type).trim().toLowerCase()) || "";
  const hasMoneyClaim = moneyTypes.has(ct) || ct.startsWith("valuation_");
  const stmtLower = (claimText || "").toLowerCase();
  const hasStatementCue = ["raised", "financing round", "round", "series"].some((cue) => stmtLower.includes(cue));
  return hasMoneyClaim && hasStatementCue;
}

/**
 * A6.49k: Direct confirming support — exact / rounded / unit only; excludes partial, paraphrase,
 * placeholders, and mismatch-compatible bindings.
 */
function isDirectConfirmingSupportBinding(binding, claim, claimText) {
  if (!binding || typeof binding !== "object") return false;
  if (isBindingPlaceholderSynthetic(binding)) return false;
  const mt = String(binding.matchType || "").toLowerCase();
  const DIRECT = new Set(["exact", "rounded_equivalent", "unit_equivalent"]);
  if (!DIRECT.has(mt)) return false;
  if (mt === "partial_support" || mt === "paraphrase") return false;
  if (isBindingMismatchCompatible(binding, claim, claimText)) return false;
  return true;
}

/** A6.49k: Count of direct confirming bindings for this claim (for inference skip + diagnostics). */
function countDirectSupportingBindings(bindings, claim, claimText) {
 ...
 */
function inferQuantityTypeMismatch(stmt, claimText, uploadedDocs, claim) {
  ...
  const directSupportingBindingsCount = countDirectSupportingBindings(bindings, claim, claimText);
  if (directSupportingBindingsCount >= 1) return null;
  ...
  return {
    ...
    explanation:
      "The cited source describes an investor investment amount (e.g., investing, evaluating, or up to), which is not the same as the total round size raised in your claim.",
  };
}
...
function buildWhyItMattersFromExplanation(explanationCode, valueSummary = null) {
  if (explanationCode === "CONFLICT_VALUE" && valueSummary) return `Supports the ${valueSummary} reading.`;
  if (explanationCode === "FULL_CONFIRM") return "Directly confirms the claim.";
  if (explanationCode === "PARTIAL_MODIFIER") return "Confirms the main point but not every detail in your sentence.";
  if (explanationCode === "RELATED_CONTEXT") return "Discusses the topic but does not confirm this sentence.";
  if (explanationCode === "NO_SUPPORT") return "No source directly confirms this claim.";
...
function buildHoverPayload(classification, selectedEvidence, selectedExcerptText, whyItMattersText, originalSentenceText) {
  const fallbackWhy =
    whyItMattersText
    ?? (classification === "full"
      ? "Directly confirms this claim."
      : classification === "partial"
        ? "Confirms the main point but not every detail in your sentence."
        : "See commentary.");
  return {
...
export function runQcV2Pipeline(statements, context) {
  ...
  for (let idx = 0; idx < (statements || []).length; idx++) {
    ...
    for (let cIdx = 0; cIdx < canonicalClaims.length; cIdx++) {
      ...
      const directSupportingBindingsCount = countDirectSupportingBindings(supportBindingsArr, claim, claimText);
      const supportMismatchEffective = inferQuantityTypeMismatch(stmt, claimText, uploadedDocsForMismatch, claim);
      const hadMetaMismatch =
        stmt.meta?.supportMismatch?.kind != null
        && typeof stmt.meta?.supportMismatch?.explanation === "string"
        && String(stmt.meta.supportMismatch.explanation).trim();
      let quantityMismatchInferenceSkipped = false;
      let skipReason = "none";
      if (hadMetaMismatch) {
        skipReason = "meta_supportMismatch";
      } else if (directSupportingBindingsCount >= 1) {
        quantityMismatchInferenceSkipped = true;
        skipReason = "direct_support_binding";
      } else if (supportMismatchEffective?.kind != null) {
        skipReason = "inferred_quantity_mismatch";
      } else {
        skipReason = "heuristic_no_quantity_mismatch";
      }
      const mismatchPartialEligible =
        classification !== "conflict"
        && supportMismatchEffective?.kind != null
      ...
      if (!downgradedForNoExcerpt && mismatchPartialEligible && supportMismatchEffective?.explanation) {
        commentaryPayload = String(supportMismatchEffective.explanation).trim();
      }
