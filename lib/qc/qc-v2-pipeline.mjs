 ...
 */
function passesExcerptQualityGate(ev, components, claimType, refsById = new Map(), gateLog = null) {
  const diagVerbose = typeof process !== "undefined" && process.env?.BRIGHTLINE_DIAG_VERBOSE === "1";
...
/**
 * A6.49e: A4.70-aligned quantity-type mismatch (meta or inferred when no support bindings).
 */
function inferQuantityTypeMismatch(stmt, claimText, uploadedDocs) {
...
function mismatchPartialEvaluationsOk(evaluations, refsById, components, claimType, gateLog = null) {
  return evaluations.some((ev) => {
    ...
    if (!passesExcerptQualityGate(ev, components, claimType, refsById, gateLog)) return false;
...
export function runQcV2Pipeline(statements, context) {
  ...
  for (let idx = 0; idx < (statements || []).length; idx++) {
    ...
    for (let cIdx = 0; cIdx < canonicalClaims.length; cIdx++) {
      ...
      const supportMismatchEffective = inferQuantityTypeMismatch(stmt, claimText, uploadedDocsForMismatch);
      const mismatchPartialEligible =
        classification !== "conflict"
        && supportMismatchEffective?.kind != null
        && mismatchPartialEvaluationsOk(evaluations, refsById, components, claimType, { log, claimId });
      ...
      evaluations.forEach((ev) => {
        ...
        if (!passesExcerptQualityGate(ev, components, claimType, refsById, { log, claimId })) {
