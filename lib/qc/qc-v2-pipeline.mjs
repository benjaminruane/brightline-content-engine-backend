...
/**
 * A6.46: Evidence-driven plain-language card commentary (no banned reviewer terms).
 */
function buildPlainLanguageCommentary(classification, claimText, components, missingComponents, conflictingComponents, conflictOpts) {
  const claim = (claimText || "").trim();
  const lower = claim.toLowerCase();

  if (classification === "conflict" && conflictOpts.valueA != null && conflictOpts.valueB != null) {
    const comp = conflictingComponents?.[0];
    const aspect = comp === "amount" ? "the funding amount" : comp === "target" ? "who the product was intended for" : "this point";
    return `Sources disagree on ${aspect}. One source refers to ${conflictOpts.valueA}, while another refers to ${conflictOpts.valueB}.`;
  }
  ...
  if (classification === "full") {
    if (claim.length <= 200) {
      const body = claim.endsWith(".") ? claim.slice(0, -1) : claim;
      return `The source confirms ${body}.`;
    }
    return "The source confirms this claim.";
  }
  if (classification === "partial") {
    const lead = leadClaimFragment(claim);
    const tail = tailClaimFragment(claim);
    if (lead && tail) {
      return `The source confirms ${lead.endsWith(".") ? lead.slice(0, -1) : lead}, but it does not say ${tail.endsWith(".") ? tail.slice(0, -1) : tail}.`;
    }
    if (components?.target && /large\s+merchant|intended\s+for/i.test(lower)) {
      const head = lead || claim.split(",")[0]?.trim() || claim.slice(0, 120);
      return `The source confirms ${head.replace(/\.$/, "")}, but it does not say the platform was intended for large merchants.`;
    }
    if ((missingComponents || []).includes("projection_or_expectation") || components?.certainty) {
      return `The source confirms what ${components?.entity || "the company"} stated in the source, but it does not support the outlook as written in this sentence.`;
    }
    if (lead) return `The source confirms ${lead.replace(/\.$/, "")}, but it does not support everything in this sentence as written.`;
    return "No source directly confirms this claim.";
  }
  if (classification === "related") {
    if (/\$[\d,.]+\s*(?:million|billion|m|bn)?/i.test(claim) && components?.amount) {
      return `The sources refer to a ${components.amount} investment in the round, which is not the same as the company raising that amount in funding.`;
    }
    if (/merchant\s+demand|integrated\s+payments|shift/i.test(lower)) {
      return "The source discusses broader platform adoption, but it does not say merchant demand is shifting toward integrated payments platforms.";
    }
    return "The sources discuss the topic, but they do not support this specific claim.";
  }
  return "No source directly confirms this claim.";
}

/** A6.46: Per-citation popup sections (deterministic). */
function buildPopupSectionsForDisplayItem(item, claimText, claimLevelClassification) {
...
export function runQcV2Pipeline(statements, context) {
  ...
  for (let idx = 0; idx < (statements || []).length; idx++) {
    ...
    for (let cIdx = 0; cIdx < canonicalClaims.length; cIdx++) {
      ...
      let verdictPayload = classification === "conflict" ? "conflicting_sources" : classification === "full" ? "confirmed" : classification === "partial" ? "partially_confirmed" : "no_clear_support";
      ...
      // A6.46: Do not show citations that imply support when the claim is only "related" / none.
      if (classification === "related" || classification === "none") {
      ...
      for (const raw of displaySourceItems) {
        const popup = buildPopupSectionsForDisplayItem(raw, claimText, classification);
      ...
      let commentaryPayload;
      if (downgradedForNoExcerpt || (effectiveClassification === "none" && !downgradedForNoExcerpt)) {
        commentaryPayload = "No source directly confirms this claim.";
      } else {
        commentaryPayload = buildPlainLanguageCommentary(effectiveClassification, claimText, components, missingComponents, conflictingComponents, conflictOpts);
      }
