      ...
      if (excerpt) {
        ...
        if (
          ...
          if (oldWordCount < 2 || highValueWordsMissingFromExcerpt(claimText, excerpt).length > 0) {
            ...
            if (bestStart != null) {
              const windowLen = Math.min(300, Math.max(0, docFullText.length - bestStart));
              const centerIdx = bestStart + Math.floor(windowLen / 2);
              const newExcerpt = extractExcerptForPassage(docFullText, centerIdx, 300);
              if (newExcerpt) {
                const newWords = findA650jContentWordsInExcerpt(contentWords, newExcerpt);
                excerpt = newExcerpt;
                excerptOptimised = true;
                console.log(
                  `[A6.53] excerpt replaced: refId=${refId} oldWords=${oldWordCount} newWords=${newWords.length} claimPreview=${(claimText || "").slice(0, 50)}`,
                );
              }
            }
          }
        ...
        if (!isWeb && ct655b !== "numeric_finance") {
          ...
          if (bounds) {
            ...
            const spanStart = Math.max(0, rawMin - 80);
        ...
        if (
          ...
          if (missingBefore.length > 0) {
            ...
            if (bestStart56 != null) {
              const windowLen56 = Math.min(300, Math.max(0, docFullText56.length - bestStart56));
              const centerIdx56 = bestStart56 + Math.floor(windowLen56 / 2);
              const newExcerpt56 = extractExcerptForPassage(docFullText56, centerIdx56, 300);
              if (newExcerpt56) {
                ...
                if (newHvCount > oldHvCount) {
                  ...
                  excerptOptimised = true;
                  console.log(
                    `[A6.56] high-value word recovery: refId=${refId} missingBefore=${missingBefore.join(",")} recoveredWords=${recoveredWords.join(",")} claimPreview=${(claimText || "").slice(0, 50)}`,
                  );
                }
              }
            }
...
function findBestPassageForClaim(claimText, claimType, docFullText) {
  ...
  for (let start = 0; start < len; start += step) {
    ...
    const n = findA650jContentWordsInExcerpt(contentWords, windowText).length;
    if (n > bestCount || (n === bestCount && start < bestStart)) {
      bestCount = n;
      bestStart = start;
    }
  }
