...
function phrasesForMissingElements(missingComponents) {
  ...
  const parts = [];
  if (miss.includes("target_market")) parts.push("the intended audience");
...
function buildTypedExplanationFromSignals({
  ...
  switch (explanationType) {
    case "mismatch":
      ...
      break;
    case "partial": {
      const supported = phrasesForSupportedElements(confirmedComponents || [], components);
      const missing = phrasesForMissingElements(missingComponents || []);
      text = `The source confirms ${supported}, but it does not say ${missing}.`;
      break;
    }
    case "confirm":
...
export function runQcV2Pipeline(statements, context) {
  ...
  for (let idx = 0; idx < (statements || []).length; idx++) {
    ...
    for (let cIdx = 0; cIdx < canonicalClaims.length; cIdx++) {
      ...
      const commentaryPayload = typedExplanation.text;
