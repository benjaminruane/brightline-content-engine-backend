 ...
 * @param {unknown[]} [uploadedDocs] Optional full uploaded docs (A6.53 excerpt refinement).
 * @param {string|null} [claimType] From detectClaimType — used for A6.53 numeric_finance guard.
 */
...
function extractConfirmingSentence(excerpt, claimText) {
  ...
  const allWords = extractA650jContentWords(claimText);
  let bestIdx = -1;
  let bestScore = -1;
  for (let i = 0; i < parts.length; i++) {
    const s = parts[i];
    const hvFound = findA650jContentWordsInExcerpt(highValueWords, s).length;
    const regFound = findA650jContentWordsInExcerpt(allWords, s).length;
    const score = (2 * hvFound) + regFound;
    if (score > bestScore) {
      bestScore = score;
      bestIdx = i;
    }
...
function findBestPassageForClaim(claimText, claimType, docFullText) {
  ...
  for (let start = 0; start < len; start += step) {
    ...
    if (weightedScore > bestWeightedScore || (weightedScore === bestWeightedScore && start < bestStart)) {
...
export async function runQcV2Pipeline(statements, context) {
  ...
  for (let idx = 0; idx < (statements || []).length; idx++) {
    ...
    for (let cIdx = 0; cIdx < canonicalClaims.length; cIdx++) {
      ...
      /** A7.1: LLM verifier for commentary + mechanically resolved excerpt (uploaded source only). */
      let a71Applied = null;
      if (verdictBasisClassification === "full" || verdictBasisClassification === "partial" || verdictBasisClassification === "conflict") {
        const uploadedCand = candidates.find((c) => c.sourceOrigin === "uploaded");
        const sourceFullTextA71 = uploadedCand
          ? getUploadedDocFullTextForRef(contextUploadedDocs, uploadedCand.refId)
          : "";
        if (typeof sourceFullTextA71 === "string" && sourceFullTextA71.trim()) {
          ...
          try {
            resultA71 = await Promise.race([
              verifyClaimWithLLM(claimText, sourceFullTextA71, verdictHint, {
          ...
          const validatedA71 = validateLLMVerifierOutput(resultA71, sourceFullTextA71);
          if (validatedA71.valid) {
            console.log("[A7.1] verifier used", { claimId, hasPassage: validatedA71.passage != null });
            a71Applied = {
              explanation: validatedA71.explanation,
              passage: validatedA71.passage,
              uploadedCand,
            };
          } else {
            console.log("[A7.1] verifier fallback", { claimId, failReason: validatedA71.failReason });
          }
        }
      }
      ...
      if (!downgradedForNoExcerpt && a71Applied) {
        if (a71Applied.passage != null && a71Applied.uploadedCand) {
          authoritySelectedExcerptText = a71Applied.passage;
          const uc = a71Applied.uploadedCand;
          authoritySelectedEvidence = {
            refId: uc.refId,
            sourceType: "uploaded",
            title: uc.displayTitle,
            url: refsById.get(String(uc.refId))?.url ?? null,
          };
        }
      }
      ...
      if (!downgradedForNoExcerpt && authoritySelectedEvidence && authoritySelectedExcerptText) {
        const candForHover =
          a71Applied?.uploadedCand
          && authoritySelectedEvidence
          && String(authoritySelectedEvidence.refId) === String(a71Applied.uploadedCand.refId)
            ? a71Applied.uploadedCand
            : selectedCandidate;
        hoverPayload = buildHoverPayload(
          effectiveClassification,
          candForHover ? { ...candForHover, displayTitle: candForHover.displayTitle } : null,
