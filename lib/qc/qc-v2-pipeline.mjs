...
function mismatchPartialEvaluationsOk(evaluations, refsById, components, claimType) {
  return evaluations.some((ev) => {
    ...
    if (!passesExcerptQualityGate(ev, components, claimType)) return false;
  ...
  for (let idx = 0; idx < (statements || []).length; idx++) {
    ...
    for (let cIdx = 0; cIdx < canonicalClaims.length; cIdx++) {
      ...
      const mismatchPartialEligible =
        ...
        && mismatchPartialEvaluationsOk(evaluations, refsById, components, claimType);
