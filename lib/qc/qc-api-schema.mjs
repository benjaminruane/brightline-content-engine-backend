...
export function validateQcResponse(qcResponse) {
  ...
  for (let i = 0; i < statements.length; i++) {
    ...
    const coverageUnion = qcCard.coverageUnion;
    if (coverageUnion !== undefined) {
      if (coverageUnion == null || typeof coverageUnion !== "object" || Array.isArray(coverageUnion)) {
        throw Object.assign(
          new Error(`qcCard.coverageUnion must be an object when present (statement index ${i})`),
          { code: QC_API_SCHEMA_VIOLATION, statementIndex: i }
        );
      }
      if (typeof coverageUnion.promoted !== "boolean") {
        throw Object.assign(
          new Error(`qcCard.coverageUnion.promoted must be a boolean (statement index ${i})`),
          { code: QC_API_SCHEMA_VIOLATION, statementIndex: i }
        );
      }
      if (!Array.isArray(coverageUnion.contributingSourceIndices)) {
        throw Object.assign(
          new Error(
            `qcCard.coverageUnion.contributingSourceIndices must be an array (statement index ${i})`
          ),
          { code: QC_API_SCHEMA_VIOLATION, statementIndex: i }
        );
      }
      for (let c = 0; c < coverageUnion.contributingSourceIndices.length; c++) {
        if (typeof coverageUnion.contributingSourceIndices[c] !== "number") {
          throw Object.assign(
            new Error(
              `qcCard.coverageUnion.contributingSourceIndices[${c}] must be a number (statement index ${i})`
            ),
            { code: QC_API_SCHEMA_VIOLATION, statementIndex: i }
          );
        }
      }
      if (!Array.isArray(coverageUnion.union)) {
        throw Object.assign(
          new Error(`qcCard.coverageUnion.union must be an array (statement index ${i})`),
          { code: QC_API_SCHEMA_VIOLATION, statementIndex: i }
        );
      }
      for (let u = 0; u < coverageUnion.union.length; u++) {
        const pair = coverageUnion.union[u];
        if (
          !Array.isArray(pair) ||
          pair.length !== 2 ||
          ...
          throw Object.assign(
            new Error(
              `qcCard.coverageUnion.union[${u}] must be a [start, end] number pair (statement index ${i})`
            ),
            { code: QC_API_SCHEMA_VIOLATION, statementIndex: i }
          );
        }
