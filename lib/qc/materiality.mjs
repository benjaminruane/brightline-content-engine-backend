...
const TITLE_CASE_NAME_RE = /\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+){1,3}\b/g;

/**
 * THE AUTHOR IS NOT AN ATTRIBUTION.
 *
 * This feature asks whether a statement attributes something to a named person
 * or entity. A sentence naming only the authoring organisation attributes
 * nothing to anyone else — it is the author speaking about itself — so firing
 * here overstated the materiality of the client's own statements. The register
 * already filtered this feature out downstream at a5be4f0; this stops it being
 * produced in the first place, so every consumer sees the same answer.
 *
 * A statement naming the author AND someone else still fires, on the someone
 * else. Where no organisation is configured nothing is suppressed.
 */
function namesSomeoneOtherThanTheAuthor(text) {
  const matches = String(text ?? "").match(TITLE_CASE_NAME_RE) ?? [];
  return matches.some((m) => !isAuthoringOrganisationName(m));
}
...
export function extractStatementFeatures(statement) {
  ...
  if (
    ...
    namesSomeoneOtherThanTheAuthor(t)
  ) {
