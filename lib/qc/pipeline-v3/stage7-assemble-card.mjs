...
import { computeCardMateriality } from "../materiality.mjs";
import {
  buildFramingFidelityConcern,
  collectFramingEvidence,
  detectFramingFidelity,
} from "../framing-fidelity.mjs";
import {
  buildSourceRecencyConcern,
  detectSourceRecency,
  selectRecencySource,
} from "../source-recency.mjs";
import { gateExcerpt, rewriteSpanFromSource } from "../excerpt-locate.mjs";
import { logCheckNotReviewed, NOT_REVIEWED_REASONS } from "../not-reviewed-reason.mjs";
...
function mapVerdictToSupportState(verdict) {
  ...
  if (verdict === "not_reviewed") return "skipped";
...
function mapSupportStateToDisplayVerdict(supportState) {
  ...
  if (supportState === "skipped") return "not reviewed";
  return "not_supported";
}

/**
 * B322. A supported card with no locatable excerpt must not read as Confirmed.
 * supportState stays the Stage 3 result. displayVerdict is the reviewer-facing claim.
 */
function applyExcerptHonestyToDisplay({ displayVerdict, hasRealExcerpt, excerptNotLocatable }) {
  ...
  const excerptAbsent = hasRealExcerpt !== true;
  const excerptUnusable = excerptNotLocatable === true;
  if (!excerptAbsent && !excerptUnusable) {
    return { displayVerdict, evidenceNotReviewedReason: null };
  }
  return {
...
function mapSupportStateToEvidenceConcernLevel(supportState) {
  if (supportState === "supported") return "none";
  if (supportState === "partial") return "moderate";
  if (supportState === "conflicting") return "high";
  if (supportState === "skipped") return "none";
  return "high";
}

function recencySourceIndices(entry) {
  ...
  return matches
    .filter((m) => {
      const c = typeof m?.classification === "string" ? m.classification.trim() : "";
      return c.length > 0 && c !== "no_support" && c !== "not_reviewed";
    })
    .map((m) => m.sourceIndex)
    .filter((v) => Number.isFinite(v));
}

function buildSourceRecencyConcerns(entry, statement, assemblyContext) {
  const sources = Array.isArray(assemblyContext?.sources) ? assemblyContext.sources : [];
  if (sources.length === 0) return [];
  const chosen = selectRecencySource(sources, recencySourceIndices(entry));
  if (!chosen?.source || typeof chosen.source.text !== "string") return [];
  const today =
    assemblyContext?.today instanceof Date && !Number.isNaN(assemblyContext.today.getTime())
      ? assemblyContext.today
      : undefined;
  const det = detectSourceRecency({
    statement,
    sourceText: chosen.source.text,
    today,
  });
  if (!det.fire || typeof det.note !== "string" || !det.note.trim()) return [];
  return [buildSourceRecencyConcern({ statement, note: det.note })];
}

async function buildFramingFidelityConcerns(entry, statement, verdict, statementIndex, assemblyContext) {
  if (verdict !== "confirmed" && verdict !== "partially_confirmed") return [];
  const { passages, evidenceSummary } = collectFramingEvidence(entry);
  if (passages.length === 0) return [];
  const runJudge =
    typeof assemblyContext?.framingFidelityJudge === "function"
      ? assemblyContext.framingFidelityJudge
      : undefined;
  ...
  if (!det.fire || typeof det.note !== "string" || !det.note.trim()) return [];
  return [buildFramingFidelityConcern({ statement, note: det.note })];
}

function reviewEnabled(assemblyContext, key) {
  const opts = assemblyContext?.reviewOptions;
  if (!opts || typeof opts !== "object") {
    console.warn(`[ASSEMBLE_CARD] reviewOptions missing; ${key} treated as not requested`);
    return false;
  }
  return opts[key] === true;
}

const KNOWN_ASSEMBLED_VERDICTS = new Set([
  "clean",
  "concern",
  "soft_concern",
  "hard_concern",
...
function resolveAssembledVerdict(raw, enabled) {
  const v = typeof raw === "string" ? raw.trim() : "";
  if (enabled !== true) return "not_reviewed";
  if (KNOWN_ASSEMBLED_VERDICTS.has(v)) return v;
  return "not_reviewed";
}

function safeEditorialDefaults(assemblyContext = {}) {
  return {
...
function safeCard(statementIndex = 0, assemblyContext = {}) {
  const editorial = safeEditorialDefaults(assemblyContext);
  ...
  return {
    ...
    evidenceNotReviewedReason: null,
    concernLevel: mapSupportStateToEvidenceConcernLevel(supportState),
    evidenceConcernLevel: mapSupportStateToEvidenceConcernLevel(supportState),
 ...
 * @param {Array<{ text?: string, label?: string }>} [assemblyContext.sources]
 * @param {Date} [assemblyContext.today]
 * @param {boolean} [assemblyContext.skipEditorialDuplicationJudge] diagnostic-only; production omit
 * @param {Function} [assemblyContext.framingFidelityJudge] test-only override
 * @param {{ evidenceEnabled?: boolean, editorialEnabled?: boolean, complianceEnabled?: boolean }} [assemblyContext.reviewOptions]
 */
export async function assembleCard(statementEntry, statementIndex, assemblyContext = {}) {
  try {
    ...
    const concernLevel = mapSupportStateToEvidenceConcernLevel(supportState);
    const hasConflict = entry?.verdictResult?.hasConflict === true;
    const primaryExcerpt = entry?.excerptResult?.primaryExcerpt ?? null;
    const conflictExcerptRaw = entry?.excerptResult?.conflictExcerpt ?? null;
    ...
    const editorialDefaults = safeEditorialDefaults(assemblyContext);
    const editorialEnabled = reviewEnabled(assemblyContext, "editorialEnabled");
    const complianceEnabled = reviewEnabled(assemblyContext, "complianceEnabled");
    const editorial = entry?.editorialResult && typeof entry.editorialResult === "object"
      ? {
          editorialVerdict: resolveAssembledVerdict(
          ...
          editorialSuggestedRewrite:
            typeof entry.editorialResult.editorialSuggestedRewrite === "string"
              ...
              : null,
          complianceVerdict: resolveAssembledVerdict(
          ...
          editorialNotReviewedReason:
            typeof entry.editorialResult.editorialNotReviewedReason === "string"
              ? entry.editorialResult.editorialNotReviewedReason
              : null,
          complianceNotReviewedReason:
            typeof entry.editorialResult.complianceNotReviewedReason === "string"
              ? entry.editorialResult.complianceNotReviewedReason
              : null,
        }
      : editorialDefaults;
    ...
    if (
      pipelineRoute === "v4" &&
      verdict === "conflicting" &&
      assemblyContext?.skipEditorialDuplicationJudge !== true &&
      ...
      if (kept.length !== original.length) {
        const incomingVerdict =
          typeof editorial.editorialVerdict === "string" ? editorial.editorialVerdict.trim() : "";
        if (incomingVerdict === "not_reviewed") {
          editorialOut = {
            ...editorial,
            editorialConcerns: kept,
            editorialVerdict: "not_reviewed",
            editorialNote: typeof editorial.editorialNote === "string" ? editorial.editorialNote : "",
          };
        } else {
          editorialOut = {
            ...editorial,
    ...
    const commentaryNotReviewed =
      entry?.commentaryResult?.notReviewed === true ||
      entry?.commentaryResult?.schemaValid === false ||
      rawCommentary.includes("Specific commentary is unavailable from the system");
    const evidenceSummary = commentaryNotReviewed ? "" : rawCommentary;

    // R7 build A — additive display field only; never feeds verdict/excerpt selection.
    const supportSpans = Array.isArray(entry.supportSpans) ? entry.supportSpans : [];
    const rawPrimaryPassage = typeof primaryExcerpt?.passage === "string" ? primaryExcerpt.passage : "";
    ...
    const primaryPassage = gatedPrimary?.passage ?? "";
    const hasRealExcerpt = primaryPassage.trim().length > 0;
    const passageRejected = (Array.isArray(entry.sourceMatches) ? entry.sourceMatches : []).some(
      (m) => m?.passageRejected === true
    );
    const excerptNotLocatable =
      passageRejected || (rawPrimaryPassage.trim().length > 0 && gatedPrimary == null);
    const honesty = applyExcerptHonestyToDisplay({
      displayVerdict,
      hasRealExcerpt,
      excerptNotLocatable,
    });
    const honestDisplayVerdict = honesty.displayVerdict;
    const evidenceNotReviewedReason = honesty.evidenceNotReviewedReason;
    if (evidenceNotReviewedReason) {
      logCheckNotReviewed({
        kind: "evidence",
        statementIndex: stmtIdx,
        reason: evidenceNotReviewedReason,
      });
    }
    let conflictExcerpt = conflictExcerptRaw;
    const conflictPassage =
      conflictExcerptRaw && typeof conflictExcerptRaw.passage === "string"
        ? conflictExcerptRaw.passage
        : "";
    const conflictLabel =
      conflictExcerptRaw && typeof conflictExcerptRaw.sourceLabel === "string"
        ? conflictExcerptRaw.sourceLabel
        : "";
    if (conflictPassage.trim() && conflictLabel.trim()) {
      const gatedConflict = gateExcerpt({
        passage: conflictPassage,
        sourceLabel: conflictLabel,
        sources: assemblyContext?.sources,
        supportSpans,
      });
      if (gatedConflict?.passage) {
        conflictExcerpt = { ...conflictExcerptRaw, passage: gatedConflict.passage };
      } else {
        conflictExcerpt = { ...conflictExcerptRaw, passage: null };
      }
    }
    const outSupportSpans = supportSpans.map((span) => rewriteSpanFromSource(span, assemblyContext?.sources));
    const unsupportedSpans = Array.isArray(entry.unsupportedSpans) ? entry.unsupportedSpans : [];
    const stage2SourceFingerprints = (Array.isArray(entry.sourceMatches) ? entry.sourceMatches : []).map(
      (m) => ({
    ...
    const sourceRecencyConcerns = buildSourceRecencyConcerns(entry, statement, assemblyContext);
    const framingFidelityConcerns = await buildFramingFidelityConcerns(
      entry,
      statement,
      verdict,
      stmtIdx,
    ...
    const materiality = computeCardMateriality({
      statement,
    ...
    const card = {
      ...
      supportState,
      hasConflict,
      primaryExcerpt: hasRealExcerpt ? primaryPassage : null,
      conflictExcerpt,
      evidenceSummary,
      commentaryNotReviewed,
      commentaryNotReviewedReason: commentaryNotReviewed
        ? typeof entry?.commentaryResult?.notReviewedReason === "string"
          ? entry.commentaryResult.notReviewedReason
          : null
        : null,
      ...
      supportRefTitles,
      primaryRefId: null,
      primaryRefTitle: gatedPrimary?.sourceLabel ?? primaryExcerpt?.sourceLabel ?? null,
      primarySourceOrigin: null,
      primaryExcerptText: hasRealExcerpt ? primaryPassage : null,
      excerptNotLocatable,
      primaryExcerptStart: hasRealExcerpt && Number.isFinite(gatedPrimary?.start) ? gatedPrimary.start : null,
      primaryExcerptEnd: hasRealExcerpt && Number.isFinite(gatedPrimary?.end) ? gatedPrimary.end : null,
      secondarySupportCount: 0,
      supportingReferenceIds: [],
      supportingReferenceTitles: [],
      hasRealExcerpt,
      ...
      suggestedImprovement: null,
      whyItMatters: null,
      displayVerdict: honestDisplayVerdict,
      evidenceNotReviewedReason,
      concernLevel,
      evidenceConcernLevel: concernLevel,
      sentenceSubclaimCount: null,
      qcClaimId: null,
      ...
      conflictEvidence: null,
      pipelineVersion: pipelineRoute,
      supportSpans: outSupportSpans,
      unsupportedSpans,
      stage2SourceFingerprints,
      materiality,
      sourceRecencyConcerns,
      framingFidelityConcerns,
      supersededSourceNotes: Array.isArray(entry.supersededSourceNotes) ? entry.supersededSourceNotes : [],
      ...editorialOut,
    };
    if (entry.claimSpans && typeof entry.claimSpans === "object") {
      card.decomposed = entry.claimSpans.decomposed === true;
      card.claimUpgrade = entry.claimSpans.claimUpgrade === true;
      card.claims = Array.isArray(entry.claimSpans.claims) ? entry.claimSpans.claims : [];
    }
    if (entry.coverageUnion && typeof entry.coverageUnion === "object") {
      card.coverageUnion = entry.coverageUnion;
    }
    return card;
  } catch {
    return safeCard(statementIndex, assemblyContext);
  }
}
