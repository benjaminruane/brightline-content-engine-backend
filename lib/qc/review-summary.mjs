...
export function turnedOffLineFromClass(cls) {
  ...
  const names = [];
  if (cls.evidence == null) names.push(CHECK_DISPLAY_NAMES.evidence);
  if (cls.editorial == null) names.push(CHECK_DISPLAY_NAMES.editorial);
  if (cls.compliance == null) names.push(CHECK_DISPLAY_NAMES.compliance);
  if (names.length === 0) return null;
  return `${TURNED_OFF_LINE_PREFIX}: ${names.join(", ")}.`;
}
...
const CARD_TONE_AMBER = new Set(["partial", "concern", "notChecked"]);
...
export function summariseReview(cards, reviewOptions) {
  ...
  for (const row of counted) {
    ...
    if (cls.editorial === "notChecked" || cls.compliance === "notChecked" || cls.evidence === "notChecked") {
      notChecked += 1;
    }
