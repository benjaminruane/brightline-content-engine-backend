...
export function isCardFullyClean(card, reviewOptions = {}) {
  const { evidenceEnabled, editorialEnabled, complianceEnabled } = asReviewOptions(reviewOptions);
  return (
...
function collectEvidenceInput(card) {
  if (card?.commentaryNotReviewed === true) return "";
  ...
  if (summary.includes("Specific commentary is unavailable from the system")) return "";
  if (reasoning.includes("Specific commentary is unavailable from the system")) return "";
