...
export function collectMarginNotes(rows, reviewOptions = {}, draftText = "") {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    if (evidenceEnabled && isEvidenceConfirmed(card) === false) {
      const cardNote = collectEvidenceInput(card) || "Sources do not confirm this statement.";
      notes.push({
        kind: "evidence",
        cardIndex,
        statementText,
    ...
    if (editorialEnabled && !isEditorialClean(card)) {
      ...
      if (concerns.length === 0) {
        notes.push({
          ...
          cardIndex,
          statementText,
          cardNote: "Editorial concern on this sentence.",
          draftSays: "",
          sourceSays: "",
        });
      } else {
    ...
    if (complianceEnabled && !isComplianceClean(card)) {
      ...
      if (concerns.length === 0) {
        notes.push({
          ...
          cardIndex,
          statementText,
          cardNote: "Compliance concern on this sentence.",
          draftSays: "",
          sourceSays: "",
        });
      } else {
...
function pieceCoversNote(piece, note) {
  ...
  const fragment = foldForCoverage(quoteFragment(note.statementText, 8));
  if (fragment && text.includes(fragment)) return true;
  if (note.draftSays && text.includes(foldForCoverage(note.draftSays))) return true;
  const cardFrag = foldForCoverage(quoteFragment(note.cardNote, 6));
  if (cardFrag && text.includes(cardFrag)) return true;
  return false;
}
...
export function appendCoverageRemainder(piece, notes) {
  ...
  const missing = list.filter((note) => !pieceCoversNote(text, note));
