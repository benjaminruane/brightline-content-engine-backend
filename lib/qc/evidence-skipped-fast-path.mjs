...
export async function finalizeEvidenceSkippedReview(ctx) {
  ...
  const { editorialEnabled, complianceEnabled } = asReviewOptions(body);
  ...
  try {
    ...
    const fromRoot =
      typeof body?.authoringOrganisation === "string" && body.authoringOrganisation.trim()
        ...
        : "";
    logAuthoringOrganisationResolution({ argument: fromOptions, request: fromRoot });
    const authoringOrganisation = fromOptions || fromRoot || null;
