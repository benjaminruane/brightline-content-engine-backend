...
/**
 * Production default. Unset AUTHORING_ORGANISATION keeps live behaviour.
 * Harnesses and tests override the env var in-process; they do not change
 * this default or any deployed configuration.
 */
export const DEFAULT_AUTHORING_ORGANISATION = "Partners Group";

/** Process env key. Empty or whitespace falls back to DEFAULT_AUTHORING_ORGANISATION. */
 ...
 * @returns {string}
 */
export function resolveAuthoringOrganisationName() {
  const raw = process.env[AUTHORING_ORGANISATION_ENV];
  if (typeof raw === "string" && raw.trim()) return raw.trim();
  return DEFAULT_AUTHORING_ORGANISATION;
}
...
export function buildFirstPersonActorInstruction(houseName = resolveAuthoringOrganisationName()) {
  const house = String(houseName || DEFAULT_AUTHORING_ORGANISATION).trim() || DEFAULT_AUTHORING_ORGANISATION;
 ...
 * rules cannot drift. Built at module load from the current resolved house
 * name (production default, or AUTHORING_ORGANISATION if already set).
 */
export const FIRST_PERSON_ACTOR_INSTRUCTION = buildFirstPersonActorInstruction();
 ...
 * @param {string} [houseName] Explicit override for a fixture. Defaults to resolveAuthoringOrganisationName().
