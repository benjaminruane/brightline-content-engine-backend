...
/**
 * Canonical house voice for attaching FIRST_PERSON_ACTOR_INSTRUCTION and the
 * voice_consistency pronoun backstop. One place. Investor letter, press
 * release, and LinkedIn are first person. Reporting commentary is third.
 */
...
export const THIRD_PERSON_HOUSE_VOICE_SLUGS = Object.freeze(["reporting_commentary"]);
...
export function houseVoiceSlug(outputType) {
  const raw = String(outputType || "")
    ...
    .toLowerCase()
    .replace(/-/g, "_");
  if (raw === "reporting_commentary") return "reporting_commentary";
  if (raw === "investor_letter") return "investor_letter";
  if (raw === "press_release") return "press_release";
...
export function houseVoiceIsThirdPerson(outputType) {
  return houseVoiceSlug(outputType) === "reporting_commentary";
}

/** Logged when example wording would otherwise reach a reader. Fail loud; do not ship the string. */
export const PLACEHOLDER_LEAK = "PLACEHOLDER_LEAK";
...
export function buildFirstPersonActorInstruction(houseName = resolveAuthoringOrganisationName()) {
  ...
  Fault (first person): "We were attracted to X". Corrected (third person): "${house} was attracted to X"
  Fault (first person): "we believe the fund should Y". Corrected (third person): "${house} believes the fund should Y"
  Fault (first person): "we recommend the commitment". Corrected (third person): "${house} recommends the commitment"
  Fault (first person): "we would note that". Corrected (third person): "${house} notes that"
  Fault (first person): "available to us". Corrected (third person): "available to ${house}"
...
If that sentence subject is the authoring organisation, delete the marker. The sentence already attributes the judgement. A marker that repeats an owner the sentence already has is redundant rather than protective.
  Fault (redundant first-person marker): "${house} was attracted to X on the strength of a track record that is, in our view, exceptional". Corrected: "${house} was attracted to X on the strength of a track record that is exceptional"
...
If that sentence subject is not the authoring organisation, convert the marker. Removing it would leave an unattributed judgement.
  Fault (first-person marker on a third-person subject): "The pipeline is, in our view, thin". Corrected: "The pipeline is, in ${possessive} view, thin"

The authoring organisation is given to you, or it is not. When it is given, it has already been confirmed to appear in the draft. Never infer one. Do not substitute the name of the investment, fund, or portfolio company. If it is not given, raise the first-person concern. Do not recast into an agentless or passive construction, and do not invent a firm name. Do not tell the writer to keep the first-person wording. An unfixed style issue is far less harmful than a recast that turns a judgement into an unattributed statement of fact.
...
/**
 * Presence of the named organisation in text. Uses the same matcher as
 * identifyAuthoringOrganisation. An empty name is never present.
 * @param {unknown} text
 * @param {unknown} name
 * @returns {boolean}
 */
export function textContainsAuthoringOrganisation(text, name) {
  const n = trimAuthoringOrganisationName(name);
  if (!n) return false;
  return houseNameRe(n).test(String(text ?? ""));
}

/** Same pattern Review already uses to drop first_person_plural with no pronoun. */
export const FIRST_PERSON_PLURAL_RE = /\b(?:we|our|ours|us|we're|we've|we'll|we'd|ourselves)\b/i;

/**
 * @param {unknown} text
 * @returns {boolean}
 */
export function statementHasFirstPersonPronoun(text) {
  return FIRST_PERSON_PLURAL_RE.test(String(text ?? ""));
}
...
function containsUnresolvedAuthoringOrganisationPlaceholder(text) {
  const raw = String(text ?? "");
  if (!raw.trim()) return false;
  return raw.toLowerCase().includes(AUTHORING_ORGANISATION_EXAMPLE_PLACEHOLDER.toLowerCase());
}

/**
 * True when `text` contains the example placeholder, including a leading capital.
 * Logs PLACEHOLDER_LEAK. Does not rewrite. Callers blank the displayed field.
 * @param {unknown} text
 * @returns {boolean}
 */
export function assertNoUnresolvedAuthoringOrganisationPlaceholder(text) {
  if (!containsUnresolvedAuthoringOrganisationPlaceholder(text)) return false;
  console.error(PLACEHOLDER_LEAK);
  return true;
}

/**
 * The configured house may be written into a reader-visible sentence only when
 * that exact name is already in the draft. Same check editorial already uses.
 * @param {unknown} draftText
 * @param {unknown} houseName
 * @returns {boolean}
 */
export function authoringOrganisationWriteIsLicensed(draftText, houseName) {
  return identifyAuthoringOrganisation(draftText, houseName) != null;
}

/**
 * True when the result newly inserts the configured name and the draft-presence
 * check has not passed. A first-person pronoun does not license the insert.
 * @param {unknown} statement
 * @param {unknown} resultingSentence
 * @param {unknown} houseName
 * @param {unknown} [draftText]
 * @returns {boolean}
 */
export function resultAddsUnlicensedOrganisation(statement, resultingSentence, houseName, draftText) {
  const name = trimAuthoringOrganisationName(houseName);
  if (!name) return false;
  if (!textContainsAuthoringOrganisation(resultingSentence, name)) return false;
  if (textContainsAuthoringOrganisation(statement, name)) return false;
  const corpus =
    typeof draftText === "string" && draftText.trim() ? draftText : String(statement ?? "");
  return !authoringOrganisationWriteIsLicensed(corpus, name);
}

/**
 * On a placeholder hit in a public concern field, blank the direction so the
 * finding degrades to an acknowledgement. Keep the concern and its note.
 * @param {object} concern
 * @returns {object}
 */
export function blankPlaceholderLeaksInPublicConcern(concern) {
  if (!concern || typeof concern !== "object") return concern;
  const directionHit = containsUnresolvedAuthoringOrganisationPlaceholder(concern.suggestedDirection);
  const noteHit = containsUnresolvedAuthoringOrganisationPlaceholder(concern.note);
  const rewriteHit = containsUnresolvedAuthoringOrganisationPlaceholder(concern.suggestedRewrite);
  const cardDirectionHit = containsUnresolvedAuthoringOrganisationPlaceholder(
    concern.editorialSuggestedDirection
  );
  if (!directionHit && !noteHit && !rewriteHit && !cardDirectionHit) return concern;
  console.error(PLACEHOLDER_LEAK);
  const next = { ...concern };
  delete next.suggestedDirection;
  if (rewriteHit) delete next.suggestedRewrite;
  if (cardDirectionHit) delete next.editorialSuggestedDirection;
  return next;
}
...
export function formatAuthoringOrganisationPromptBlock(
  ...
  return (
    ...
    "Raise the first-person concern. Do not recast into an agentless or passive construction, and do not invent a firm name. Do not tell the writer to keep the first-person wording."
  );
}
