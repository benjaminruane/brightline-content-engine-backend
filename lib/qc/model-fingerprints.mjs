...
/** A pinned snapshot ends in -YYYY-MM-DD, or for some families -YYYYMMDD. */
const DATE_SUFFIX_RE = /-(\d{4}-\d{2}-\d{2}|\d{8})$/;

/**
 * Is this a pinned snapshot rather than a floating alias?
 *
 * @param {string} model
 * @returns {boolean}
 */
export function hasDateSuffix(model) {
  return typeof model === "string" && DATE_SUFFIX_RE.test(model);
}
