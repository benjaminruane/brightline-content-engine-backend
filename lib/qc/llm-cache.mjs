/**
 * B63/B69: LLM result cache for the verdict path (Stages 1, 1b, 2).
 *
 * Default persistence: process-global in-memory Map (production / Vercel).
 * Local diagnostics may set QC_LLM_CACHE_DISK to a file path; that loads the
 * file into a Map and write-throughs. Unset = memory only, nothing read or
 * written on disk. Not a production datastore. No new dependency.
 *
...
/**
 * File-backed store, same get/put/clear/size surface as createMemoryStore.
 * LRU caps are not applied: they exist to bound a warm Vercel instance.
 * The file is allowed to grow. Enabled only when a path is passed in.
 */
export function createDiskStore({ filePath } = {}) {
  const resolved = typeof filePath === "string" ? filePath.trim() : "";
  if (!resolved) {
    throw new Error("createDiskStore requires filePath");
  }
  const rows = new Map();
  ...
  function loadFromDisk() {
    ...
    try {
      ...
      lastFileBytes = Buffer.byteLength(raw, "utf8");
      const parsed = JSON.parse(raw);
      const entries = parsed && typeof parsed === "object" ? parsed.entries : null;
      if (!entries || typeof entries !== "object" || Array.isArray(entries)) {
        console.warn(`[QC_LLM_CACHE] disk file missing entries object; starting empty. path=${resolved}`);
        rows.clear();
        return;
      }
      for (const [key, entry] of Object.entries(entries)) {
        if (!key || !entry || typeof entry !== "object") continue;
        rows.set(key, { entry: cloneValue(entry), bytes: approximateBytes(entry) });
      }
      console.log(
        `[QC_LLM_CACHE] disk loaded path=${resolved} entries=${rows.size} fileBytes=${lastFileBytes}`
      );
    } catch (err) {
      console.warn(
        `[QC_LLM_CACHE] disk file unreadable; starting empty. path=${resolved} ${err?.message || String(err)}`
      );
      rows.clear();
      lastFileBytes = 0;
    }
  ...
  async function persist() {
    ...
    const payload = `${JSON.stringify({ format: 1, entries })}\n`;
    lastFileBytes = Buffer.byteLength(payload, "utf8");
  ...
  function enqueuePersist() {
    const job = persistChain.then(() => persist());
    persistChain = job.catch((err) => {
      console.warn(`[QC_LLM_CACHE] disk write failed. ${err?.message || String(err)}`);
    });
    return job;
  }
  ...
  return {
    ...
    async get(key) {
      if (!rows.has(key)) return null;
      return cloneValue(rows.get(key).entry);
    },
    async put(key, entry) {
      const stored = cloneValue(entry);
      const bytes = approximateBytes(stored);
    ...
    byteSize() {
      let n = 0;
      for (const row of rows.values()) n += Number(row.bytes) || 0;
      return n;
    },
    evictionCount() {
      return 0;
...
export function llmCacheDiskPathFromEnv() {
  const v = String(process.env.QC_LLM_CACHE_DISK || "").trim();
  return v || null;
}
...
function maybeAttachDiskStore() {
  const filePath = llmCacheDiskPathFromEnv();
  if (!filePath) return;
  if (activeStore && activeStore.kind === "disk" && activeStore.filePath === filePath) return;
  if (activeStore !== defaultStore) return;
  try {
    activeStore = createDiskStore({ filePath });
  } catch (err) {
    console.warn(`[QC_LLM_CACHE] disk store attach failed; using memory. ${err?.message || String(err)}`);
  }
}

export function getLlmCacheStore() {
  maybeAttachDiskStore();
  return activeStore;
}
...
export function logCacheRunSummary(summary, label = "run") {
  ...
  if (activeStore?.kind === "disk" && typeof activeStore.fileBytes === "function") {
    console.log(
      `[QC_LLM_CACHE] disk fileBytes=${activeStore.fileBytes()} path=${activeStore.filePath}`
    );
  }
}
