...
const FACTUAL_RELATION_PHRASES = [
  "founded in", "founded", "based in", "headquartered in", "headquarters",
  "launched", "launch", "announced", "announce", "acquired", "acquire",
  "raised", "release", "released", "introduced", "introduce",
];

/**
 * A6.31: Core proposition confirmed when entity + relation/action appear in both statement and excerpt.
 ...
 */
export function corePropositionConfirmed(statementText, excerpt) {
  ...
  const entityMatch = stmt.match(/\b([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b/);
  ...
  const relationInStmt = FACTUAL_RELATION_PHRASES.filter((phrase) => stmtLower.includes(phrase));
  const relationInExcerpt = FACTUAL_RELATION_PHRASES.filter((phrase) => exLower.includes(phrase));
  const overlap = relationInStmt.some((r) => relationInExcerpt.some((e) => r.includes(e) || e.includes(r)));
