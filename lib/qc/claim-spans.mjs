 ...
 * claim span. Case-insensitive substring match anywhere in the sentence.
 */
...
export function isCompoundCandidate(sentenceText) {
  ...
  if (RELATIONAL_CONNECTIVES.some((c) => lower.includes(c.toLowerCase()))) return false;
  return true;
}
...
export function validateClaimSpans(parentSentence, claims) {
  ...
  if (raw.length < 2) return { ok: false, reason: "fewer_than_two_claims" };
  if (raw.length > MAX_CLAIMS_PER_SENTENCE) return { ok: false, reason: "over_claim_cap" };

  const located = [];
  let minStart = 0;
  for (let i = 0; i < raw.length; i += 1) {
    const modelText = asText(raw[i]);
    if (!modelText.trim()) return { ok: false, reason: "empty_claim" };
    ...
    if (!span) {
      ...
      if (anywhere && anywhere.start < minStart) {
        return { ok: false, reason: anywhere.start < (located[located.length - 1]?.localStart ?? 0) ? "out_of_order" : "overlap" };
      }
      return { ok: false, reason: "not_contiguous_substring" };
    }
    ...
    if (extractVerifiableAnchors(slice).length < 1) {
      return { ok: false, reason: "anchorless_claim" };
    }
    located.push({
      text: slice,
