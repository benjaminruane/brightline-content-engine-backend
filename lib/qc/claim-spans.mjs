...
import { extractEvaluativeAssertionSpans } from "./evaluative-language.mjs";
...
/**
 * Residual coverage guard (rollup condition c).
 *
 * SAFETY INVARIANT: this function can only WITHHOLD an upgrade. Evaluative
 * residual anchors are not used for claim extraction, Stage 2, Stage 3
 * aggregation, or any downgrade path. No card can become more confident
 * than it is today. Worst case is more partially_confirmed cards.
 *
 * @param {string} parentSentence
 * @param {Array<object>} claims
 * @param {{ includeEvaluative?: boolean }} [options]
 */
export function residualHasUnclaimedAnchor(parentSentence, claims, options = {}) {
  const parent = asText(parentSentence);
  const includeEvaluative = options.includeEvaluative !== false;
  ...
  const evaluative = includeEvaluative ? extractEvaluativeAssertionSpans(residual) : [];
  const merged = [];
  for (const span of [...anchors, ...evaluative]) {
    if (merged.some((k) => k.start < span.end && span.start < k.end && k.kind === span.kind)) continue;
    merged.push(span);
  }
