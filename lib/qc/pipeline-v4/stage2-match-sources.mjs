...
function moneyMetricWindow(text, index, matchLen, len = 48) {
  const t = typeof text === "string" ? text : "";
  const start = Math.max(0, index - len);
  const end = Math.min(t.length, index + matchLen + len);
  return t.slice(start, end).toLowerCase();
}

function resolveMoneyMetric(window) {
  const w = typeof window === "string" ? window : "";
  for (const [phrase, id] of MONEY_METRIC_PHRASES_LONGEST_FIRST) {
    const escaped = phrase.replace(/[.*+?^${}()|[\]\\]/g, "\\$&").replace(/ /g, "\\s+");
    if (new RegExp(`\\b${escaped}\\b`, "i").test(w)) return id;
  }
  return undefined;
}
...
function extractMoney(text) {
  ...
  while ((m = re.exec(t))) {
    ...
    const metric = resolveMoneyMetric(moneyMetricWindow(t, m.index, m[0].length));
...
function phraseAroundRaw(text, raw) {
  ...
  const end = Math.min(t.length, idx + needle.length + 48);
