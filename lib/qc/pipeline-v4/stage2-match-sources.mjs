...
const MONEY_METRIC_PHRASES_LONGEST_FIRST = MONEY_METRIC_PHRASES.slice().sort((a, b) => {
  const d = b[0].length - a[0].length;
...
/**
 * Sentence containing a money figure, using the Stage 1 fallback splitter.
 * If no boundary is found, the whole passage (Stage 2 cap 400 chars).
 *
 * Metric scope is the whole sentence. Truncation is impossible by
 * construction: do not reintroduce a character window here. A window can
 * assign a wrong id from a truncated phrase (e.g. "revenue" from "annual
 * recurring revenue"). Percent still uses its own window; do not share this.
 */
function sentenceContainingFigure(text, figIndex) {
  const t = typeof text === "string" ? text : "";
...
function moneyMetricHits(sentence) {
  const s = typeof sentence === "string" ? sentence : "";
  const hits = [];
  for (const [phrase, id] of MONEY_METRIC_PHRASES_LONGEST_FIRST) {
...
function resolveMoneyMetric(sentence, figLocalStart, figLen) {
  const hits = moneyMetricHits(sentence);
...
function extractMoney(text) {
  ...
  while ((m = re.exec(t))) {
    ...
    const metric = resolveMoneyMetric(scope.text, m.index - scope.start, m[0].length);
...
function moneyMetricId(fig) {
  const m = typeof fig?.metric === "string" ? fig.metric.trim() : "";
  return m || "";
}

/** True only when both sides resolved and the ids differ. Fail closed otherwise. */
function moneyMetricsDiffer(stmtFig, srcFig) {
  const a = moneyMetricId(stmtFig);
  const b = moneyMetricId(srcFig);
...
function logMoneyForceSuppressed(statement, passage, stmtFig, srcFig) {
  const stmtPhrase = phraseAroundRaw(statement, stmtFig?.raw);
  const srcPhrase = phraseAroundRaw(passage, srcFig?.raw);
  console.log(
    `[magnitude-backstop] suppressed money force stmt="${stmtPhrase}" metric=${moneyMetricId(stmtFig)} src="${srcPhrase}" metric=${moneyMetricId(srcFig)}`
  );
}
 ...
 * @returns {Array<{ value: number, raw: string, kind: "percent"|"money"|"count", keys?: string[], metric?: string }>}
 */
...
export function hasEgregiousMagnitudeGap(statement, passage) {
  ...
  for (const [stmtFigs, srcFigs, kind] of groups) {
    ...
    for (const s of stmtFigs) {
      ...
      // Money: suppress only when BOTH sides resolved to a recognised metric
      // id and those ids differ. Unrecognised metrics fail closed (force).
      if (kind === "money" && moneyMetricsDiffer(s, best)) {
        logMoneyForceSuppressed(statement, passage, s, best);
        continue;
      }
      return true;
    }
