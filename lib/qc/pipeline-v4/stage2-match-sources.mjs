...
const STAGE2_SPAN_PROMPT_PATH = path.join(__dirname, "prompts", "stage2_v4_unsupported_span.md");

let stage2SystemPromptCache = null;
let stage2SpanPromptCache = null;
...
/**
 * Exact substring check. No whitespace normalisation, no case-fold, no trim.
 * Absent / empty / non-string is null and is not a rejection.
 * A non-empty string that is not an exact substring is a rejection.
 *
 * @param {unknown} raw
 * @param {string} statementText
 * @returns {{ span: string|null, returned: boolean, rejected: boolean }}
 */
export function validateUnsupportedSpan(raw, statementText) {
  if (typeof raw !== "string" || raw.length === 0) {
    return { span: null, returned: false, rejected: false };
  }
  const statement = typeof statementText === "string" ? statementText : "";
  if (!statement.includes(raw)) {
    return { span: null, returned: true, rejected: true };
  }
  return { span: raw, returned: true, rejected: false };
}
...
export function attachUnsupportedSpanFields(parsed, statementText, spanEnabled) {
  if (!spanEnabled) return {};
  const result = validateUnsupportedSpan(parsed?.unsupportedSpan, statementText);
  if (result.rejected) {
    unsupportedSpanRejectionCount += 1;
    const preview = typeof parsed?.unsupportedSpan === "string" ? parsed.unsupportedSpan.slice(0, 80) : "";
    console.debug(`[stage2] unsupportedSpan rejected preview=${JSON.stringify(preview)}`);
  }
  return {
    unsupportedSpan: result.span,
    unsupportedSpanRaw: result.returned ? parsed.unsupportedSpan : null,
    unsupportedSpanRejected: result.rejected === true,
  };
}

async function getStage2SystemPrompt(spanEnabled = false) {
  ...
  if (!spanEnabled) return stage2SystemPromptCache;
  if (!(typeof stage2SpanPromptCache === "string" && stage2SpanPromptCache.trim())) {
    const extra = await readFile(STAGE2_SPAN_PROMPT_PATH, "utf8");
    stage2SpanPromptCache = extra.trim();
  }
  return `${stage2SystemPromptCache}\n\n${stage2SpanPromptCache}`;
}

export async function getStage2SystemPromptForTest(spanEnabled = false) {
  return getStage2SystemPrompt(spanEnabled);
}

export function resetStage2PromptCache() {
  stage2SystemPromptCache = null;
  stage2SpanPromptCache = null;
}
...
async function stage2CacheParts(statementText, sourceText, parentSentence, spanEnabled = false) {
  const stageModel = STAGE_MODELS["stage2-matching"];
  const prompt = await getStage2SystemPrompt(spanEnabled);
  return {
    stage: "stage2",
...
async function matchSingleSource(statement, sourceText, options = {}) {
  const stageModel = STAGE_MODELS["stage2-matching"];
  const spanEnabled = options.stage2SpanEnabled === true;
  const stage2SystemPrompt = await getStage2SystemPrompt(spanEnabled);
...
async function matchOnePair({
  ...
  const spanEnabled = stage2SpanEnabled === true;
  const stageModel = STAGE_MODELS["stage2-matching"];
  const baseMetadata = {
  ...
  async function matchOnePairLive() {
    try {
      ...
      let parsedForSpan = first.parsed;
      ...
      if (!normalized) {
        ...
        return {
          ...
          costUsd,
          systemFingerprint,
          ...attachUnsupportedSpanFields(parsedForSpan, statementText, spanEnabled),
        };
      }
      ...
      return {
        ...
        schemaValid: false,
        retried: false,
        ...attachUnsupportedSpanFields(null, statementText, spanEnabled),
        latencyMs: 0,
        usage: { ...emptyUsage },
        costUsd: 0,
        systemFingerprint: null,
  ...
  const parts = await stage2CacheParts(statementText, sourceText, parentSentence, spanEnabled);
