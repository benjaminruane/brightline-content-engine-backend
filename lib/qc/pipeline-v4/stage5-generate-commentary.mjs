...
const STAGE5_PROMPT_PATH = path.join(__dirname, "prompts", "stage5_v1.md");
...
function buildFallbackCommentary(verdict) {
  if (verdict === "confirmed") {
    return "Verdict: confirmed. Specific commentary is unavailable from the system; please review the excerpt directly before finalizing.";
  }
  if (verdict === "partially_confirmed") {
    return "Verdict: partially confirmed. Specific commentary is unavailable from the system; please review the excerpt and adjust the statement to match the source language.";
  }
...
function buildUserPrompt({ statement, verdict, hasConflict, primaryExcerpt, conflictExcerpt }) {
  const blocks = [
