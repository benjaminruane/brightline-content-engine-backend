...
const STANDIN_FRAGMENT = "Specific commentary is unavailable from the system";

function notReviewedResult(extra = {}) {
  return {
    commentary: "",
    schemaValid: false,
    notReviewed: true,
...
export async function generateCommentary({
  ...
  try {
    ...
    if (!normalized) {
      ...
      console.warn(
        `stage5: commentary schema failed for statement ${
          Number.isFinite(statementIndex) ? statementIndex : "unknown"
        }`
      );
      return notReviewedResult({ retried, latencyMs, usage, costUsd });
    }

    if (normalized.commentary.includes(STANDIN_FRAGMENT)) {
      return notReviewedResult({ retried, latencyMs, usage, costUsd });
    }
    ...
    if (reason) incrementBoundHit("commentary");
    console.warn(
      `stage5: commentary failed for statement ${
        Number.isFinite(statementIndex) ? statementIndex : "unknown"
      }: ${err?.message || String(err)}`
    );
    return notReviewedResult({ retried, latencyMs, usage, costUsd, notReviewedReason: reason });
  }
}
