...
export const STAGE5_CONCURRENCY = 24;
export const STAGE6_CONCURRENCY = 24;
...
function notReviewedEditorialResult(toggles) {
  return {
...
async function runEvidenceSkippedPath(stage1Result, safeDraft, safeSources, options, toggles, traceId) {
  ...
  if (toggles.editorialEnabled || toggles.complianceEnabled) {
    await mapPool(reviewStatements, STAGE6_CONCURRENCY, async (reviewStatement, index) => {
        try {
          ...
          if (card && typeof card === "object") {
            Object.assign(card, notReviewedEditorialResult(toggles));
          }
        }
      }
    );
...
async function runPipelineV4Inner(draft, sources, options = {}) {
  ...
  if (frozenIn) {
    ...
    stage1Result = { statements, source: "frozen", errors: [] };
  } else {
  ...
  const stage2WithEditorial = await mapPool(
    ...
    async (entry, index) => {
      try {
        ...
        return {
          ...
          editorialResult: notReviewedEditorialResult(toggles),
        };
      }
    }
  );
