...
Passage rule:
Each passage must be a contiguous verbatim excerpt from the source. Do not abridge, summarise, or stitch together multiple non-adjacent quotes using ellipsis or '[...]' markers within a single passage. Return ALL contiguous spans in the source that bear on the statement — do not pick only the single most relevant span. Each distinct supporting passage MUST be a separate `matches` array entry. Do NOT merge separate sentences or non-adjacent passages into one span, even when they jointly support the statement. Do NOT join adjacent sentences into one span when each sentence independently bears on a different verifiable fact in the statement — return one entry per fact-bearing sentence. Every `passage` value must itself be an individually-contiguous verbatim substring of the source (so an exact substring locate succeeds). If one span is longer than needed, prefer a shorter focused excerpt for that span. Better to return multiple focused excerpts than one merged or abridged composite.
...
`matches` is an array of 0..N items. Include every contiguous verbatim span that bears on the statement, each with its own classification, as its own array item. Never combine two distinct bearing passages into a single item. If the source does not address the statement at all, return an empty array (or a single item with classification `no_support` and an empty passage).
...
For each match, return a verbatim excerpt from the source. Maximum 400 characters per passage.
If a passage is longer, trim at a sentence boundary and do not paraphrase.
