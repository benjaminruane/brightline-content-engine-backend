...
Passage rule:
Each passage must be a contiguous verbatim excerpt from the source. Do not abridge, summarise, or stitch together multiple non-adjacent quotes using ellipsis or '[...]' markers within a single passage. Return ALL contiguous spans in the source that bear on the statement — do not pick only the single most relevant span. If one span is longer than needed, prefer a shorter focused excerpt for that span. Better to return multiple focused excerpts than one abridged composite.
...
`matches` is an array of 0..N items. Include every contiguous verbatim span that bears on the statement, each with its own classification. If the source does not address the statement at all, return an empty array (or a single item with classification `no_support` and an empty passage).
