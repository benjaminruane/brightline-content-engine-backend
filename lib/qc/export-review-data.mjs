...
import { CHECK_NOT_CHECKED_LABEL, classifyCard, turnedOffLineFromClass } from "./review-summary.mjs";
...
export function buildReviewData(qcResult, reviewOptions) {
  ...
  const normalizedStatements = statements.map((s) => {
    ...
    const displaySlug = String(qcCard.displayVerdict || "").toLowerCase().trim();
    const evidenceOff = qcCard.supportState === "skipped";
    const honestyNotReviewed = !evidenceOff && displaySlug === "not reviewed";
    const evidenceSkipped = evidenceOff || honestyNotReviewed;
    const verdict = evidenceOff
      ? null
      : honestyNotReviewed
        ? CHECK_NOT_CHECKED_LABEL
        : normalizeExportVerdict(qcCard.displayVerdict);
    const concernLevel =
