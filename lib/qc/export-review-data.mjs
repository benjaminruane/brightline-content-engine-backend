...
export function buildReviewData(qcResult, reviewOptions) {
  ...
  const normalizedStatements = statements.map((s) => {
    ...
    const opts = reviewOptions && typeof reviewOptions === "object" ? reviewOptions : {};
    const editorialEnabled = opts.editorialEnabled !== false;
    const complianceEnabled = opts.complianceEnabled !== false;
...
export function renderCanonicalExportText({
  ...
  for (const row of review.statements) {
    ...
    const concernSuffix =
      row.concernLevel && String(row.concernLevel).toLowerCase() !== "none"
        ? ` (${row.concernLevel} concern)`
        : "";
