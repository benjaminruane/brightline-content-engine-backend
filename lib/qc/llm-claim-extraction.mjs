 ...
 */
async function extractSubclaimsForSentence(provider, model, timeoutMs, sentence) {
  ...
  const completion = await Promise.race([
    callLLM({
      ...
      traceName: "qc-claim-extraction",
      spanName: "qc-claim-extraction-sentence",
      metadata: { module: "llm-claim-extraction" },
    }),
...
export async function extractClaimsFromDraftLLM(draftText, opts = {}) {
  ...
  const modelConfig = STAGE_MODELS["claim-extraction"] ?? { provider: "openai", model: DEFAULT_MODEL };
  const providerAvailable = hasProviderApiKey(modelConfig.provider);
  emit("LLM_CLAIM_EXTRACTION_PROVIDER", { provider: modelConfig.provider, providerAvailable });

  if (!providerAvailable) {
    emit("LLM_CLAIM_EXTRACTION_DONE", { fallback_mode: true, fallback_reason: "missing_provider_api_key", acceptedClaimsCount: 0, reason: "missing_provider_api_key" });
    return { fallback_mode: true, error: "missing_provider_api_key" };
  }

  const model = (typeof process !== "undefined" && process.env?.QC_LLM_CLAIM_EXTRACTION_MODEL)?.trim() || modelConfig.model || DEFAULT_MODEL;
  ...
  const settleds = await Promise.allSettled(
    sentences.map((sent) => extractSubclaimsForSentence(modelConfig.provider, model, timeoutMs, sent.text))
  );
