...
import { callOpenAI } from "../observability.js";
 ...
 */
async function extractSubclaimsForSentence(model, timeoutMs, sentence) {
...
export async function extractClaimsFromDraftLLM(draftText, opts = {}) {
  ...
  const settleds = await Promise.allSettled(
    sentences.map((sent) => extractSubclaimsForSentence(model, timeoutMs, sent.text))
  );
