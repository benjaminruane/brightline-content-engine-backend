...
'The phrase "will deliver guaranteed returns of 15% IRR" uses promissory language that guarantees investment outcomes, which is not permissible. Replace with "aims to achieve a target of 15% IRR" or similar hedging language.'`;
...
Example for two thousands_separator violations:
'The statement uses a comma as a thousands separator instead of a straight apostrophe. Replace "5,500" with "5'500" and "10,000" with "10'000".'`;
...
Public visibility: Wider, non-NDA audience. Apply rules at stricter threshold. Borderline cases that would be allowed on Complete should be flagged on Public. Specifically:
  - Forward-looking claims without explicit qualifier ('expect', 'aim', 'believe', 'currently anticipate') should be flagged
  - Comparative claims ('outperform', 'better than') without stated basis should be flagged
...
Good:
  "The statement 'will continue to outperform peer benchmarks' is a forward-looking claim without an uncertainty qualifier."
...
The visibility setting (Complete or Public) is provided in the user payload. On Public visibility, apply tighter tolerance to:
  - Promotional language and intensifiers ('truly', 'incredibly', 'remarkably', 'stunning')
  - Subjective qualifiers ('strong', 'compelling', 'exceptional') used without specific substantiation
  - Marketing register more broadly — Public-facing copy should read as professional and institutional, not promotional

On Complete visibility, the same patterns may be flagged at standard threshold; treat as borderline unless the language is clearly excessive.`;
...
export function recomputeV4EditorialVerdictFromConcerns(editorialConcerns, outputType) {
  ...
  const styleMap = buildRuleMap(styleGuideRules);
...
function normalizeCombinedConcern(entry, allowedStyleIds, allowedEditorialIds) {
  ...
  if (category !== "editorial" && category !== "style_guide") return null;
  const ruleId = typeof entry?.ruleId === "string" ? entry.ruleId.trim() : "";
  if (!ruleId) return null;
  if (category === "editorial" && !allowedEditorialIds.has(ruleId)) return null;
  if (category === "style_guide" && !allowedStyleIds.has(ruleId)) return null;
...
/**
 * R5.1: Derive character span for a concern from quoted phrases in note / suggestedDirection.
 * Known limitation: multiple occurrences → earliest match only.
 * @param {{ concern: object, statementText: string }} args
 * @returns {{ startChar: number, endChar: number, source: string } | null}
 */
export function deriveConcernSpan({ concern, statementText }) {
  try {
    ...
    for (const { quote } of extractQuotedSnippets(note, "note")) {
      ...
      if (match) return { ...match, source: "note_quote" };
    }
    for (const { quote } of extractQuotedSnippets(direction, "suggestedDirection")) {
      const match = findPhraseSpanInStatement(quote, statementText);
      if (match) return { ...match, source: "direction_quote" };
    }
    return null;
  } catch {
    return null;
...
function buildEditorialStyleSystemPrompt({ outputTypeLabel, editorialRules, styleRules, outputSlug, outputType }) {
  const conventions = buildOutputTypeConventionsBlock(outputType);
...
## Editorial rules
${formatRulesForPrompt(editorialRules, outputSlug, "editorial")}

## Style rules
${formatRulesForPrompt(styleRules, outputSlug, "style")}
...
export async function runEditorialStyleReview(args) {
  ...
  const sorted = sortConcernsBySeverity(withSpans, mergedMap, outputSlug);
  ...
  if (useCombinedEditorialStyle && traceId) {
    ...
    for (const stmt of toProcess) {
      ...
      for (const c of editorialConcerns) {
        ...
        if (
          c?.span &&
          Number.isFinite(c.span.startChar) &&
          Number.isFinite(c.span.endChar)
        ) {
          editorialWithSpan += 1;
        }
      }
    ...
    if (editorialTotal > 0) {
      logCanaryScore({
        ...
        metadata: { totalConcerns: editorialTotal, concernsWithSpan: editorialWithSpan },
      });
    }
    if (complianceTotal > 0) {
      logCanaryScore({
        ...
        metadata: { totalConcerns: complianceTotal, concernsWithSpan: complianceWithSpan },
      });
    }
  }
}
