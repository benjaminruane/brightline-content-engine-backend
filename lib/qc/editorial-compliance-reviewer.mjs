...
// A7.14 / A8.22–A8.28: Rulebook-driven style, editorial, and compliance LLM review per statement.
...
wording, not soften it. Omit it.

Pre-check 1 — Approximator word.
...
Pre-check 2 — Precise figure in evidence.
Identify the specific figure in the EVIDENCE EXCERPT that the
approximator in pre-check 1 refers to. The evidence figure
must be precise — stated without an approximator, or with a
tighter approximator. If the evidence figure uses the same
approximator word as the statement, or an equivalent
(approximately ≈ roughly ≈ around ≈ about), the rule does
not apply. Omit.

Pre-check 3 — Same underlying fact.
...
Pre-check 4 — Not a numeric conflict.
...
If all four pre-checks pass, emit the concern with a
suggestedDirection that follows the fixDirection format: a
single imperative sentence quoting the vague phrase and the
precise figure.`;
