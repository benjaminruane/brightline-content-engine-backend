...
const STYLE_RULE_DETERMINISTIC_FILTERS = {
  ...
  defined_term_capitalisation: (citedSpan, _statementText) => {
    const CAPITALISED_DEFINED_TERM =
      /^The\s+(Company|Fund|Investment|Sponsor|Group|Partnership|Manager|Adviser|Issuer|Borrower)\b/;
    if (CAPITALISED_DEFINED_TERM.test(citedSpan.trim())) {
      console.log(
        `[style_guide] defined_term_capitalisation concern dropped by deterministic filter ` +
          `(cited span "${citedSpan.trim()}" starts with capitalised defined term — already correct)`
      );
      return false;
    }
    return true;
...
function applyDeterministicStyleFilters(concerns, statementText) {
  if (!Array.isArray(concerns)) return concerns;
  return concerns.filter((c) => {
    if (!c || typeof c.rule !== "string") return true;
    const filter = STYLE_RULE_DETERMINISTIC_FILTERS[c.rule];
    ...
    return filter(cited, statementText);
  });
}
...
export async function runEditorialStyleReview(args) {
  ...
  const idx = statementIndex ?? null;
  const structuredStyleRules = resolveStyleGuide({ outputType });
  ...
  const styleFiltered = applyDeterministicStyleFilters(withSpans, sentenceText);
  const sorted = sortConcernsBySeverity(styleFiltered, mergedMap, outputSlug);
