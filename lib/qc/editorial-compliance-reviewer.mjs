...
import { incrementBoundHit, isRateLimitBoundError } from "./request-budget.mjs";
...
/** B202: the check was requested and did not complete. Never a silent clean. */
function markEditorialNotReviewed(qcCard, reason) {
  if (!qcCard || typeof qcCard !== "object") return;
  ...
  if (typeof reason === "string" && reason.trim()) {
    qcCard.editorialNotReviewedReason = reason.trim();
  }
}

/** B202: the check was requested and did not complete. Never a silent clean. */
function markComplianceNotReviewed(qcCard, reason) {
  if (!qcCard || typeof qcCard !== "object") return;
  ...
  qcCard.complianceSuggestedRewrite = null;
  if (typeof reason === "string" && reason.trim()) {
    qcCard.complianceNotReviewedReason = reason.trim();
  }
}

function stampNotReviewedFromFailure(qcCard, err, kind) {
  const reason = isRateLimitBoundError(err) ? "rate_limit_window" : undefined;
  if (reason) incrementBoundHit(kind === "compliance" ? "compliance" : "editorial");
  if (kind === "compliance") markComplianceNotReviewed(qcCard, reason);
  else markEditorialNotReviewed(qcCard, reason);
}
    ...
    toProcess.map(async (stmt, index) => {
      ...
      if (useCombinedEditorialStyle && editorialEnabled && complianceEnabled) {
        ...
        if (editorialStyleSettled.status !== "fulfilled") {
          ...
          const reason = isRateLimitBoundError(editorialStyleSettled.reason)
            ? "rate_limit_window"
            : undefined;
          if (reason) incrementBoundHit("editorial");
          markEditorialNotReviewed(stmt.qcCard, reason);
        } else {
          ...
          stmt.qcCard.editorialVerdict =
            typeof result?.editorialVerdict === "string" ? result.editorialVerdict : "not_reviewed";
        ...
        if (complianceSettled.status !== "fulfilled") {
          ...
          stampNotReviewedFromFailure(stmt.qcCard, complianceSettled.reason, "compliance");
        } else {
          ...
          if (complianceRaw == null) {
            ...
            markComplianceNotReviewed(stmt.qcCard);
          } else {
      ...
      if (useCombinedEditorialStyle && editorialEnabled) {
        ...
        if (editorialStyleSettled.status !== "fulfilled") {
          stampNotReviewedFromFailure(stmt.qcCard, editorialStyleSettled.reason, "editorial");
        } else {
          const result = editorialStyleSettled.value;
          stmt.qcCard.editorialConcerns = Array.isArray(result?.editorialConcerns) ? result.editorialConcerns : [];
          stmt.qcCard.editorialVerdict =
      ...
      if (editorialEnabled && complianceEnabled) {
        ...
        if (styleSettled.status !== "fulfilled" || editorialSettled.status !== "fulfilled") {
          ...
          if (styleRaw == null || edRaw == null) {
            ...
            markEditorialNotReviewed(stmt.qcCard);
          } else {
