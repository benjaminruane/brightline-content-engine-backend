...
const HOUSE_STYLE_IS_THE_STANDARD = `The style rules above are the only formatting standard. Source documents are not written to them and their formatting carries no authority: a difference between the CURRENT STATEMENT and a source in thousands separators, currency symbols, dashes, quotation marks, numerals, spelling variant or date order is never a reason to raise a concern. If the CURRENT STATEMENT already matches a rule's Correct example, that rule is satisfied: omit it. Never propose changing text from a form the rules call Correct to a form they call Incorrect. Never emit a suggestedDirection whose replacement text is identical to the text it replaces.`;
...
EVIDENCE EXCERPT (from the source supporting this statement. Factual content only. Its formatting is not a standard):
${excerptBlock}
...
SOURCE EVIDENCE (factual context only — do not assess this
text for style, formatting, or language quality, and do not restyle the CURRENT STATEMENT to match it):
${evidenceBlock}`;
}
