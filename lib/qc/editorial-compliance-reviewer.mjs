...
import { buildSourcePublicationStateBlock, normalizePublicationState } from "../source-publication-state.mjs";
...
const STYLE_RULE_DETERMINISTIC_FILTERS = {
  ...
  number_spelling: (citedSpan, _statementText) => {
    ...
    if (PERCENT_TOKEN.test(String(citedSpan || "").trim())) {
      console.log(
  ...
  percentage_notation: (citedSpan, statementText) => {
    ...
    if (/\d[\d.,]*\s*%/.test(trimmed) && !/\bpercent\b/i.test(trimmed)) {
      console.log(
    ...
    if (/\d[\d.,]*\s*%/.test(fullText) && !/\d[\d.,]*\s+percent\b/i.test(fullText)) {
      console.log(
        `[style_guide] percentage_notation concern dropped by deterministic filter ` +
...
function resolveSourceName(source, index) {
  return (
...
function buildComplianceUserPayload(sentenceText, ctx) {
  ...
  for (let i = 0; i < sources.length; i++) {
    const publicationState = normalizePublicationState(sources[i]?.publicationState);
    const sourceName = resolveSourceName(sources[i], i);
    console.log(`[R6.6-DIAG] source="${sourceName}" publicationState="${publicationState}"`);
  }
...
export async function runComplianceReview(args) {
  ...
  const suppressionBlockAnchor = "SOURCE PUBLICATION STATE AWARENESS";
  const assembledPrompt = `${systemPrompt}\n${userContent}`;
  const suppressionBlockPresent = assembledPrompt.includes(suppressionBlockAnchor);
  console.log(`[R6.6-DIAG] suppression_block_present=${suppressionBlockPresent}`);
...
export async function runEditorialComplianceReview(statements, documentContext = {}) {
  ...
  const ctx = { outputType, requiredVersion, eventType, outputTypeLabel, outputSlug, sources: complianceSources };
