...
Does the sentence say what the evidence actually shows, or has
something been added, lost, or distorted in translation? Flag
any gap between what is written and what the evidence
demonstrates.
...
- A clean verdict requires genuine confidence. Do not default
  ...
  writer judge.
- House style violations always cap at soft_concern. Reserve
  hard_concern for issues that materially affect accuracy, integrity,
  or fitness for purpose — not formatting preferences or conventions.
...
- Asset-level confidentiality requirements are not assessed
  ...
  publishing.
- Be specific. Go straight to the substance. Each note must name the
  specific words or phrase in the sentence under review that create
  the risk. A note that does not reference specific language from the
  sentence has failed the specificity test. Never use generic phrases
  such as 'a reasonable reader' or 'the audience'.
- Surface all concerns found, ordered by severity — most
  significant first.
...
/**
 * A7.14: Run editorial + compliance LLM review per statement (sequential statements; concurrent calls per statement).
 *
...
export async function runEditorialComplianceReview(statements, documentContext = {}) {
  ...
  for (const stmt of toProcess) {
    ...
    if (!stmt.qcCard || typeof stmt.qcCard !== "object") continue;
