...
// A7.14 / A8.22 / A8.23 / A8.24: Rulebook-driven style, editorial, and compliance LLM review per statement.
...
- When a rule specifies a fixDirection, use it as guidance for what to include in your suggestedDirection output. Always substitute the actual phrase, figure, or word from the statement into your suggestedDirection — do not emit placeholders or generic descriptions.`;
...
const EDITORIAL_EVIDENCE_EXCERPT_GUIDANCE = `The EVIDENCE EXCERPT shows the source passage that supports the CURRENT STATEMENT, when available. Use it to judge rules that compare the statement against what the source actually says. Specifically: for imprecision_when_precision_available, fire the rule ONLY when the evidence excerpt contains a more precise figure than the statement. If the excerpt is equally imprecise, or no excerpt is available, do not fire the rule.`;
...
STATEMENT CONTEXT
CURRENT STATEMENT (under review): ${sentenceText}
...
${EDITORIAL_EVIDENCE_EXCERPT_GUIDANCE}
...
function buildStyleSystemPrompt(outputTypeLabel, rules, outputSlug) {
...
export async function runStyleGuideReview(client, args) {
  const { draftMarked, sentenceText, evidenceBlock, ctx, rules, statementIndex } = args;
  ...
  const systemPrompt = buildStyleSystemPrompt(ctx.outputTypeLabel, rules, outputSlug);
...
export async function runEditorialComplianceReview(statements, documentContext = {}) {
  ...
  await Promise.allSettled(
    toProcess.map(async (stmt, index) => {
      ...
      const styleP =
        editorialEnabled && runStyleGuideReview(client, { ...reviewArgs, rules: styleFiltered, statementIndex });
      const editorialP =
        editorialEnabled &&
      ...
      const complianceP =
        complianceEnabled && runComplianceReview(client, { ...reviewArgs, rules: complianceFiltered, statementIndex });
