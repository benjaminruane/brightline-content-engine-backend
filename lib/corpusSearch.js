...
/**
 * A3.8.96: Check if a number is in a percent context within statement text
 * @param {string} stmtText - The statement text
 * @param {number} n - The numeric value to check
 * @returns {boolean} True if the number appears in a percent context
 */
function isPercentContext(stmtText, n) {
...
export function corpusSearch(statementText, uploadedDocs, options = {}) {
  ...
  for (const doc of docsWithFullText) {
    ...
    if (statementNumericValues.length > 0) {
      ...
      // A3.8.78: Store as objects with docValue for compatibility
      if (matchesAccepted.length > 0) {
        const existingAccepted = [];
        const existingValues = new Set();
        // Preserve existing matchesAccepted objects
        if (Array.isArray(debug.matchesAccepted)) {
          for (const m of debug.matchesAccepted) {
            const val = typeof m === "object" && m !== null ? (m.docValue || m.value || m) : m;
            if (!existingValues.has(val)) {
              existingAccepted.push(m);
              existingValues.add(val);
            }
          }
        }
        // Add new matches
        for (const m of matchesAccepted) {
          const val = m.docValue;
          if (!existingValues.has(val)) {
            existingAccepted.push(m);
            existingValues.add(val);
          }
        }
        debug.matchesAccepted = existingAccepted;
      }
    }
    
    // A3.8.77: Numeric-only acceptance for selection mode with uploaded docs
    // A3.8.94: Only accept numbers that are present in the document numeric set
    ...
    if (selectionMode && statementNumericValues.length > 0 && docNumericValues.length > 0) {
      // A3.8.94: Build Set from docNums for fast lookup
      const docNumsSet = new Set(docNumericValues);
      ...
      for (const stmtValue of statementNumericValues) {
        ...
        const isPercent = isPercentContext(statementText, stmtValue);
        
        // A3.8.94: Only accept if stmtValue matches a number in docNumsSet
        ...
        for (const docValue of docNumericValues) {
          ...
          if (isPercent) {
            ...
            if (stmtValue === docValue) {
              shouldAccept = true;
            } else if (numericValuesMatch(stmtValue, docValue)) {
              // Would have matched with tolerance, but blocked for percent
              console.log(`[DIAG][A3.8.96][PCT_TOLERANCE_BLOCKED] stmtNum=${stmtValue} docNum=${docValue} docId=${docId}`);
              shouldAccept = false;
            }
          } else {
            // Non-percent: use tolerance matching
            shouldAccept = numericValuesMatch(stmtValue, docValue);
          }
          
          if (shouldAccept) {
            // A3.8.94: Verify docValue is in docNumsSet (should always be true, but be explicit)
            if (docNumsSet.has(docValue)) {
        ...
        if (!foundMatch && stmtValue != null) {
          // Check if stmtValue is close to any docValue but not exactly matching
          ...
          if (wouldHaveMatched) {
            console.log(`[DIAG][A3.8.94][CORPUS_REJECT_NOT_IN_DOC] stmtNum=${stmtValue} docId=${docId}`);
          }
        }
      }
  ...
  // A3.8.78: Numeric-only acceptance for selection mode with uploaded sources (bypass keyword gate)
  // A3.8.94: Only accept numbers that are present in the document numeric set
  // This runs AFTER all existing acceptance logic, only if matchesAccepted is empty
  const numsInStmt = debug.numsInStmt || [];
  const numsInDoc = debug.numsInDoc || [];
  
  // A3.8.78: Narrow guard - selection mode, uploaded sources, non-empty stmtNums
  // Since corpusSearch only receives uploadedDocs, all hits are from uploaded sources
  const selectionUploadedNumericOnly =
    selectionMode === true &&
    uploadedDocs.length > 0 && // All docs in corpusSearch are uploaded
    Array.isArray(numsInStmt) &&
    numsInStmt.length > 0;
  
  if (
    (debug.matchesAccepted?.length || 0) === 0 &&
    selectionUploadedNumericOnly &&
    numsInDoc.length > 0
  ) {
    // A3.8.94: Build Set from docNums for fast lookup
    const docNumsSet = new Set(numsInDoc);
    
    // A3.8.78: Compute numeric overlap using exact numeric equality
    // A3.8.94: Only accept if stmtNum matches a number in docNumsSet
    // A3.8.96: For percent context, require exact match (no tolerance)
    const overlap = [];
    let rejectionLogged = false; // A3.8.94: Log at most once per statement
    for (const stmtNum of numsInStmt) {
      // A3.8.96: Check if this is a percent context
      const isPercent = isPercentContext(statementText, stmtNum);
      
      let foundMatch = false;
      for (const docNum of numsInDoc) {
        let shouldAccept = false;
        ...
        if (isPercent) {
          ...
          if (stmtNum === docNum) {
            shouldAccept = true;
          } else if (numericValuesMatch(stmtNum, docNum)) {
            // Would have matched with tolerance, but blocked for percent
            if (!rejectionLogged) {
              console.log(`[DIAG][A3.8.96][PCT_TOLERANCE_BLOCKED] stmtNum=${stmtNum} docNum=${docNum}`);
      ...
      if (!foundMatch && stmtNum != null && !rejectionLogged) {
        ...
        let wouldHaveMatched = false;
        for (const docNum of numsInDoc) {
          if (numericValuesMatch(stmtNum, docNum)) {
            wouldHaveMatched = true;
            break;
          }
        }
        if (wouldHaveMatched) {
          const docId = docsWithFullText[0]?.id || "unknown";
          console.log(`[DIAG][A3.8.94][CORPUS_REJECT_NOT_IN_DOC] stmtNum=${stmtNum} docId=${docId}`);
          rejectionLogged = true;
        }
      }
    }
    
    // A3.8.78: Filter overlap - if contains both < 1000 and >= 1000, keep only >= 1000
    const hasSmall = overlap.some(n => n < 1000);
    const hasLarge = overlap.some(n => n >= 1000);
    let filteredOverlap = overlap;
    if (hasSmall && hasLarge) {
      filteredOverlap = overlap.filter(n => n >= 1000);
    }
    
    // A3.8.78: Deduplicate and cap to 10
    const uniqueOverlap = Array.from(new Set(filteredOverlap)).slice(0, 10);
    
    if (uniqueOverlap.length > 0) {
      // A3.8.78: Format matchesAccepted - use docValue for compatibility with evidenceMatch
      // Also include value, source, matchType as specified
      debug.matchesAccepted = uniqueOverlap.map(n => ({
        stmtValue: n,
        docValue: n, // A3.8.78: Required for evidenceMatch compatibility
        value: n,    // A3.8.78: As specified in spec
        source: "uploaded",
        matchType: "numeric_only_selection"
      }));
      
      debug.numericAcceptanceMode = "numeric_only_uploaded_selection";
      debug.numericOverlap = uniqueOverlap;
      
      // A3.8.78: Diagnostic
      const keywordsMatchedCount = debug.keywordsMatched?.length || 0;
      const docNumsCount = numsInDoc.length;
      const overlapSample = uniqueOverlap.slice(0, 8).join(",");
      console.log(`[DIAG][A3.8.78][NUMERIC_ONLY_ACCEPT] stmtNums=[${numsInStmt.join(",")}] docNumsCount=${docNumsCount} overlap=[${overlapSample}] keywordsMatchedCount=${keywordsMatchedCount}`);
    }
  }
  ...
  // A3.8.78: Ensure acceptedStr shows docValue from matchesAccepted objects
  if (debug.numsInStmt && debug.numsInStmt.length > 0) {
    ...
    // A3.8.78: Extract docValue from matchesAccepted objects (or use number if it's just a number)
    const acceptedValues = (debug.matchesAccepted || []).map(m => 
      typeof m === "object" && m !== null ? (m.docValue || m.value || m) : m
    );
    const acceptedStr = acceptedValues.join(",");
    const rejectedCount = (statementNumericValues.length || 0) - acceptedValues.length;
