...
export function corpusSearch(statementText, uploadedDocs) {
  ...
  for (const doc of docsWithFullText) {
    ...
    if (statementNumericValues.length > 0) {
      ...
      if (docNumericValues.length > 0) {
        const existingNums = new Set(debug.numsInDoc || []);
        docNumericValues.forEach(v => existingNums.add(v));
        debug.numsInDoc = Array.from(existingNums);
      }
