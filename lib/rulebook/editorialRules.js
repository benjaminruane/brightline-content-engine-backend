...
export default [
  ...
  },
  {
    ...
    severity: "soft_concern",
    description:
      "Vague qualifiers (nearly, roughly, approximately, around, about, some) are flagged ONLY when all of the following hold: (a) the statement contains an approximation qualifier on a specific figure; (b) the evidence excerpt contains the SAME underlying figure stated precisely, LITERALLY in the excerpt — not computed, derived, or inferred from other figures in the excerpt; (c) the evidence figure uses no approximator, or a distinctly tighter one. The words 'approximately', 'roughly', 'around', and 'about' are all equivalent vagueness and are treated as the same qualifier. E2 does not fire when statement and evidence both use any of these words, even if the exact word differs. If the statement and evidence contain different figures for the same fact, this is a conflict handled by the evidence pipeline — E2 does not fire. If the precise figure is not literally stated in the excerpt, E2 does not fire. If the evidence does not mention the figure at all, E2 does not fire. This rule exists to flag unnecessary hedging — a writer softening a number the source stated precisely. It is not a hard rule; the writer may have hedged intentionally, so output a suggestion rather than a command.",
    appliesTo: [...THREE],
    appliesToVersion: null,
    fixDirection:
      "Write a single suggestion sentence that quotes the exact vague phrase from the CURRENT STATEMENT and names the precise figure from the EVIDENCE EXCERPT as an alternative. Use suggestion wording, not a command. Example: 'Consider using \"9,842\" instead of \"nearly 10,000\" — the source gives this precise figure.' or 'The source states \"43\" — consider using it in place of \"around 40\".'",
  },
  {
