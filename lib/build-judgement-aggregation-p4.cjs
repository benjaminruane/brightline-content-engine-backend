...
// P4.W1 + P4.W2: Judgement Aggregation + Weighted Scoring & Stability Smoothing (Meta Only).
...
function statementJudgement(stmt, previousPosture) {
  try {
    ...
    return { fragilityPosture, primaryDrivers, confidence };
  } catch (_) {
    return null;
  }
}
...
function documentJudgement(doc, previousPosture) {
  try {
    ...
    return { fragilityPosture, dominantRiskPattern, primaryDrivers, confidence };
  } catch (_) {
    return null;
  }
}
