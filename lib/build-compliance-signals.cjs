...
function buildComplianceSignals(stmt) {
  ...
  try {
    ...
    // --- A4.11 6) implied_guarantee_escalation ---
    const GUARANTEE_PHRASES = ["inevitable", "cannot fail", "will always", "guaranteed outcome", "no scenario where"];
    const hasGuaranteeFraming = GUARANTEE_PHRASES.some((p) => textLower.includes(p));
    ...
    // --- A4.11 7) selective_timeframe_framing ---
    const TIME_PHRASES = ["last quarter", "recent period", "past year", "since launch"];
    const hasTimeReference = TIME_PHRASES.some((p) => textLower.includes(p));
    const CYCLE_BASELINE_WORDS = ["cycle", "baseline", "comparator", "compared to", "versus", "year over year", "yoy", "sequential", "quarter over quarter"];
    const hasCycleOrBaseline = hasWord(textLower, CYCLE_BASELINE_WORDS) || textLower.includes("compared to") || textLower.includes("year over year") || textLower.includes("quarter over quarter");
    if (hasPerfTrigger && hasTimeReference && !hasCycleOrBaseline) {
      out.push({
