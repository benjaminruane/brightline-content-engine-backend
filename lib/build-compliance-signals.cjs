...
// A4.11: Compliance Phase 2 — risk_omission, implied_guarantee_escalation, selective_timeframe_framing.
...
function buildComplianceSignals(stmt) {
  ...
  try {
    ...
    // --- A4.11 5) risk_omission ---
    const POSITIVE_OUTCOME_WORDS = ["strong growth", "clear advantage", "compelling", "success", "outperform", "leading", "robust", "outstanding", "market leader", "dominant", "highly attractive"];
    const hasPositiveOutcomeFraming = hasWord(textLower, POSITIVE_OUTCOME_WORDS) || hasWord(textLower, POSITIVE_FRAMING_WORDS);
    const hasForwardLookingOrPerf = hasWord(textLower, PROJECTION_WORDS) || hasPerfTrigger;
    const RISK_CONSTRAINT_WORDS = ["risk", "downside", "challenge", "uncertain", "may", "could", "depends", "subject to", "exposed to", "sensitive to"];
    const hasRiskOrConstraint = hasWord(textLower, RISK_CONSTRAINT_WORDS) || textLower.includes("subject to") || textLower.includes("exposed to") || textLower.includes("sensitive to");
    if (hasPositiveOutcomeFraming && hasForwardLookingOrPerf && !hasRiskOrConstraint) {
      let severity = "medium";
      if (band === "weak" || band === "none" || coverageGapMaterial) severity = "high";
      out.push({
        type: "risk_omission",
        severity,
        confidence: 0.7,
        reason: "Positive outcome claim without accompanying risk or constraint framing.",
      });
    }

    // --- A4.11 6) implied_guarantee_escalation (A4.11.1: single-word + phrase triggers) ---
    const GUARANTEE_SINGLE_WORDS = ["inevitably", "certainly", "guaranteed", "unstoppable", "unavoidable", "inevitable"];
    const GUARANTEE_PHRASES = ["cannot fail", "will always", "no scenario where", "guaranteed outcome"];
    const hasGuaranteeFraming =
      hasWord(textLower, GUARANTEE_SINGLE_WORDS) || GUARANTEE_PHRASES.some((p) => textLower.includes(p));
    if (hasGuaranteeFraming) {
      out.push({
        type: "implied_guarantee_escalation",
        severity: "high",
        confidence: 0.85,
        reason: "Inevitability or guarantee framing beyond evidence support.",
      });
    }

    // --- A4.11 7) selective_timeframe_framing (A4.11.1: relaxed trigger = time + positive/perf, no comparator) ---
    const TIME_PHRASES = [
      "last quarter", "last year", "past year", "recent period", "since launch", "since inception",
      "recently", "in recent quarters",
    ];
    const hasTimeReference = TIME_PHRASES.some((p) => textLower.includes(p));
    const POSITIVE_POSITIONING_PHRASES = [
      "market leadership", "industry leading", "outperform", "best in class", "exceptional performance",
      "strong performance", "category leader",
    ];
    const hasStrongPositiveOrPerf = hasPerfTrigger || POSITIVE_POSITIONING_PHRASES.some((p) => textLower.includes(p));
    const COMPARATOR_PHRASES = [
      "cycle", "cyclical", "historical average", "long term average", "baseline", "normalised",
      "across cycles", "through cycle", "vs historical",
    ];
    const hasComparatorLanguage = COMPARATOR_PHRASES.some((p) => textLower.includes(p));
    if (hasTimeReference && hasStrongPositiveOrPerf && !hasComparatorLanguage) {
      let severity = "medium";
      if (band === "weak" || band === "none" || coverageGapMaterial) severity = "high";
      out.push({
        type: "selective_timeframe_framing",
        severity,
        confidence: 0.65,
        reason: "Performance claim references narrow timeframe without cycle or baseline context.",
      });
    }
