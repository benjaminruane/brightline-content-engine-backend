...
import { applyUnreportedChangeMarkers } from "./pr9-unreported-change-markers.mjs";
import { isAuthorStatementKeptNote } from "./revise-author-statement.mjs";
import { endsWithFlagRegisterNote } from "./revise-flag-register.mjs";
...
function collectEditorialConcerns(card) {
  ...
  const silent = resolveEvidenceKind(card) === "no_support";
  const out = [];
  for (const concern of list) {
    ...
    // B138: silent cards copy only first-person actor rules into the reviser
    // prompt. The card is not mutated; the user still sees the withheld flag.
    if (silent && !isFirstPersonActorRule(norm(rule), concern.rule)) continue;
...
export function gatherConcerns(statements, publicationMap = null) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    // Read-only copy for the flag register. Additive: nothing reads it for
    // verdict, aggregation or prompt text, and the card is not modified.
    if (card?.materiality) item.materiality = card.materiality;
    ...
    if (hasClaimSpans) {
      ...
      for (const c of item.claims) {
        ...
        if (c.role === "confirmed_preserve") {
          ...
          lines.push(
            `  Unsupported element (the keep-and-flag rule applies to this span): "${quoted}" [verdict=${verdict}]`
          );
      ...
      if (item.evidence) {
        ...
        if (!item.evidence.reason && !item.evidence.excerpt && !item.evidence.sourcePassage) {
          lines.push("  (No excerpt/reason available - the source is silent, so keep the claim as written and flag it.)");
        }
      }
      ...
      if (!item.evidence.reason && !item.evidence.excerpt && !item.evidence.sourcePassage) {
        lines.push("  (No excerpt/reason available - the source is silent, so keep the claim as written and flag it. A named first-person subject replacement on this statement is still followed.)");
      }
    }
...
function buildResolvedStyleGuideBlock(outputType) {
  ...
  // B139: drop hyperbole_vs_qualitative from Suggest's global block. Formatting
  // rules stay entire-draft; this rule changes what a sentence asserts, so it
  // may reach Suggest only as a named concern. B138 already withholds that
  // concern on silent cards. Review still receives the full guide via
  // formatStyleGuideRulesForPrompt in the editorial reviewer — this filter is
  // Suggest-injection only.
  return formatStyleGuideRulesForPrompt(rules.filter((r) => r.id !== "hyperbole_vs_qualitative"));
}

function copyMarker(m, extra = {}) {
  const out = {
    // Spread first so additive fields (generated-marker provenance) survive
    // the finalise chain; the explicit keys below still win.
    ...m,
    start: m.start,
    end: m.end,
...
export function normalizeMarkerNoteText(note) {
  ...
  if (!trimmed) return "";
  // The author-statement note is deliberately closer-less; see
  // AUTHOR_STATEMENT_KEPT_NOTE.
  if (isAuthorStatementKeptNote(trimmed)) return trimmed;
  // The loud and quiet flag registers set their own volume; see
  // lib/revise-flag-register.mjs. This also covers the combined form, where an
  // edit clause is followed by a register clause because the edit left the
  // silence finding unaddressed — the register still closes the note.
  if (endsWithFlagRegisterNote(trimmed)) return trimmed;
 ...
 */
function applyNoteWhatFromDiff(originalDraft, parsed, concerns) {
  const draft = typeof parsed?.revisedDraft === "string" ? parsed.revisedDraft : "";
  const incoming = Array.isArray(parsed?.markers) ? parsed.markers : [];
  if (!originalDraft) return { revisedDraft: draft, markers: incoming.map((m) => copyMarker(m)) };

  const markers = incoming.map((m) => {
    // A generated marker's note was already written against the model's raw
    // output. Regenerating it here would diff against a draft that code has
    // since normalised, folding house-style edits into the model's account.
    if (m.generated === true) return copyMarker(m);
    // Nothing changed under an author-statement marker, so there is no diff to
    // describe and the note already says exactly why. Register notes are
    // re-derived below rather than short-circuited, so a span that DID change
    // falls back to an honest account of the change.
    if (isAuthorStatementKeptNote(m.note)) return copyMarker(m);
...
export function finalizeSuggestRevisionText(rawLlmText, opts = {}) {
  ...
  // Runs here, on the model's raw output, because every stage below mutates
  // the draft and would otherwise be reported as an unreported model edit.
  const unreported = applyUnreportedChangeMarkers(originalDraft, parsed, {
    concerns: opts.concerns,
    traceId: opts.traceId,
    log: opts.log,
  });

  const notesNormalized = applyNormalizeMarkerNotes(unreported);
  ...
  const whatFromDiff = applyNoteWhatFromDiff(originalDraft, punctClean, opts.concerns);
  ...
  if (!originalDraft) {
    return {
      ...
      unreportedEvents: unreported.unreportedEvents,
    };
  }
...
export function buildRevisionPrompt(draftText, concerns, opts = {}) {
  ...
  // Retired 2026-08-29: the edge case existed only to brake the cut branch of
  // rule (b), and silence no longer cuts. The constant stays exported.
  void UNSUPPORTED_WHOLE_SENTENCE_EDGE_CASE_LIVE;
...
- The ENTIRE revised draft must comply with HOUSE STYLE RULES below (not only the flagged statements) — including currency_format (ISO code + "million"/"billion", not symbol/"m"), thousand_separator (high comma / apostrophe), number_spelling, and first_person_plural where it applies to this output type.
...
NEVER SUBSTITUTE A DIFFERENT FACT. Where the source is silent on what the draft asserts, do not replace the draft's claim with some other statement drawn from the source, however well supported that other statement is. "with equity checks of EUR 80-100 million apiece" must NOT become "with reserved capital for bolt-on acquisitions" because the source happens to mention reserved capital. That deletes the author's claim, puts a claim they never made in its place, and makes the sentence read as fully supported on re-review. It is the same laundering as approximating an unsupported figure, and it is worse, because the result looks clean.
...
Wrap the revised element or the kept element in a marker.
c) kind "partial": Keep the CONFIRMED portion unchanged. If the source STATES a specific value for the unsupported element, inject that source value into the prose (house-style) and wrap THAT element in a marker (e.g. "around USD 1.9 billion"). When the source is SILENT or vague on the unsupported element, leave that element exactly as written and wrap it in a marker: no soften, no cut, and no substituting a different fact from the source. Never approximate the author's unsupported figure. Do not vague out a supported fact because another part of the same statement is unsupported.
...
- Example (unsupported, source silent, wording kept): {{delivered 22% revenue growth last year||KEPT: Kept the 22% figure as written — no supplied source states a rate. Confirm before publishing.}}
- Example (partial, source silent on the element, wording kept): {{trades at 14x EV/EBITDA||KEPT: Kept the 14x multiple as written — no supplied source states a multiple. Confirm before publishing.}}
