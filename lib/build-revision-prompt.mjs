...
e) kind "soften" (marketing_language_excess): Tone down the promotional phrase in the prose AND wrap the revised phrase in a marker. Example: "Softened 'excited to announce incredible growth' to a neutral description — it read as promotional. Confirm before publishing."
...
- Example (partial / stated source value): {{around USD 1.9 billion||Changed from over USD 2 billion to around USD 1.9 billion — the sources don't back the higher figure. Confirm before publishing.}}
- Example (unsupported, source silent, claim stands): {{delivered revenue growth last year||Removed the unsupported 22% figure - sources do not state a rate. Confirm before publishing.}}
- Example (unsupported, source silent, figure WAS the claim): {{The company serves customers across Europe||Removed the unsupported 14x EV/EBITDA clause - sources do not state a multiple. Confirm before publishing.}}
- Example (deletion / keep-and-flag): {{The office also has a red kettle||Kept the kettle detail — review flagged it as immaterial, so consider cutting. Confirm before publishing.}}
- Example (soften / marketing): {{solid growth across every segment||Softened 'excited to announce incredible growth' to a neutral description — it read as promotional. Confirm before publishing.}}
- Example (compliance_strip, no downgrade): {{the diligence lead||Removed Jane Smith — named person in a public version. Confirm before publishing.}}
- Example (compliance_strip, KEEP-AND-FLAG): {{Jane Smith led the diligence||Kept Jane Smith — a supporting source is already public, so check whether removal is still needed. Confirm before publishing.}}
- Example (evidence named entity / keep-and-flag): {{BVP is evaluating an investment of up to USD 7 million in Shopify||Kept 'BVP' — the source says 'the firm', not BVP, so confirm the attribution. Confirm before publishing.}}
- Example (compliance_claim): {{aims to deliver improved returns||Softened 'certain to deliver outsized returns' to a non-guaranteed aim — guaranteed-return language isn't permitted. Confirm before publishing.}}
