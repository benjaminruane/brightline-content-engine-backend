 ...
 * @param {{ originalDraft?: string, traceId?: string, log?: Function }} [opts]
 * @returns {{
 *   revisedDraft: string,
...
export function finalizeSuggestRevisionText(rawLlmText, opts = {}) {
  ...
  if (!originalDraft) {
    return { ...punctClean, honestyEvents: [] };
  }
  return applyMarkerHonestyCheck(originalDraft, punctClean, {
...
e) kind "soften" (marketing_language_excess): Never substitute a milder evaluative word for a stronger one. Follow suggestedDirection. If it begins with Delete, delete the quoted evaluative phrase (and any intensifier attached only to it), repair stranded scaffolding if needed, and wrap a surviving remnant. If it begins with Keep, keep the author's wording unchanged and wrap it. Apply the same remaining-clause test as unsupported figures: if the clause still informs after the word is gone, delete the word; if the evaluation is the whole point, keep and flag. Never replace "exceptional" with "strong" or any quieter synonym.
