...
/**
 * Region-independent: CUT (removal) claim while the marked span's containing
 * sentence still appears verbatim (same word sequence) in the original draft.
 * The span is a slice of the revised draft, so a non-empty span is present in
 * the revised text by construction (Condition A wrap-in-place failure).
 * Neighbour edits must not mask this; no LCS.
 *
...
export function cutSpanTextPresentInRevised(original, revised, start, end) {
  ...
  return tokenSequenceIncludes(honestyCompareKey(original), sentKey);
}
