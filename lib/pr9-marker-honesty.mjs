...
import { confineRegionToOwnSentence } from "./pr9-note-what-from-diff.mjs";
...
/**
 * Abbreviations whose full stop is not a sentence end. Lower-cased for lookup.
 * Deliberately short: every entry here is a place a sentence CANNOT end, and a
 * wrong entry silently merges two real sentences.
 */
...
/**
 * Is the character at `i` really the end of a sentence?
 *
 * "!" and "?" always are. "." usually is, but investment writing is full of
 * places it is not, and reading one of those as a sentence end truncates the
 * bounds. Measured on the committed corpus: 19 of 67 sentences, 28%, every one
 * of them a decimal in a figure.
 *
 * @param {string} source
 * @param {number} i
 * @returns {boolean}
 */
function isSentenceTerminatorAt(source, i) {
  const ch = source[i];
  ...
  // "EUR 1.2 billion", "2.4x". The overwhelming majority of the real cases.
  if (/\d/.test(prev) && /\d/.test(next)) return false;

  // An ellipsis is one token, not three sentence ends.
  if (next === "." || prev === ".") return false;

  // "J. Smith", "Fund V. The". A single letter standing alone before the stop
  // is an initial or an enumerator, never the last word of a sentence. This
  // outranks the capital-follows test below, which is why "Fund V. The" joins.
  //
  // The letter must not follow a digit, or the "x" of "1.9x." reads as an
  // initial and a sentence that genuinely ends on a multiple never closes.
  if (/[A-Za-z]/.test(prev) && !/[A-Za-z0-9]/.test(source[i - 2] ?? "")) return false;

  // "e.g.", "i.e.", "U.S.A." — a stop wedged between letters with no space.
  if (/[A-Za-z]/.test(prev) && /[A-Za-z]/.test(next)) return false;

  // A known abbreviation, but only where what follows does not look like a new
  // sentence. "approx. 20 investments" and "Meridian Ltd. of London" join;
  // "Meridian Capital Management Ltd. It was founded" still splits, because an
  // abbreviation rule that swallows a real boundary is worse than the bug.
  const word = /([A-Za-z]+)$/.exec(source.slice(Math.max(0, i - 12), i));
  if (word && NON_TERMINAL_ABBREVIATIONS.has(word[1].toLowerCase())) {
    const following = /^\s+(\S)/.exec(source.slice(i + 1));
    const startsLikeASentence = following ? /[A-Z"\u201c]/.test(following[1]) : false;
    if (!startsLikeASentence) return false;
  }
 ...
 * Terminals are . ! ? or a blank line, excluding the false stops listed in
 * isSentenceTerminatorAt. Does not expand marker offsets.
 *
 * @param {string} text
 * @param {number} start
 * @param {number} end
...
export function sentenceBoundsContaining(text, start, end) {
  ...
  while (left > 0) {
    ...
    if (isSentenceTerminatorAt(source, left - 1)) break;
  ...
  while (right < source.length) {
    ...
    if (isSentenceTerminatorAt(source, right)) {
      right += 1;
      break;
    }
...
export function applyMarkerHonestyCheck(originalDraft, parsed, opts = {}) {
  ...
  const markers = incoming.map((m) => {
    ...
    // KEPT only. The leaky LCS window is still what CUT honesty and the
    // aligner itself use. A neighbour-sentence edit must not make a byte-
    // identical KEPT span look rewritten (B140).
    if (intent === MARKER_INTENT_KEPT && spanStatus === SPAN_CHANGED) {
      const confined = confineRegionToOwnSentence(
        original,
        revisedDraft,
        start,
        end,
      ...
      const confinedKey = confined.map((t) => t.text).join("\0");
      const revKey = align.revSpan.map((t) => t.text).join("\0");
      if (confinedKey === revKey) spanStatus = SPAN_UNCHANGED;
    }
    if (intent === MARKER_INTENT_KEPT && spanStatus === SPAN_CHANGED) {
