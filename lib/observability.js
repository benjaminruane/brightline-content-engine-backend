...
export function isRateLimitError(err) {
  ...
  if (code === "rate_limit_exceeded" || code === "rate_limit_error") return true;
...
export async function withRateLimitRetry(fn, { sleep = defaultSleep, now = Date.now } = {}) {
  ...
  while (attempt < RATE_LIMIT_MAX_ATTEMPTS) {
    ...
    try {
      ...
      if (!isRateLimitError(err) || isRateLimitBoundError(err)) throw err;
      try {
      ...
      const at = typeof now === "function" ? Number(now()) : Date.now();
      const boundMs = computeWaitBoundMs(at);
