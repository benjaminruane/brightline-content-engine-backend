...
const openaiClient = openAiApiKey ? new OpenAI({ apiKey: openAiApiKey, maxRetries: 0 }) : null;
...
export const RATE_LIMIT_MAX_DELAY_MS = 2000;
...
export function rateLimitDelayMs(err, random = Math.random) {
  const base = Math.min(parseRetryAfterMs(err), RATE_LIMIT_MAX_DELAY_MS);
  const roll = typeof random === "function" ? Number(random()) : 0;
  const jitter = Math.floor(base * 0.25 * (Number.isFinite(roll) ? Math.min(Math.max(roll, 0), 1) : 0));
  return Math.min(base + jitter, RATE_LIMIT_MAX_DELAY_MS);
}
...
export async function withRateLimitRetry(fn, { sleep = defaultSleep, random = Math.random } = {}) {
  let lastErr;
  for (let attempt = 1; attempt <= RATE_LIMIT_MAX_ATTEMPTS; attempt += 1) {
    try {
      ...
      await sleep(rateLimitDelayMs(err, random));
    }
  }
  throw lastErr;
}
