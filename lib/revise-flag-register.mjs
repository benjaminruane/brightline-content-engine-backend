 ...
 */
const NAMED_THIRD_PARTY_RE = /\b[A-Z][a-zA-Z0-9&'’-]+(?:\s+(?:[A-Z][a-zA-Z0-9&'’-]+|[IVXLC]{1,5}\b))+/g;

/**
 * THE AUTHOR IS NOT A THIRD PARTY. "Halden Group expects the relationship to
 * deepen over the life of the fund" was LOUD purely because the authoring
 * organisation's own name matched the named-entity probe, which made the
 * author's own expectation read as an unbacked claim about someone else. Ben
 * decided on 2026-08-29 that these belong in QUIET.
 *
 * Read-only, resolved at call time from the same precedence chain Review uses
 * (argument, then request, then AUTHORING_ORGANISATION). Nothing about Review
 * or any card changes. Where no organisation is configured the resolution is
 * null and every match stands, so behaviour is unchanged.
 *
 * Only the organisation's own name is excluded. "Halden Group committed to
 * Meridian Capital Partners V" still matches on Meridian, because that IS a
 * third party.
 *
 * @param {string} text
 * @param {string|null} [authoringOrganisation]
 * @returns {string[]} the named entities that are not the author
 */
export function namedThirdPartiesIn(text, authoringOrganisation = undefined) {
  const source = collapse(text);
  if (!source) return [];
  const house =
    authoringOrganisation === undefined
      ? resolveAuthoringOrganisationName()
      : trimAuthoringOrganisationName(authoringOrganisation);
  const houseKey = house ? collapse(house).toLowerCase() : "";
  const matches = source.match(NAMED_THIRD_PARTY_RE) ?? [];
  if (!houseKey) return matches;
  // A match "is the author" when it is the house name, or the house name plus
  // trailing capitalised words that belong to it ("Halden Group Partners").
  return matches.filter((m) => {
    const key = collapse(m).toLowerCase();
 ...
 * @param {string} elementText
 * @param {string|null} [authoringOrganisation] omit to resolve from config
 * @returns {string[]}
 */
export function loudTextSignalsOf(elementText, authoringOrganisation = undefined) {
  const text = collapse(elementText);
  if (!text) return [];
  const fired = TEXT_SIGNALS.filter(([, re]) => re.test(text)).map(([name]) => name);
  if (namedThirdPartiesIn(text, authoringOrganisation).length > 0) fired.push("named_third_party");
 ...
 * @param {{ authoringOrganisation?: string|null }} [opts] omit to resolve from config
 * @returns {{
 ...
 * }}
 */
export function flagRegister(concern, card = null, elementText = undefined, opts = {}) {
  ...
  const textSignals = loudTextSignalsOf(element, opts.authoringOrganisation);

  // Review's `named_person_entity_attribution` counts the authoring
  // organisation like any other name, so it reaches the same wrong conclusion
  // as the text probe did: "Halden Group expects the relationship to deepen"
  // is the author's own expectation, not an unbacked claim about a third
  // party. Drop the feature where the element names nobody but the author.
  // Review and its cards are untouched; this is a read at classification time.
  const featureSignals = loudFeaturesOf(card ?? concern).filter(
    (f) =>
      f !== "named_person_entity_attribution" ||
      namedThirdPartiesIn(element, opts.authoringOrganisation).length > 0
  );
...
/**
 * A note that ENDS on a register clause, whether or not an edit clause precedes
 * it. The combined form is what a statement gets when it was edited AND still
 * carries an unaddressed silence finding, and it needs the same closer
 * exemption as a bare register note: LOUD already ends on its own instruction,
 * and QUIET asks nothing of the reader.
 *
 * @param {unknown} note
 * @returns {boolean}
 */
export function endsWithFlagRegisterNote(note) {
  const text = collapse(note);
  if (!text) return false;
  return text.endsWith(collapse(LOUD_NOTE)) || text.endsWith(collapse(QUIET_NOTE));
}

/**
 * Did the edit deal with the thing the source is silent about, or is the claim
 * still standing in the revised text?
 *
 * The finding is ADDRESSED only when the flagged element's own content is
 * largely gone from the revised span — the model cut the unsupported clause, so
 * there is nothing left to flag. A rewrite that keeps the claim and changes its
 * voice, its punctuation or its subject leaves the finding exactly where it
 * was, and the reader still needs telling.
 *
 * Ties go to UNADDRESSED. Over-flagging repeats something true; under-flagging
 * is the masking this exists to stop.
 *
 * @param {string} elementText  the flagged element, from the original
 * @param {string} revisedSpan  what stands in its place now
 * @returns {boolean}
 */
export function findingAddressedByEdit(elementText, revisedSpan) {
  const words = (s) =>
    collapse(s)
      .toLowerCase()
      .replace(/[^\p{L}\p{N}\s]/gu, " ")
      .split(/\s+/)
      .filter((w) => w.length > 2 && !EDIT_NEUTRAL_WORDS.has(w));

  const before = words(elementText);
  if (before.length === 0) return false;
  const after = new Set(words(revisedSpan));
  // Prefix match, so "recommend" still counts as surviving in "recommends".
  const survivors = before.filter((w) =>
    [...after].some((a) => a.startsWith(w.slice(0, 5)) || w.startsWith(a.slice(0, 5)))
  );
  return survivors.length / before.length < 0.5;
}

/**
 * Words a house-style rewrite routinely swaps, which therefore say nothing
 * about whether the CLAIM survived. First person is the main one: "we
 * recommend" becoming "Halden Group recommends" changes every word on this list
 * and none of the substance.
 */
const EDIT_NEUTRAL_WORDS = new Set([
  "the",
  "and",
  "that",
  "this",
