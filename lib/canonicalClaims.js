...
export function canonicalizeClaims(rawClaims, ctx) {
  ...
  const normalized = rawClaims.length > 0
    ? rawClaims.map((raw, idx) => normalizeRawClaim(raw, statementText, knownEntities, idx, rawClaims, log, entityDropLogCounter))
    : [];
  ...
  // A3.8.9: Step 6: Qualitative fallback canonical claim (hard invariant - always create if empty)
