  ...
  if (hasNumeric && (hasCurrency || hasPercent)) {
    ...
    if (hasCurrency && statementText) {
      ...
      if (usdMatch) {
        ...
        if (valuationScore > 0) {
          ...
          if (/\b(pre-?money|premoney|pre\s+money)\b/i.test(contextWindow)) {
            if (log) {
              ...
              const ctxPreview = contextWindow.substring(0, 60);
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_pre_money valScore=${valuationScore} invScore=${investmentScore} ctxPreview="${ctxPreview}..."`);
            }
          ...
          if (/\b(post-?money|postmoney|post\s+money)\b/i.test(contextWindow)) {
            if (log) {
              const valueDisplay = usdMatch[1];
              const ctxPreview = contextWindow.substring(0, 60);
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_post_money valScore=${valuationScore} invScore=${investmentScore} ctxPreview="${ctxPreview}..."`);
            }
          ...
          if (/\b(enterprise\s+value|EV)\b(?!\w)/i.test(contextWindow)) {
            if (log) {
              const valueDisplay = usdMatch[1];
              const ctxPreview = contextWindow.substring(0, 60);
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_enterprise_value valScore=${valuationScore} invScore=${investmentScore} ctxPreview="${ctxPreview}..."`);
            }
        ...
        // Only classify as investment_amount if investment context present and valuation absent
        if (investmentScore > 0 && valuationScore === 0) {
          // Will fall through to investment_amount classification below
        } else if (valuationScore === 0 && investmentScore === 0) {
