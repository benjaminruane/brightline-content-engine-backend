...
export function canonicalizeClaims(rawClaims, ctx) {
  ...
  // A3.8.2: Step 6: Qualitative fallback canonical claim
  ...
  if (finalCanonical.length === 0) {
    ...
    const fallbackClaim = createQualitativeFallbackClaim(statementText, fallbackCtx, computedSelectionHash, selectionMode);
    if (fallbackClaim) {
      finalCanonical = [fallbackClaim];
      log(`[CANON][FALLBACK_QUAL] created=1`);
    }
  }
...
function normalizeRawClaim(raw, statementText, knownEntities, idx) {
  ...
  // Extract entity hints (A3.8.2: pass statementText for inference)
  const entityHints = {
    company: extractCompany(claimText, knownEntities.company, statementText),
    round: extractRound(claimText, knownEntities.round, statementText),
  };
  
  // Infer canonical type (A3.8.2: pass roleKeywords)
  let inferredType = inferCanonicalType(claimText, numeric, currency, percent, anchorFamily, roleKeywords);
  ...
  if (hasNumeric && (hasCurrency || hasPercent) && qualitativeTypes.has(inferredType)) {
    ...
    inferredType = inferCanonicalType(claimText, numeric, currency, percent, anchorFamily, roleKeywords);
...
function extractRoleKeywords(text) {
  ...
  const rolePatterns = [
    ...
    { pattern: /\b(ownership|own|stake|equity|%\s*|fully\s+diluted|FD)\b/i, keyword: "ownership" },
...
function normalizeEntity(str) {
  ...
  // Disallow generic finance tokens as company
  const bannedCompanyTokens = ["Series", "Financing", "Investment", "Valuation", "Enterprise", "Value", "EV"];
  if (bannedCompanyTokens.includes(normalized)) {
    return null;
  }
...
/**
 * Extract company name from claim text or use known entity (A3.8.2: improved)
 */
function extractCompany(text, knownCompany = null, statementText = null) {
  // Use known entity if available
  if (knownCompany) {
    const normalized = normalizeEntity(knownCompany);
  ...
  const bannedCompanyTokens = ["Series", "Financing", "Investment", "Valuation", "Enterprise", "Value", "EV", "USD", "Million", "Billion"];
  const candidates = capitalized.filter(w => !bannedCompanyTokens.includes(w));
  
  if (candidates.length > 0) {
    const normalized = normalizeEntity(candidates[0]);
    if (normalized) return normalized;
  }
  
  // A3.8.2: Inference fill - if statementText has exactly one likely company token
  if (statementText) {
    const stmtWords = statementText.split(/\s+/);
    const stmtCapitalized = stmtWords.filter(w => /^[A-Z][a-z]+/.test(w) && w.length > 2 && !bannedCompanyTokens.includes(w));
    const uniqueStmtCompanies = Array.from(new Set(stmtCapitalized));
    if (uniqueStmtCompanies.length === 1) {
      const normalized = normalizeEntity(uniqueStmtCompanies[0]);
      if (normalized) return normalized;
    }
  }
 ...
 */
function inferCanonicalType(claimText, numeric, currency, percent, anchorFamily, roleKeywords = []) {
  ...
  if (hasNumeric && (hasCurrency || hasPercent)) {
    ...
    // If we have percent but no ownership keywords, still return ownership_percent
    if (hasPercent) {
      return "ownership_percent";
    }
  }
...
/**
 * A3.8.2: Create qualitative fallback canonical claim
 */
function createQualitativeFallbackClaim(statementText, ctx, selectionHash, selectionMode) {
  ...
  // Trim and cap displayText at 140 chars
  const displayText = statementText.trim().substring(0, 140);
  
  // Get citations from assessment if available
  const citations = Array.isArray(assessment.citations) ? assessment.citations : [];
  
  // Get reliability from assessment or default to Medium
...
function groupNormalizedClaims(normalized, selectionHash, selectionMode, statementText, knownEntities) {
  ...
  // If one has round and other is null, merge them
  const financialTypes = new Set([
    "investment_amount",
    "valuation_pre_money",
    "valuation_post_money",
  ...
  for (const [key, claims] of groups.entries()) {
    ...
    if (isFinancial && claims.length > 0) {
      // Try to find other groups that differ only by round (one null, one non-null)
...
/**
 * Build grouping key (A3.8.2: soft round matching for financial types)
 */
function buildGroupingKey(norm, selectionHash, selectionMode) {
  ...
  // Normalize company (null-safe)
  const companyKey = norm.entityHints.company || "null";
...
function mergeGroupIntoCanonical(groupClaims, statementText, selectionHash, selectionMode, groupKey) {
  ...
  return {
    ...
    company: mergedCompany || first.entityHints.company,
    round: mergedRound || first.entityHints.round,
