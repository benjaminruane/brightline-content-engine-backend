...
export function canonicalizeClaims(rawClaims, ctx) {
  ...
  for (const norm of normalized) {
    const ctx = { statementText, knownEntities, selectionMode, runId, reqSig, statementIndex };
    ...
    if (wouldBeJunk) {
      ...
      // A3.8.59: Selection-mode guard - never drop metric_amount due to missing span metadata
      const selectionMode = ctx.selectionMode === true;
      const hasUsdAnchor = norm.anchorFamily && norm.anchorFamily.startsWith("usd_");
      const inferredType = norm.inferredType;
      // A3.8.62: Extended to include all economically meaningful types (used by both A3.8.59 and A3.8.60 guards)
      const isEconomicallyMeaningful = inferredType === "metric_amount" || 
                                       inferredType === "investment_amount" ||
                                       inferredType === "valuation_amount" ||
                                       inferredType === "valuation_pre_money" ||
                                       inferredType === "valuation_post_money" ||
                                       inferredType === "valuation_enterprise_value" ||
                                       inferredType === "valuation_equity_value" ||
                                       inferredType === "ownership_percent" ||
                                       inferredType === "secondary_purchase";
      
      if (selectionMode && hasUsdAnchor && isEconomicallyMeaningful) {
        // A3.8.59: Preserve metric_amount claims in selection mode even if sourceSpan is null
        // Set sourceSpan to statement-level fallback
        const statementText = ctx.statementText || "";
        if (statementText && !norm.sourceSpan) {
          norm.sourceSpan = { start: 0, end: statementText.length };
        } else if (statementText && (norm.sourceSpan.start === null || norm.sourceSpan.end === null)) {
          norm.sourceSpan = { start: 0, end: statementText.length };
        }
        
        // A3.8.59: DIAG log for selection-mode override
        if (ctx.runId && ctx.reqSig) {
          const preview = (norm.claimText || "").substring(0, 60);
          const anchor = norm.anchorFamily || "none";
          log(`[DIAG][A3.8.59][SELECTION_NUMERIC_FRAGMENT_OVERRIDE] idx=${ctx.statementIndex || 0} anchor=${anchor} type=${inferredType} preview="${preview}"`);
        }
      ...
      // A3.8.60: Selection mode guard - block DROP_RAW at actual drop site
      // A3.8.62: selectionMode, hasUsdAnchor, inferredType, and isEconomicallyMeaningful already declared above (lines ~184-193), reuse them
      const anchor = norm.anchorFamily || "none";
      // Reuse hasUsdAnchor, inferredType, and isEconomicallyMeaningful from above (lines ~185-193)
      
      if (selectionMode && hasUsdAnchor && isEconomicallyMeaningful) {
        // A3.8.60: Do NOT drop raw claim for reason=numeric_fragment in selection mode
        // Preserve it and continue canonicalization
        if (runId && reqSig && log) {
          const preview = (norm.claimText || "").substring(0, 60);
          log(`[DIAG][A3.8.60][SELECTION_NO_DROP_NUMERIC_FRAGMENT] anchor=${anchor} type=${inferredType} preview="${preview}"`);
        }
        
        // Ensure sourceSpan is set (use fallback if needed)
        if (!norm.sourceSpan || norm.sourceSpan.start === null || norm.sourceSpan.end === null) {
          const statementText = ctx.statementText || "";
          if (statementText) {
            norm.sourceSpan = { start: 0, end: statementText.length };
          }
...
function normalizeRawClaim(raw, statementText, knownEntities, idx, rawClaims = null, log = null, logCounter = { count: 0 }, statementIndex = 0, selectionMode = false, runId = null, reqSig = null) {
  ...
  // A3.8.60: Apply span fallback BEFORE binding and drop checks
  // If selectionMode=true AND rawClaim.anchor starts with "usd_" AND sourceSpan is null/invalid:
  // set statement-level fallback span
  if (selectionMode) {
    const anchor = raw.anchor || "";
    const hasUsdAnchor = anchor.startsWith("usd_");
    const sourceSpan = raw.sourceSpan;
    const spanInvalid = !sourceSpan || sourceSpan.start === null || sourceSpan.end === null ||
                       sourceSpan.start < 0 || sourceSpan.end > (statementText?.length || 0) || sourceSpan.start >= sourceSpan.end;
    
    if (hasUsdAnchor && spanInvalid && statementText) {
      raw.sourceSpan = { start: 0, end: statementText.length };
      raw.sourceSpanFallback = true; // Mark as fallback for DIAG
      
      // A3.8.60: DIAG log for span fallback
      if (runId && reqSig && log) {
        log(`[DIAG][A3.8.60][SPAN_FALLBACK] anchor=${anchor} applied=true start=0 end=${statementText.length}`);
      }
    }
  }
 ...
 *
 * AUTHOR-NAME-BLIND: entityHints.company is a symmetric MERGE KEY. Two claims
 * with different companies are not deduplicated into one; it never decides
 * support, relevance or a verdict. The author is a legitimate subject of its own
 * claims ("Partners Group committed EUR 100m"), so excluding its name would make
 * the key less discriminating rather than more correct. First-person pronouns are
 * already dropped by isJunkEntityToken. Adjudicated in anchor-fix-corpus.md.
 */
...
function inferCanonicalType(claimText, numeric, currency, percent, anchorFamily, roleKeywords = [], statementText = null, statementIndex = 0, log = null, selectionMode = false) {
  ...
  if (hasNumeric && (hasCurrency || hasPercent)) {
    ...
    // A3.8.58: Binding-first classification (non-arbitrary, span-bound)
    if (hasCurrency && statementText) {
      ...
      if (usdMatch) {
        ...
        // A3.8.58: Try binding-first classification
        const bindingResult = classifyCurrencyByBindingPatterns(statementText, sourceSpan, { windowChars: 120 });
        
        // A3.8.58: DIAG log for binding classification
        if (log && statementIndex !== null) {
          const trimmedWindow = statementText.substring(
            Math.max(0, sourceSpan.start - 60),
            Math.min(statementText.length, sourceSpan.end + 60)
          );
          const windowPreview = trimmedWindow.length > 120 ? trimmedWindow.substring(0, 120) + "..." : trimmedWindow;
          log(`[DIAG][A3.8.58][CURRENCY_BINDING] idx=${statementIndex} anchor=${anchorFamily || "unknown"} span=${sourceSpan.start},${sourceSpan.end} family=${bindingResult.family || "NONE"} subtype=${bindingResult.subtype || "null"} matched=${bindingResult.reason || "none"} window="${windowPreview}"`);
        }
        
        // A3.8.58: If binding found a match, use it (override local context)
        if (bindingResult.family && bindingResult.subtype) {
          if (log) {
            const valueDisplay = usdMatch[1];
            log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=${bindingResult.subtype} decisionReason=binding_pattern_${bindingResult.reason}`);
          }
          return bindingResult.subtype;
        }
        
        // A3.8.57: Fall back to local context window classification
        ...
        if (chosenFamily === "INVESTMENT") {
          ...
          // A3.8.58: Add equity value detection
          ...
          const hasEquityValue = /\b(equity\s+value|equity\s+valuation|equity-value|market\s+cap(?:italization)?)\b/i.test(windowText);
          ...
          } else if (hasEquityValue) {
            // A3.8.58: Equity value classification
            if (log) {
              const valueDisplay = usdMatch[1];
              const trimmedWindow = windowText.length > 80 ? windowText.substring(0, 80) + "..." : windowText;
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_equity_value decisionReason=currency_local_ctx_valuation_equity ctxPreview="${trimmedWindow}..."`);
            }
            return "valuation_equity_value";
          } else {
...
function classifyCurrencyByLocalContext(windowText, anchorMidRelative) {
  ...
  const VALUATION_TERMS = [
    ...
    // A3.8.58: Equity value variants
    /\bequity value\b/gi, /\bequity valuation\b/gi, /\bequity-value\b/gi, /\bmarket cap\b/gi, /\bmarket capitalization\b/gi
  ];
...
/**
 * A3.8.58: Classify currency by binding patterns (binding-first, non-arbitrary)
 * Attempts to find explicit label-to-money bindings before using distance scoring
 */
function classifyCurrencyByBindingPatterns(fullText, sourceSpan, options = {}) {
  const { windowChars = 120 } = options;
  
  if (!fullText || typeof fullText !== "string" || !sourceSpan || 
      sourceSpan.start === null || sourceSpan.end === null ||
      sourceSpan.start < 0 || sourceSpan.end > fullText.length || sourceSpan.start >= sourceSpan.end) {
    return { family: null, subtype: null, reason: "invalid_input" };
  }
  
  // Construct local snippet around the span
  const spanMid = Math.floor((sourceSpan.start + sourceSpan.end) / 2);
  const windowStart = Math.max(0, spanMid - windowChars);
  const windowEnd = Math.min(fullText.length, spanMid + windowChars);
  const localSnippet = fullText.substring(windowStart, windowEnd);
  const moneyText = fullText.substring(sourceSpan.start, sourceSpan.end);
  
  // Normalize money text for pattern matching (remove currency symbols, normalize spacing)
  const moneyPattern = moneyText.replace(/[$,]/g, "").trim();
  
  // Binding patterns in priority order (most specific first)
  // Pattern format: [priority, family, subtype, regex, patternName]
  const bindingPatterns = [
    // VALUATION - Enterprise Value (highest priority)
    [1, "VALUATION", "valuation_enterprise_value", 
     /\b(?:enterprise\s+value|enterprise\s+valuation|ev)\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "enterprise_value"],
    [1, "VALUATION", "valuation_enterprise_value",
     /\$[\d,]+(?:\.\d+)?\s+(?:enterprise\s+value|enterprise\s+valuation|ev)\b/i, "enterprise_value_reverse"],
    [1, "VALUATION", "valuation_enterprise_value",
     /(?:at\s+(?:a|an)\s+)?(?:enterprise\s+value|enterprise\s+valuation|ev)\s+of\s+\$[\d,]+(?:\.\d+)?/i, "enterprise_value_at"],
    [1, "VALUATION", "valuation_enterprise_value",
     /(?:implying\s+(?:a|an)\s+)?(?:enterprise\s+value|enterprise\s+valuation|ev)\s+of\s+\$[\d,]+(?:\.\d+)?/i, "enterprise_value_implying"],
    [1, "VALUATION", "valuation_enterprise_value",
     /(?:enterprise\s+value|enterprise\s+valuation|ev)[:\s]+\$[\d,]+(?:\.\d+)?/i, "enterprise_value_colon"],
    
    // VALUATION - Equity Value (A3.8.58)
    [2, "VALUATION", "valuation_equity_value",
     /\b(?:equity\s+value|equity\s+valuation|equity-value|market\s+cap(?:italization)?)\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "equity_value"],
    [2, "VALUATION", "valuation_equity_value",
     /\$[\d,]+(?:\.\d+)?\s+(?:equity\s+value|equity\s+valuation|equity-value|market\s+cap(?:italization)?)\b/i, "equity_value_reverse"],
    [2, "VALUATION", "valuation_equity_value",
     /(?:at\s+(?:a|an)\s+)?(?:equity\s+value|equity\s+valuation|equity-value|market\s+cap(?:italization)?)\s+of\s+\$[\d,]+(?:\.\d+)?/i, "equity_value_at"],
    [2, "VALUATION", "valuation_equity_value",
     /(?:implying\s+(?:a|an)\s+)?(?:equity\s+value|equity\s+valuation|equity-value|market\s+cap(?:italization)?)\s+of\s+\$[\d,]+(?:\.\d+)?/i, "equity_value_implying"],
    [2, "VALUATION", "valuation_equity_value",
     /(?:equity\s+value|equity\s+valuation|equity-value|market\s+cap(?:italization)?)[:\s]+\$[\d,]+(?:\.\d+)?/i, "equity_value_colon"],
    
    // VALUATION - Pre-money
    [3, "VALUATION", "valuation_pre_money",
     /\b(?:pre-?money|premoney|pre\s+money)\s+(?:valuation\s+)?(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "pre_money"],
    [3, "VALUATION", "valuation_pre_money",
     /\$[\d,]+(?:\.\d+)?\s+(?:pre-?money|premoney|pre\s+money)\s+(?:valuation\s+)?/i, "pre_money_reverse"],
    [3, "VALUATION", "valuation_pre_money",
     /(?:at\s+(?:a|an)\s+)?(?:pre-?money|premoney|pre\s+money)\s+(?:valuation\s+)?of\s+\$[\d,]+(?:\.\d+)?/i, "pre_money_at"],
    
    // VALUATION - Post-money
    [4, "VALUATION", "valuation_post_money",
     /\b(?:post-?money|postmoney|post\s+money)\s+(?:valuation\s+)?(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "post_money"],
    [4, "VALUATION", "valuation_post_money",
     /\$[\d,]+(?:\.\d+)?\s+(?:post-?money|postmoney|post\s+money)\s+(?:valuation\s+)?/i, "post_money_reverse"],
    [4, "VALUATION", "valuation_post_money",
     /(?:at\s+(?:a|an)\s+)?(?:post-?money|postmoney|post\s+money)\s+(?:valuation\s+)?of\s+\$[\d,]+(?:\.\d+)?/i, "post_money_at"],
    
    // VALUATION - Generic (A3.8.59: Add "$X valuation" form)
    [5, "VALUATION", "valuation_amount",
     /\bvaluation\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "valuation_generic"],
    [5, "VALUATION", "valuation_amount",
     /\$[\d,]+(?:\.\d+)?\s+valuation\b/i, "valuation_generic_reverse"],
    [5, "VALUATION", "valuation_amount",
     /(?:at\s+(?:a|an)\s+)?valuation\s+of\s+\$[\d,]+(?:\.\d+)?/i, "valuation_generic_at"],
    // A3.8.59: Generic "$X (million|m|bn|b)? valuation" form
    [5, "VALUATION", "valuation_amount",
     /\$[\d,]+(?:\.\d+)?\s*(?:million|m|bn|b)?\s+valuation\b/i, "valuation_generic_dollar_first"],
    [5, "VALUATION", "valuation_amount",
     /(?:at\s+(?:a|an)\s+)?\$[\d,]+(?:\.\d+)?\s*(?:million|m|bn|b)?\s+valuation\b/i, "valuation_generic_dollar_at"],
    
    // INVESTMENT
    [6, "INVESTMENT", "investment_amount",
     /\b(?:investment|invest|check|commitment|primary\s+investment)\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "investment"],
    [6, "INVESTMENT", "investment_amount",
     /\$[\d,]+(?:\.\d+)?\s+(?:investment|invest|check|commitment|primary\s+investment)\b/i, "investment_reverse"],
    [6, "INVESTMENT", "investment_amount",
     /(?:at\s+(?:a|an)\s+)?(?:investment|invest|check|commitment|primary\s+investment)\s+of\s+\$[\d,]+(?:\.\d+)?/i, "investment_at"],
    
    // METRIC - ARR
    [7, "METRIC", "metric_amount",
     /\b(?:arr|annual\s+recurring\s+revenue)\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "arr"],
    [7, "METRIC", "metric_amount",
     /\$[\d,]+(?:\.\d+)?\s+(?:arr|annual\s+recurring\s+revenue)\b/i, "arr_reverse"],
    
    // METRIC - MRR
    [7, "METRIC", "metric_amount",
     /\b(?:mrr|monthly\s+recurring\s+revenue)\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "mrr"],
    [7, "METRIC", "metric_amount",
     /\$[\d,]+(?:\.\d+)?\s+(?:mrr|monthly\s+recurring\s+revenue)\b/i, "mrr_reverse"],
    
    // METRIC - Annualized Revenue
    [7, "METRIC", "metric_amount",
     /\bannualized\s+revenue\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "annualized_revenue"],
    [7, "METRIC", "metric_amount",
     /\$[\d,]+(?:\.\d+)?\s+annualized\s+revenue\b/i, "annualized_revenue_reverse"],
    
    // METRIC - Revenue
    [7, "METRIC", "metric_amount",
     /\brevenue\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "revenue"],
    [7, "METRIC", "metric_amount",
     /\$[\d,]+(?:\.\d+)?\s+revenue\b/i, "revenue_reverse"],
    
    // METRIC - EBITDA (with variants)
    [7, "METRIC", "metric_amount",
     /\b(?:adjusted|pro\s+forma|adj\.?)\s+ebitda\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "ebitda_variant"],
    [7, "METRIC", "metric_amount",
     /\bebitda\s+(?:of\s+)?\$[\d,]+(?:\.\d+)?/i, "ebitda"],
    [7, "METRIC", "metric_amount",
     /\$[\d,]+(?:\.\d+)?\s+(?:(?:adjusted|pro\s+forma|adj\.?)\s+)?ebitda\b/i, "ebitda_reverse"],
  ];
  
  // Try patterns in priority order (lower number = higher priority)
  let bestMatch = null;
  let bestPriority = Infinity;
  
  for (const [priority, family, subtype, pattern, patternName] of bindingPatterns) {
    pattern.lastIndex = 0; // Reset regex
    const match = pattern.exec(localSnippet);
    if (match && priority < bestPriority) {
      // Verify the matched money amount is close to our span
      const matchIndex = windowStart + match.index;
      const spanDistance = Math.abs(matchIndex - spanMid);
      
      // Only accept if match is within reasonable distance of the span
      if (spanDistance <= windowChars * 2) {
        bestMatch = { family, subtype, patternName, match };
  ...
  if (bestMatch) {
    return {
      ...
      reason: `binding_pattern_${bestMatch.patternName}`
    };
  }
  
  return { family: null, subtype: null, reason: "no_binding_found" };
}
