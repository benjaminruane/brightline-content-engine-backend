...
export function canonicalizeClaims(rawClaims, ctx) {
  ...
  finalCanonical = finalCanonical.map(claim => sanitizeCanonicalUnits(claim, statementIndex, log, unitFixLogCounter));
  ...
  if (hasNumeric && (hasCurrency || hasPercent)) {
    ...
    if (hasCurrency && statementText) {
      ...
      if (usdMatch) {
        ...
        // A3.8.39: Track token positions for distance-based priority
        const invHits = [];
        const valHits = [];
        const usdIdx = (claimIndexInStatement >= 0 ? claimIndexInStatement : 0) + matchIndex - contextStart;
        ...
        // A3.8.39: Compute distances for priority override
        let nearestInvDist = invHits.length > 0 ? Math.min(...invHits.map(hit => Math.abs(hit - usdIdx))) : Infinity;
        let nearestValDist = valHits.length > 0 ? Math.min(...valHits.map(hit => Math.abs(hit - usdIdx))) : Infinity;
        ...
        const isStrongValToken = valHits.some(hit => {
        ...
        if (shouldPreferInvestment && investmentScore > 0) {
          if (log) {
            ...
            log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=investment_amount valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} nearestInvDist=${nearestInvDist} nearestValDist=${nearestValDist} usdIdx=${usdIdx} decisionReason=investment_near_usd ctxPreview="${ctxPreview}..."`);
          }
        ...
        if (valuationScore > 0 && !shouldPreferInvestment) {
          ...
          if (/\b(pre-?money|premoney|pre\s+money)\b/i.test(contextWindow)) {
            if (log) {
              const valueDisplay = usdMatch[1];
              const ctxPreview = contextWindow.substring(0, 80);
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_pre_money valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} nearestInvDist=${nearestInvDist} nearestValDist=${nearestValDist} usdIdx=${usdIdx} decisionReason=valuation ctxPreview="${ctxPreview}..."`);
            }
          ...
          if (/\b(post-?money|postmoney|post\s+money)\b/i.test(contextWindow)) {
            if (log) {
              const valueDisplay = usdMatch[1];
              const ctxPreview = contextWindow.substring(0, 80);
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_post_money valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} nearestInvDist=${nearestInvDist} nearestValDist=${nearestValDist} usdIdx=${usdIdx} decisionReason=valuation ctxPreview="${ctxPreview}..."`);
            }
          ...
          if (/\b(enterprise\s+value|EV)\b(?!\w)/i.test(contextWindow)) {
            if (log) {
              const valueDisplay = usdMatch[1];
              const ctxPreview = contextWindow.substring(0, 80);
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_enterprise_value valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} nearestInvDist=${nearestInvDist} nearestValDist=${nearestValDist} usdIdx=${usdIdx} decisionReason=valuation ctxPreview="${ctxPreview}..."`);
            }
        ...
        if (metricScore > 0) {
          if (log) {
            const valueDisplay = usdMatch[1];
            const ctxPreview = contextWindow.substring(0, 80);
            log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=metric_amount valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} nearestInvDist=${nearestInvDist} nearestValDist=${nearestValDist} usdIdx=${usdIdx} decisionReason=metric ctxPreview="${ctxPreview}..."`);
          }
        ...
        if (investmentScore > 0) {
          if (log) {
            const valueDisplay = usdMatch[1];
            const ctxPreview = contextWindow.substring(0, 80);
            log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=investment_amount valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} nearestInvDist=${nearestInvDist} nearestValDist=${nearestValDist} usdIdx=${usdIdx} decisionReason=investment ctxPreview="${ctxPreview}..."`);
          }
          return "investment_amount";
        }
