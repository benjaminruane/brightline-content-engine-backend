  ...
  for (const norm of normalized) {
    ...
    if (selectionMode) {
      ...
      if (hasUsdAnchor && matchesStandaloneUsd && !hasTruncationMarker) {
        ...
        if (runId && reqSig && log) {
          log(`[DIAG][A3.8.45][BARE_USD_ALLOW] anchor=${anchor} preview="${preview}"`);
        }
        // Ensure type is not other_qualitative - default to metric_amount if still qualitative
        if (norm.inferredType === "other_qualitative" || !norm.inferredType) {
          norm.inferredType = "metric_amount";
        }
...
function normalizeRawClaim(raw, statementText, knownEntities, idx, rawClaims = null, log = null, logCounter = { count: 0 }, statementIndex = 0, selectionMode = false, runId = null, reqSig = null) {
  ...
  let inferredType = expandedType || inferCanonicalType(claimText, numeric, currency, percent, anchorFamily, roleKeywords, statementText, idx, log);
