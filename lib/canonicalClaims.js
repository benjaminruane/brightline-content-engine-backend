  ...
  if (hasNumeric && (hasCurrency || hasPercent)) {
    ...
    if (hasCurrency && statementText) {
      ...
      if (usdMatch) {
        ...
        for (const token of valuationTokens) {
        ...
        for (const token of investmentTokens) {
          const regex = new RegExp(`\\b${token.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, "i");
          if (regex.test(contextWindow)) {
            investmentScore++;
          }
        ...
        if (valuationScore > 0) {
          ...
          if (/\b(pre-?money|premoney|pre\s+money)\b/i.test(contextWindow)) {
            if (log) {
              ...
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_pre_money valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} ctxPreview="${ctxPreview}..."`);
            }
          ...
          if (/\b(post-?money|postmoney|post\s+money)\b/i.test(contextWindow)) {
            if (log) {
              const valueDisplay = usdMatch[1];
              const ctxPreview = contextWindow.substring(0, 80);
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_post_money valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} ctxPreview="${ctxPreview}..."`);
            }
          ...
          if (/\b(enterprise\s+value|EV)\b(?!\w)/i.test(contextWindow)) {
            if (log) {
              const valueDisplay = usdMatch[1];
              const ctxPreview = contextWindow.substring(0, 80);
              log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=valuation_enterprise_value valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} ctxPreview="${ctxPreview}..."`);
            }
        ...
        if (metricScore > 0) {
          if (log) {
            const valueDisplay = usdMatch[1];
            const ctxPreview = contextWindow.substring(0, 80);
            log(`[CANON][USD_TYPE] idx=${statementIndex} value=${valueDisplay} decided=metric_amount valScore=${valuationScore} metricScore=${metricScore} invScore=${investmentScore} ctxPreview="${ctxPreview}..."`);
          }
          return "metric_amount";
        }
        
        // A3.8.37: c) Else if invScore > 0 → investment_amount
        // Only classify as investment_amount if investment context present and valuation/metric absent
        if (investmentScore > 0 && valuationScore === 0 && metricScore === 0) {
          // Will fall through to investment_amount classification below
        } else if (valuationScore === 0 && investmentScore === 0 && metricScore === 0) {
