...
export function canonicalizeClaims(rawClaims, ctx) {
  ...
  const normalized = rawClaims.map((raw, idx) => normalizeRawClaim(raw, statementText, knownEntities, idx, rawClaims, log, entityDropLogCounter));
  ...
  for (const [groupKey, groupClaims] of groups.entries()) {
    const canonical = mergeGroupIntoCanonical(groupClaims, statementText, computedSelectionHash, selectionMode, groupKey, log, invariantLogCounter);
    if (canonical) {
 ...
 */
function normalizeRawClaim(raw, statementText, knownEntities, idx, rawClaims = null, log = null, logCounter = { count: 0 }) {
 ...
 */
function mergeGroupIntoCanonical(groupClaims, statementText, selectionHash, selectionMode, groupKey, log = null, logCounter = { count: 0 }) {
