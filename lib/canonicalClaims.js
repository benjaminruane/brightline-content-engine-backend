...
function normalizeRawClaim(raw, statementText, knownEntities, idx, rawClaims = null, log = null, logCounter = { count: 0 }, statementIndex = 0, selectionMode = false, runId = null, reqSig = null) {
  ...
  if (anchorFamily) {
    const normalizedAnchor = normalizeAnyAnchor(anchorFamily);
    if (normalizedAnchor && normalizedAnchor.startsWith("usd_")) {
      preferDollar = true;
    } else if (normalizedAnchor && normalizedAnchor.startsWith("pct_")) {
      preferPercent = true;
    }
  }
