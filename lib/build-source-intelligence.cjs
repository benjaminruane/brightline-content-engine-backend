...
function buildSourceIntelligence(stmt, refById) {
  ...
  try {
    ...
    // A4.NC2.1: Per-source numeric match meta (additive; only when stmt.meta.numericSupportBySourceId exists)
    const numericSupportBySourceId = stmt.meta != null && typeof stmt.meta.numericSupportBySourceId === "object" && stmt.meta.numericSupportBySourceId !== null
      ? stmt.meta.numericSupportBySourceId
      : null;
    const hasNumericMeta = numericSupportBySourceId !== null;
    ...
    const perSource = perSourceRaw.map((s) => {
      ...
      if (hasNumericMeta) {
        const sid = s.sourceId != null ? String(s.sourceId) : "";
        const ns = numericSupportBySourceId[sid];
        const matchKind = (ns && typeof ns.matchKind === "string") ? ns.matchKind : "none";
        out.numericSupport = {
          matchKind,
          statementValueNormalized: (ns && typeof ns.statementValueNormalized === "number" && Number.isFinite(ns.statementValueNormalized)) ? ns.statementValueNormalized : null,
          sourceValueNormalized: (ns && typeof ns.sourceValueNormalized === "number" && Number.isFinite(ns.sourceValueNormalized)) ? ns.sourceValueNormalized : null,
        };
      }
      return out;
    });
    ...
    // A4.NC2.2: Statement-level numeric confidence hint (meta only; set on stmt.meta when at least one numeric match)
    if (hasNumericMeta && Array.isArray(perSource) && perSource.length > 0) {
      const matchKinds = perSource.map((p) => (p.numericSupport && p.numericSupport.matchKind) ? p.numericSupport.matchKind : "none");
      const exactOrFormat = (k) => k === "exact" || k === "format_equivalent";
      const rounded = (k) => k === "rounded_equivalent_hedged" || k === "rounded_equivalent_source_hedged";
      const withMatch = matchKinds.filter((k) => k !== "none");
      if (withMatch.length > 0) {
        const allExactOrFormat = withMatch.every(exactOrFormat);
        const anyRounded = withMatch.some(rounded);
        const allRounded = withMatch.every(rounded);
        let numericConfidenceHint;
        if (allRounded && !withMatch.some(exactOrFormat)) {
          numericConfidenceHint = "approx";
        } else if (anyRounded && withMatch.some(exactOrFormat)) {
          numericConfidenceHint = "medium";
        } else if (allExactOrFormat) {
          numericConfidenceHint = "high";
        } else {
          numericConfidenceHint = "approx";
        }
        try {
          if (!stmt.meta) stmt.meta = {};
          stmt.meta.numericConfidenceHint = numericConfidenceHint;
        } catch (_) { /* NC2.3: never throw */ }

        // P3.W1: Numeric evidence quality → fragility inputs (meta only; input signals for Phase 3)
        try {
          const exactCount = withMatch.filter(exactOrFormat).length;
          const approxCount = withMatch.filter(rounded).length;
          const total = exactCount + approxCount;
          if (total > 0) {
            let qualityBand;
            if (approxCount === 0) qualityBand = "strong";
            else if (exactCount > 0) qualityBand = "mixed";
            else qualityBand = "approx_heavy";
            if (!stmt.meta.evidenceJudgementInputs) stmt.meta.evidenceJudgementInputs = {};
            stmt.meta.evidenceJudgementInputs.numericEvidenceQuality = {
              qualityBand,
              approxSourceRatio: approxCount / total,
              exactSourceRatio: exactCount / total,
              sourceCount: total,
            };
          }
        } catch (_) { /* P3.W1: never throw */ }
      }
    }
