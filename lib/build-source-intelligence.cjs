...
/** Press release: corporate_disclosure, distance near. */
...
/** independenceContribution per source: independent, related, neutral. unknown role -> neutral. */
function classifyIndependenceForSource(sourceIndex, perSourceList) {
  ...
  const domains = perSourceList.map((x) => x.domain).filter(Boolean);
  const uniqueDomains = new Set(domains);
  if (uniqueDomains.size >= 2) return "independent";
  if (uniqueDomains.size === 1 && domains.length >= 2) return "related";
  return "neutral";
}
