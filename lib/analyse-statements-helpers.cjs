...
// A3.9.57: Gate helper-load [DIAG] behind BRIGHTLINE_DIAG_VERBOSE (quiet-by-default)
const __diagVerboseEnv = (process.env.BRIGHTLINE_DIAG_VERBOSE === "1");
const logDiag = (...args) => { if (__diagVerboseEnv) console.log(...args); };

// A3.8.192: Module load marker (gated)
logDiag("[DIAG][A3.8.192][HELPERS_CJS_LOADED]");
...
// A3.9.22/A3.9.23: Local normalizeNumber — MUST exist at module scope; self-contained; never throws.
...
// A3.9.23: Baseline (no-op) implementations for REQUIRED_POST_HOOKS — never throw; pass-through or minimal shape.
function baselineDetectAnchorAmbiguity() {
  return { isAmbiguous: false, anchorType: null, values: [] };
}
function baselineEnforceCorpusVerificationBeforeAbsence(statements) {
  return Array.isArray(statements) ? statements : [];
}
function baselineComputeExtractionQuality() {
  return { quality: "ok", reasons: [] };
}
function baselineFixAnchorFactReasons(statements) {
  return Array.isArray(statements) ? statements : [];
}
function baselineApplyFinalPostCheck(statements) {
  return Array.isArray(statements) ? statements : [];
}
function baselineApplyNonAnchorCalibration(statements) {
  return Array.isArray(statements) ? statements : [];
}
function baselineApplyParaphraseTolerance(statements) {
  return Array.isArray(statements) ? statements : [];
}
...
function computeExtractionQuality(statements, extractionCandidates, rejectedCount = 0, fallbackCount = 0, incompleteNumericFragmentCount = 0, recombinedCount = 0, fragmentDropped = 0, fragmentMerged = 0, dealDedupDropped = 0, qualityPatch = {}, runId = null, reqSig = null) {
  ...
  for (const stmt of statements) {
    ...
    if (endsWithLetter && !hasTerminalPunct) {
      ...
      if (isVeryShortFragment || isSuspiciouslyShort) {
        hasTruncation = true;
        logDiag(`[DIAG][QUALITY] truncation detected: textPreview="${text.substring(0, 60)}..." lastWord="${lastWord}"`);
      }
    }
  ...
  logDiag(`[DIAG][QUALITY] extractionQuality=${quality} reasons=${JSON.stringify(reasons)}`);
...
// A3.9.23: Safe wrappers — try real implementation, on throw return baseline (never throw).
function safeDetectAnchorAmbiguity(statementText, uploadedDocs) {
  try { return detectAnchorAmbiguity(statementText, uploadedDocs); } catch (_) { return baselineDetectAnchorAmbiguity(); }
}
function safeEnforceCorpusVerificationBeforeAbsence(statements, uploadedSources, unifiedReferences, runId, reqSig, corpusVerification, selectionMode) {
  try { return enforceCorpusVerificationBeforeAbsence(statements, uploadedSources, unifiedReferences, runId, reqSig, corpusVerification, selectionMode); } catch (_) { return baselineEnforceCorpusVerificationBeforeAbsence(statements); }
}
function safeComputeExtractionQuality(statements, extractionCandidates, rejectedCount, fallbackCount, incompleteNumericFragmentCount, recombinedCount, fragmentDropped, fragmentMerged, dealDedupDropped, qualityPatch, runId, reqSig) {
  try { return computeExtractionQuality(statements, extractionCandidates, rejectedCount, fallbackCount, incompleteNumericFragmentCount, recombinedCount, fragmentDropped, fragmentMerged, dealDedupDropped, qualityPatch, runId, reqSig); } catch (_) { return baselineComputeExtractionQuality(); }
}
function safeFixAnchorFactReasons(statements, unifiedReferences) {
  try { return fixAnchorFactReasons(statements, unifiedReferences); } catch (_) { return baselineFixAnchorFactReasons(statements); }
}
function safeApplyFinalPostCheck(statements, unifiedReferences) {
  try { return applyFinalPostCheck(statements, unifiedReferences); } catch (_) { return baselineApplyFinalPostCheck(statements); }
}
function safeApplyNonAnchorCalibration(statements) {
  try { return applyNonAnchorCalibration(statements); } catch (_) { return baselineApplyNonAnchorCalibration(statements); }
}
function safeApplyParaphraseTolerance(statements, unifiedReferences) {
  try { return applyParaphraseTolerance(statements, unifiedReferences); } catch (_) { return baselineApplyParaphraseTolerance(statements); }
}

// A3.8.192: CommonJS exports (no export keyword)
// A3.9.23: Wrap exports in try/catch so module load never throws; log fallback if needed.
let A3923_EXPORTS = null;
try {
  A3923_EXPORTS = {
    setImplUtils,
    normalizeTextForSearch,
    normalizeNumber,
    detectAnchorAmbiguity: safeDetectAnchorAmbiguity,
    enforceCorpusVerificationBeforeAbsence: safeEnforceCorpusVerificationBeforeAbsence,
    computeExtractionQuality: safeComputeExtractionQuality,
    fixAnchorFactReasons: safeFixAnchorFactReasons,
    applyFinalPostCheck: safeApplyFinalPostCheck,
    applyNonAnchorCalibration: safeApplyNonAnchorCalibration,
    applyParaphraseTolerance: safeApplyParaphraseTolerance,
    enforceAnchorCitationsAndAmbiguity,
    backfillCitations,
    __A3_8_192_HELPERS_OK: true
  };
  module.exports = A3923_EXPORTS;
  logDiag("[DIAG][A3.9.23][HELPERS_EXPORTS_READY]", {
    normalizeNumber: typeof module.exports.normalizeNumber,
    normalizeTextForSearch: typeof module.exports.normalizeTextForSearch,
    detectAnchorAmbiguity: typeof module.exports.detectAnchorAmbiguity,
    enforceCorpusVerificationBeforeAbsence: typeof module.exports.enforceCorpusVerificationBeforeAbsence,
    computeExtractionQuality: typeof module.exports.computeExtractionQuality,
    fixAnchorFactReasons: typeof module.exports.fixAnchorFactReasons,
    applyFinalPostCheck: typeof module.exports.applyFinalPostCheck,
    applyNonAnchorCalibration: typeof module.exports.applyNonAnchorCalibration,
    applyParaphraseTolerance: typeof module.exports.applyParaphraseTolerance
  });
} catch (e) {
  const errorName = e?.name ?? "Error";
  const errorMessage = e?.message ?? String(e);
  logDiag("[DIAG][A3.9.23][HELPERS_CJS_INIT_FALLBACK]", { errorName, errorMessage });
  module.exports = {
    setImplUtils,
    normalizeTextForSearch,
    normalizeNumber,
    detectAnchorAmbiguity: baselineDetectAnchorAmbiguity,
    enforceCorpusVerificationBeforeAbsence: baselineEnforceCorpusVerificationBeforeAbsence,
    computeExtractionQuality: baselineComputeExtractionQuality,
    fixAnchorFactReasons: baselineFixAnchorFactReasons,
    applyFinalPostCheck: baselineApplyFinalPostCheck,
    applyNonAnchorCalibration: baselineApplyNonAnchorCalibration,
    applyParaphraseTolerance: baselineApplyParaphraseTolerance,
    __A3_8_192_HELPERS_OK: true
  };
  logDiag("[DIAG][A3.9.23][HELPERS_EXPORTS_READY]", {
    normalizeNumber: typeof module.exports.normalizeNumber,
