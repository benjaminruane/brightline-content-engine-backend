...
const VALUATION_TYPE_PREFIX = "valuation_";
const MAX_TELEMETRY_WARNINGS = 12;

function countValuationCuesInBlob(blob) {
  if (typeof blob !== "string") return 0;
  const lower = blob.toLowerCase();
  const re = /pre[-\s]?money|premoney|post[-\s]?money|postmoney|valuation|valued\s+at/g;
  let count = 0;
  while (re.exec(lower) !== null) count++;
  return count;
}

function hasLocalValuationCue(entry) {
  const cues = entry.cuesUsed || [];
  const valuationCues = cues.filter((c) => c.type && c.type.startsWith("valuation"));
  if (valuationCues.length === 0) return false;
  const hasClose = valuationCues.some((c) => (c.distanceTokens != null ? c.distanceTokens <= 18 : false));
  const hasSentence = valuationCues.some((c) => c.location === "sentence");
  return hasClose || hasSentence;
}

/**
 * A3.19.6: Apply DealTerms typing telemetry and regression guards.
 * @param {Array} valueTyping - stmt.meta.corpus.dealTerms.valueTyping (array of entries with typedAs, confidence, cuesUsed, contextSnippet)
 * @param {string} blob - dtText
 * @returns {{ warnings: string[], warningsCount: number }}
 */
export function applyDealTermsTypingTelemetryAndGuards(valueTyping, blob) {
  const warnings = [];
  if (!Array.isArray(valueTyping)) return { warnings, warningsCount: 0 };

  const valuationTyped = valueTyping.filter((e) => e.typedAs && e.typedAs.startsWith(VALUATION_TYPE_PREFIX));
  const valuationCount = valuationTyped.length;
  const totalCount = valueTyping.length;

  for (const entry of valueTyping) {
    const typedAs = entry.typedAs || "";
    if (!typedAs.startsWith(VALUATION_TYPE_PREFIX)) continue;
    const cues = entry.cuesUsed || [];
    const valuationCues = cues.filter((c) => c.type && c.type.startsWith("valuation"));
    const minDist = valuationCues.length > 0
      ? Math.min(...valuationCues.map((c) => (typeof c.distanceTokens === "number" ? c.distanceTokens : 999)))
      : 999;
    const onlyFallback = valuationCues.length > 0 && valuationCues.every((c) => c.location !== "sentence");
    if (minDist > 18 || onlyFallback) {
      if (!warnings.includes("valuation_type_without_local_valuation_cue")) {
        warnings.push("valuation_type_without_local_valuation_cue");
      }
    }
  }

  const valuationCueCountInBlob = countValuationCuesInBlob(blob);
  if (valuationCueCountInBlob === 1 && valuationCount >= 2) {
    const anyWithoutLocal = valuationTyped.some((e) => !hasLocalValuationCue(e));
    if (anyWithoutLocal && !warnings.includes("single_valuation_cue_dominating_multiple_values")) {
      warnings.push("single_valuation_cue_dominating_multiple_values");
    }
  }

  for (const entry of valueTyping) {
    const typedAs = entry.typedAs || "";
    if ((typedAs === "valuation_pre_money" || typedAs === "valuation_post_money") && entry.confidence === "low") {
      const snippet = (entry.contextSnippet || "").toLowerCase();
      if (snippet.includes("pre-money") && snippet.includes("post-money") && !warnings.includes("premoney_postmoney_ambiguous_low_confidence")) {
        warnings.push("premoney_postmoney_ambiguous_low_confidence");
      }
    }
  }

  if (totalCount >= 3 && valuationCount / totalCount > 0.7) {
    const hasInvestmentCueNearValue = valueTyping.some((entry) => {
      const cues = entry.cuesUsed || [];
      return cues.some((c) => (c.type === "investment_amount" || c.type === "raise_amount") && (c.distanceTokens == null || c.distanceTokens <= 18));
    });
    if (hasInvestmentCueNearValue && !warnings.includes("valuation_overassignment_suspected")) {
      warnings.push("valuation_overassignment_suspected");
    }
  }

  return { warnings, warningsCount: warnings.length };
}

export function createDealTermsTelemetryAccumulator() {
  return {
    methodCounts: { value_level_v1: 0, baseline_only: 0, fallback_used: 0 },
    typeCounts: {},
...
export function incrementDealTermsTelemetry(accum, method, typedAs, confidence, fallbackUsed) {
  if (!accum) return;
  if (method === "value_level_v1") accum.methodCounts.value_level_v1++;
  else if (method === "baseline_only") accum.methodCounts.baseline_only++;
  if (fallbackUsed) accum.methodCounts.fallback_used++;
  const type = typedAs && String(typedAs).trim() ? String(typedAs).trim() : "unknown";
  accum.typeCounts[type] = (accum.typeCounts[type] || 0) + 1;
  const conf = confidence && String(confidence).trim() ? String(confidence).trim() : "low";
  if (accum.confidenceCounts[conf] != null) accum.confidenceCounts[conf]++;
}

export function mergeDealTermsTelemetryWarnings(accum, warnings, maxWarnings = MAX_TELEMETRY_WARNINGS) {
  if (!accum || !Array.isArray(warnings)) return;
  for (const w of warnings) {
    if (accum.warnings.length >= maxWarnings) break;
    if (!accum.warnings.includes(w)) accum.warnings.push(w);
  }
  accum.warningsCount += warnings.length;
}
