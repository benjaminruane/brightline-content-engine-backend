...
/**
 * Align a marker to the original using the nearest LCS-aligned tokens fully
 * left and fully right of the span. The original region is every original
 * token strictly between those anchors. CUT honesty uses this same window:
 * adjacent deleted words sit in that region, so a true clause-cut is CHANGED
 * even when the remnant itself is byte-identical to a substring of the original.
 *
 ...
 * @param {number} end
 * @returns {{
 *   spanStatus: "CHANGED"|"UNCHANGED",
 *   origRegion: Array<{ text: string, start: number, end: number }>,
 *   revSpan: Array<{ text: string, start: number, end: number }>,
 *   origRegionText: string,
 *   revSpanText: string,
 * }}
 */
export function markerSpanAlignment(original, revised, start, end) {
  const s = Number.isFinite(start) ? start : 0;
  const e = Number.isFinite(end) ? end : s;
  if (e <= s) {
    return {
      spanStatus: SPAN_UNCHANGED,
      origRegion: [],
      revSpan: [],
      origRegionText: "",
      revSpanText: "",
  ...
  const spanStatus =
    wordSequenceKey(origRegion) === wordSequenceKey(revSpan) ? SPAN_UNCHANGED : SPAN_CHANGED;
  return {
    spanStatus,
    origRegion,
    revSpan,
    origRegionText: origRegion.map((t) => t.text).join(" "),
    revSpanText: revSpan.map((t) => t.text).join(" "),
  };
}
...
export function markerSpanStatus(original, revised, start, end) {
  return markerSpanAlignment(original, revised, start, end).spanStatus;
}
