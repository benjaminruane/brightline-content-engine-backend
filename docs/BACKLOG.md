...
| B26.2.2 | **QUEUE — Constructive feedback readability calibration.** (a) **Quote-length discipline** — craft and card prompts quote long offending sentences verbatim (40+ words, 2–4 lines), tiring to read. Cap quoted spans to a short identifying fragment (~8–10 words) with "opening … closing" ellipsis for long sentences. Prompt-only, both craft and card prompts. (b) **Figure-dedupe** — craft coherence (dimension 6) and the card point on the same figure overlap (e.g. $48-vs-$50 internal clash AND source-reconciliation on the same number). When craft has already flagged a figure as internally contradictory, the card point on that figure should focus on source reconciliation without re-explaining the internal clash. Light assembly/prompt dedupe — do NOT suppress either signal entirely. | B26.2 testing 2026-06-30 | M | Post-MVP | |
| B35 | **Constructive Feedback craft pass is output-type-blind.** The craft pass reads only draft text; craft norms (opening strength, register expectations, lede importance) should flex by output type — e.g. LinkedIn post vs investor letter vs press release. Cousin to **R6.12** document-type-aware voice/register (same blind spot as the editorial layer; see ROADMAP **R6.12**). Defer; scope alongside or after R6.12. | B26.2 testing 2026-06-30 | M | Post-MVP | Defer |
