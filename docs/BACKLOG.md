...
| B36 | **Convert `tests/constructive-feedback.test.mjs` to a real Vitest suite** (`describe`/`test`/`expect`). It is currently an inline assertion script that prints its own PASS and reports 'no tests' under Vitest, so it does not run in `npm test` or any Vitest-based CI/batch run — failures are only caught when invoked directly with `node`. Wrap existing assertions (B26/B26.1/B26.2/B26.2.4 cases) in Vitest blocks so they run in the suite. | B26.2.4 testing 2026-07-05 | L | Post-MVP | |
