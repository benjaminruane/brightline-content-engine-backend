...
| B6  | Wire Visibility (Complete vs Public) into rebuild — visibility now affects QC behaviour via R4.3 rule filtering and prompt calibration; Writing-path review intent wired in R6.13.1 (`versionType` / `selectedTypes` → `requiredVersion` / `outputType`). Residual Tier B decisions (eventType) remain open — see **B28**. | R4.3 (`r4.3-visibility-wiring`), R6.13.1 (`r6.13.1-visibility-log-fix`) | |
