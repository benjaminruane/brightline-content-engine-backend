...
| Pr9 | **Interim: Suggest revised draft** — lighter prequel to Implement-changes sprint (**B9**). One-click **Suggest revised draft**: deterministic gather of all Review/Assess card concerns → single temp-0 LLM call → whole-draft holistic rewrite (NOT stitched per-card `suggestedRewrite`). Reuses **B26.2** reviewed-draft snapshot (`analysedDraftText`) so revision matches what was reviewed. Frontend: modal with clean revised draft + track-changes diff vs original (diff collapsed by default) + Copy. No accept/modify/reject workflow (that remains **B9**). **Boundary:** crosses Review Correctness Principle #7 (Review must not propose verbatim rewrites) deliberately — separate opt-in action, NOT baked into QC cards (same separation as Constructive Feedback vs Reviewer Assessment). | product scoping 2026-07-05 | M | Post-MVP | Deferred until after PG demo (**ROADMAP WSC1**). See ROADMAP open list **#16** / Implement-Changes Sprint. |
