...
| B37 | **R6.9.1 — Rhetorical / opinion non-claim handling (Stage 2).** LinkedIn adapt surfaced category error: rhetorical/opinion lines get `not_supported` / high-concern evidence verdicts. Distinguish verifiable claims from rhetorical/opinion in evidence layer; framing appropriateness stays editorial/compliance. Partial machinery works (bare URL not evidence-assessed). **Elevated H; do not touch pre-Monday.** See ROADMAP **Parked → R6.9.1**. | A10 LinkedIn adapt testing 2026-07-09 | H | Post-MVP | Stage 2 change |
| B38 | **API batching / caching (parked).** (a) Batch API (50% off, up to 24h latency) for offline diagnostic/regression sweeps only. (b) Prompt caching (50% off cached input) for live QC path. Do not apply batch latency to interactive Review. | post-A10 scoping 2026-07-09 | L | Post-MVP | Parked |
...
| Pr9 | **Interim: Suggest revised draft** — lighter prequel to Implement-changes sprint (**B9**). One-click **Suggest revised draft**: deterministic gather of all Review/Assess card concerns → single temp-0 LLM call → whole-draft holistic rewrite (NOT stitched per-card `suggestedRewrite`). Reuses **B26.2** reviewed-draft snapshot (`analysedDraftText`) so revision matches what was reviewed. Frontend: modal with clean revised draft + track-changes diff vs original (diff collapsed by default) + Copy. **Version History integration:** revised draft must be promotable to a version — not dead-ended in the modal. Add **Save as new version** committing via the existing version-commit path, parented to the pre-revision draft, with a provenance-marked auto label (e.g. "Revised from QC feedback") so the audit trail shows it derived from applying Review, not manual editing. No accept/modify/reject workflow (that remains **B9**). **Boundary:** crosses Review Correctness Principle #7 (Review must not propose verbatim rewrites) deliberately — separate opt-in action, NOT baked into QC cards (same separation as Constructive Feedback vs Reviewer Assessment). **Open decisions (post-demo):** (1) confirm provenance labelling in the change label; (2) new version is un-reviewed by design — user re-runs Review; do not auto-verify. Reuses existing versioning; no new machinery. | product scoping 2026-07-05 | M | Post-MVP | Post-demo (**WR1** shipped 2026-07-06). See ROADMAP open list **#16** / Implement-Changes Sprint. |
