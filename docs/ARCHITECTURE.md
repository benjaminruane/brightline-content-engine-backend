...
**One sentence = one QC card, always.** The legacy pipeline split sentences into subclaims and produced multiple cards per sentence. v4 does not. Stage 2 classifies the **whole sentence** against each source. Stage 3 is **conflict-wins** (existence reduction over those per-source classifications): any `conflicting` → conflicting; else any `confirmed` → confirmed; else any `partially_confirmed` → partial; else `not_supported`. `hasConflict` is independent — true if any whole-sentence source returned `conflicting`.
