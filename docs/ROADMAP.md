...
Last updated: 2026-06-25 (R6.6-docsync: source-public-state retains R6.6; document-type → R6.12)
...
### R6.11 — Editorial schema-fallback observability (closed 2026-06-03)

- **R6.11a — Schema salvage** (closed 2026-06-03, tag `r6.11a-editorial-schema-salvage`). Per-concern validation on combined v4 editorial+style output; drop invalid rows without aborting the whole response; reclassify concerns when `ruleId` is valid in the other category; retry attempt 2 appends structured correction feedback; canaries for dropped/reclassified concerns; empty `concern` verdict with zero survivors returns null (retry/fallback path).
- **R6.11b — not_reviewed visible state** (closed 2026-06-03, backend `r6.11b-not-reviewed-state`, frontend `v8.54.0-r6.11b-not-reviewed-state`). After two failed schema validations, fallback emits `editorialVerdict: "not_reviewed"` (not `clean`); Stage 7 assembly preserves `not_reviewed`; export flags treat it as an editorial signal; frontend shows amber "Needs manual review", workbench filters, and summary counts. Audit signal documented in `ai/AI_OPERATING_MANUAL.md`.

### R6.6 — Source-public-state awareness (in progress)

Capability **not closed** — residual verification, audit, and diag-cleanup remain open.

- **R6.6-diag** — diagnostic logging in `editorial-compliance-reviewer.mjs`. Shipped; logging intentionally left in place pending residual verification, then cleanup.
- **R6.6.1** — carry source `publicationState` through the diagnostic harness (`scripts/diagnostic/lib/sources.mjs`; fixtures 02 + 10 declared `published_external`). Shipped 2026-06-25. Evidence: F02 S0 `precise_confidential_detail` concernCount 1 → 0; F01 (undeclared) still `unknown`.
- **R6.6-audit** — silent-default wiring audit across pipeline entry paths. SPECCED, read-only, queued (not yet run).
...
| **R6.6** | **Source-public-state awareness (residual verification)** — see near-term work-stream **R6.6 — SOURCE-PUBLIC-STATE AWARENESS** | Medium | R6.4a evidence; R6.6-diag/R6.6.1 (2026-06-25) |
...
**Recommended order:** ~~commentary calibration~~ (**SHIPPED** `commentary-calibration`) → ~~editorial rule bug-fix pass~~ (**SHIPPED** R6.2e + R6.2f) → ~~editorial schema-fallback~~ (**SHIPPED** R6.11a + R6.11b) → then **R6.6 (source-public-state)** → **constructive feedback output** (after R6.6) → R7. Front-loads cheap, broad, low-risk wins; defers feature-sized work.

| Item | Summary | Priority |
|------|---------|----------|
| **COMMENTARY CALIBRATION (Stage 5 / reviewer-prompt)** | **SHIPPED 2026-06-03** — `commentary-calibration`. Prompt-only prose calibration: v4 evidence commentary (`lib/qc/pipeline-v4/prompts/stage5_v2.md`); v4 combined editorial+style system prompt (`buildEditorialStyleSystemPrompt` in `lib/qc/editorial-compliance-reviewer.mjs`); shared compliance system prompt (`buildComplianceSystemPrompt` in same file — **also applies to v3 compliance runs**). Scope: refer to the source not the excerpt; single integrated source-to-statement account; plain register (active/passive, hedging language, immediate context); anti-preachy framing; concise directions with short quoted trigger phrases preserved (R5.1 spans). **GUARDRAIL:** substantive framing, narrative_coherence, and materiality observation unchanged. **Residual (R6.2 minor follow-up):** the "leading" superlative flag is correct but not yet collapsed to the standardised single line (`"leading" is an unsubstantiated superlative; remove or substantiate.`) — prompt instructs it; model conformance may still vary. | Shipped |
...
| **R6.11 — EDITORIAL SCHEMA-FALLBACK (silent failure)** | **SHIPPED 2026-06-03** — Replaces silent clean fallback when combined v4 editorial+style JSON fails schema validation twice with an auditable unreviewed state. **R6.11a** (salvage — per-concern validation, category reclassification, retry-with-error-feedback; tag `r6.11a-editorial-schema-salvage`). **R6.11b** (not_reviewed visible state — backend `r6.11b-not-reviewed-state`, frontend `v8.54.0-r6.11b-not-reviewed-state`). Fallback emits `editorialVerdict: "not_reviewed"` with `editorialConcerns: []` and `editorialNote: ""`; genuine clean retains the canonical note (*"No editorial or style concerns identified under the listed rules."*). **Minor follow-up (no spec yet):** `not_reviewed` currently borrows the soft-concern tier in the frontend composite badge (`StatementAnalysisPanel`); counting and surfacing are correct; a future composite-badge refactor may give it its own tier. | Shipped |
...
2. ~~**R6.11 — EDITORIAL SCHEMA-FALLBACK**~~ — **SHIPPED 2026-06-03** (R6.11a + R6.11b). See **Near-term — Review output** and **Recently shipped → R6.11**.
3. ~~**COMMENTARY CALIBRATION**~~ — **SHIPPED 2026-06-03** (`commentary-calibration`). ~~**EDITORIAL RULE BUG-FIX PASS**~~ — **SHIPPED 2026-06-03** (R6.2e + R6.2f). Next: **SOURCE-PUBLIC-STATE AWARENESS** / **R6.6 (source-public-state)** → **CONSTRUCTIVE FEEDBACK OUTPUT** — see **Near-term — Review output**.
...
11. **Document-type appropriateness (R6.12)** — Medium; expanded for document-type-aware voice/register (2026-06-01). **Source-public-state awareness (R6.6)** — Medium; residual verification in progress (R6.6-diag/R6.6.1 shipped 2026-06-25). **Forward-looking statement review (R6.7)** — Medium. **Cross-source display semantics (R6.8)** — Medium. **Non-claim statement handling (R6.9)** — shipped; residual functional-element noise confirmed 2026-06-01. **Source quality audit (R6.10)** — Low.
