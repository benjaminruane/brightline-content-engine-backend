...
Last updated: 2026-06-03 (R6.2e/R6.2f editorial+compliance rule bug-fix pass shipped; B23 closed)
...
**Recommended order:** ~~commentary calibration~~ (**SHIPPED** `commentary-calibration`) → ~~editorial rule bug-fix pass~~ (**SHIPPED** R6.2e + R6.2f) → schema-fallback (below) → then R6.6 / source-public-state / R7. Front-loads cheap, broad, low-risk wins; defers feature-sized work.
...
3. ~~**COMMENTARY CALIBRATION**~~ — **SHIPPED 2026-06-03** (`commentary-calibration`). ~~**EDITORIAL RULE BUG-FIX PASS**~~ — **SHIPPED 2026-06-03** (R6.2e + R6.2f). Next: **EDITORIAL SCHEMA-FALLBACK** → **SOURCE-PUBLIC-STATE AWARENESS** / **R6.6** — see **Near-term — Review output**.
...
**Logged 2026-06-03** (R6.2e). `internal_plausibility` may still attempt to fire on statement-vs-source figure discrepancies (e.g. rounded 17% vs source 17.1%). Scope wording is in place (intra-sentence only; rounding vs source is Evidence), but in testing the concern was suppressed by the fidelity gate (cited source figure absent from statement), not by the model obeying the scope constraint — **B14** negative-constraint pattern. User-visible result is correct (no concern shown). **Action:** review across next diagnostic batch; add a targeted deterministic backstop only if it recurs in a form the fidelity gate does not catch. Do not spec until then.
