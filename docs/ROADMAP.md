...
Last updated: 2026-08-27 (Stage 2 R10 basis-conflict shipped; last known mark-as-return false green fixed)
...
- **Stage 2 rewrite R3a SHIPPED 2026-08-26.** Tag: `stage2-rewrite-r3a` (`7ff4aa4`). Five proven false greens fixed; residuals **B108**–**B114**.
- **Stage 2 basis-conflict R10 SHIPPED 2026-08-27.** Live `lib/qc/pipeline-v4/prompts/stage2_v4.md` is R10 (trimmed len 14259, promptHash `44847c61b07bac89855b9a0f555e30f528077ebe0b3a8baa2c2c06669d60b3e1`). Tag: `stage2-basis-conflict-r10` (`971370f`). Quantity-scoped basis limb: returned/realised/distributed versus marked/valued/carried/unrealised conflicts only when quantities match; magnitude route untouched. Graded gate `8dc6be7`; corpus blast `ce3d85e` (20/378 moved). **Last known false-green shape fixed:** eval-ablation EA_E3 (Meridian `meridian_source.txt`, not claim-spans CS_E3, not corpus E3:S0:ic_memo) now conflicting. Recovered nordholt-dirty S1 and F18_S3. Accepted cost: F18_S7 lost contradiction is PASSAGE DRIFT (B115), not a rule defect (`f18-s7-loss-diagnosis.md`). Open residual: **B115** rich-mark / non-corresponding passage selection.
...
**Statement-level unsupported span (B88), shipped behind a flag 2026-08-25.** Spans exist only when `QC_STAGE2_SPAN` is ON; that flag also governs the reviser wire. Sequence of tags:
