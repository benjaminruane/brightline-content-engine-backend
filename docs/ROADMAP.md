...
- **Pr9 rewrite-correctness SHIPPED 2026-08-23.** Tags: `pr9-claim-spans` (`d8ab2df`), `pr9-soften-or-cut` (`0cd76a5`), `pr9-marker-intent` (`00cce35`), `pr9-cut-punctuation` (`71500c4`). Together they fixed a revision that could attach a note claiming a removal to text it left untouched, reproduced 3/3 on the production sentence. Residuals still open: **B80**, **B81**. Honesty repair-policy follow-on **B119** shipped 2026-08-27 (`ade84fc`). **Pr15** closed 2026-08-24 (Meridian re-run; flow holds). Harness caveat: **P15**. Narrative Claude project docs for this arc live outside the GitHub tree (sync does not carry `claude/*`); in-repo record is this ROADMAP section and BACKLOG Closed **Pr9-correctness**.
