...
**B63 — LLM result cache for the verdict path** (shipped 2026-08-20, flag `QC_LLM_CACHE` default OFF). Stages 1, 1b, and 2. Process-local in-memory LRU, logical collection `qc_llm_cache`, caps 1024 entries / 16 MiB. Gate identity diff count 0; warm hit rate 100% inside one process. **Limitation:** the cache does not survive a cold start or an instance change, so cross-session repeatability is not delivered. **Does not close B61.** Tag: `review-llm-cache` (`e6fd99c`).
