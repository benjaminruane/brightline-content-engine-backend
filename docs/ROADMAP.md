...
**Recommended order:** ~~commentary calibration~~ (**SHIPPED** `commentary-calibration`) → editorial rule bug-fix pass → schema-fallback (below) → then R6.6 / source-public-state / R7. Front-loads cheap, broad, low-risk wins; defers feature-sized work.
...
3. ~~**COMMENTARY CALIBRATION**~~ — **SHIPPED 2026-06-03** (`commentary-calibration`). Next: **EDITORIAL RULE BUG-FIX PASS** → **SOURCE-PUBLIC-STATE AWARENESS** / **R6.6** — see **Near-term — Review output**.
