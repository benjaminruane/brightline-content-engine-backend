...
Last updated: 2026-08-25 (chapter five)
...
**Still open from this chapter and chapter five:** **B88** (confirmed: the evidence lane drives the revision only through decomposed claim spans; ahead of **B84**; read `gatherConcerns` first), **B84** (false greens: verdict-adjacent, full gate; residual-anchor guard hypothesis; editorial is not a backstop, three runs), **B97** (fallback direction contaminates unrelated edits), **B98** (revision layer unstable run to run; worse than editorial instability), **B99** (editorial reviewer files concerns against the wrong statement), **B100** (observability gaps), **B101** (database failure path not graceful), **B102** (`em_dash` false positive on a hyphen), **B103** (local development environment broken), **B92** (contradictory findings: downgraded; trust problem; durable fix is an output check), **B93** (rule taxonomy overlap; separate from **B61** before B61 is scoped), **B82** (routing keys off a model-authored verb; may not be latent), **B94** (first-person-actor harness score is soft), **B95** (request-body supply never executed), **B96** (`AUTHORING_ORGANISATION` is Halden Group in Production and Preview), **B83** (editorial-layer instability; refines **B61**; part of the 4 of 10 is **B93**), **B85** (fixture 01 is a temporal hybrid), **B89** (supportSpans classification mislabel; priority M), **B90** / **B91** (B91 intermittent), **P16** / **P18** / **P19** / **P20** / **P21** / **P22** / **P23** / **P24** / **P25** / **P26** / **P27** (standing process rules). **B87** closed. **Pr15** stays closed. **B9** unchanged: acting on a finding and acting on an edit are two different workflows; B9 as scoped covers only the first.
