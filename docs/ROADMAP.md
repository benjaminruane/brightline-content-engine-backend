...
2. **Cross-document disagreement (**B173**, shipped 2026-09-13).** Two sources that disagree are asked which document governs before a proposal is offered. Copy names both figures and both labels (`sources_disagree` / `sources_disagree_neither` / `sources_agreed_kept`). Rulings are session-scoped in the request body. **B186 (2026-09-14):** first append-only `reviewer_decisions` store in code (rulings and overrides; change appends). `npm run db:migrate` against Vercel Development failed: password authentication (**B103**). Production has **not** been migrated. Retrieval beyond the simple GET waits for accounts. Do not close **B172** or **Pr16**. **Annotation 2026-09-13 (B183):** detection was correct but unreachable on the live runner until B183 kept `card` on the internal entry. The live look on 2026-09-13 is what found it.
...
**Unblocked-but-not-built** now that a store exists: **B9** (needs append-only finding-decision events, not an overwrite buffer; acting on a finding and acting on an edit are two different workflows, and B9 as scoped covers only the first. **B186** is the beginning of that table in code — Development migrate failed on stale credentials (**B103**); production has **not** been migrated — and still does not record per-finding accept or reject), **B61** (needs a durable LLM-result cache, not the frontend snapshot), **Pr13** approved-text library (needs a library of past texts; restore explicitly shipped without list/picker/history UI). See BACKLOG.
