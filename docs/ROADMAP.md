...
Decided 19 September 2026 (Ben): SOURCE TYPES ARE PROSE ONLY, with tables and fact sheets detected and marked not checked, never guessed at. HOSTED BY BEN, one deployment, not in a client's environment.

Still pending with Ben: the first client and what they do with it; cost per review and price.
...
TIER 1 OF THE REPAIR PLAN IS CLOSED. On typed text the product is faithful and honest: a check that did not run says so; a check switched off says so and is named on the card; the card's border colour comes from the one readiness calculation and agrees with the card; a provider refusal records what the provider actually said; a terminal refusal fails in seconds instead of retrying a bill for four and a half minutes; every hole carries a machine-readable reason; an absent setting can no longer mean a check ran; and the reviewer assessment distinguishes nothing-to-say from could-not-be-written.

THE ACTIVE BLOCK IS INGESTION. **B163** is the read-only reality check and everything after it is sized from its answer. **B258** is the largest single item: extraction output must be REPAIRED before review, not merely received.

Still open from the superseded paragraph and not part of that closed front: **B153** wording pass (frozen); **B156** fifth display state; **Pr16** internal consistency; **B158** second excerpt gap; **B171** user-supplied authoring organisation; **B172** unsupported dependents as a finding; Implement Changes residuals **B151** / **B152**. Frozen corpus stays at catch 9 of 11, leave-alone 67 of 74 (**B162**). Every spec still ends with `npm run verify:ship` in both repos.
...
- **One word limit (**B188**).** Pre-filled from the house figure for output type and visibility, set by the writer, stated to the model once as a ceiling, reported and never enforced. Seven places became one. Backend `15316f4`; frontend `be57661` and `0e91ccf`.
- **Rewrite path aligned (**B188b**).** The rewrite path held its own second shortening instruction; removed, so generate and rewrite behave the same way. Backend `05aead7`; frontend `0678d06`. Leftover softer line in `api/rewrite.js` is **B198**.
- **Environment visible (**B189**).** `/api/health` reports `pipelineRoute`, `authoringOrganisation`, `reviseActionList`, `ok`; the header shows it as a pill, amber when off-nominal. `api/_lib/env-flags.js` is the single reader. Backend `579dbfc`; frontend `150cdac`; tag `b189-environment-visible` (frontend only).
- **Revise flag drift (**B190**).** Pill warns when backend `REVISE_ACTION_LIST` and frontend `VITE_REVISE_ACTION_LIST` disagree. Frontend `287042c`; tag `b190-revise-flag-drift`.
- **Word limit label (**B191**).** Field labelled "Word limit" on the left, live counter on the right; amber applies to the count, not the control. Frontend `08aea65`; tag `b191-word-limit-label`.
- **Results overflow (**B192**).** A source filename can no longer widen the Results column past the viewport. Closed drawers marked inert. Frontend `b020ffd`; tag `b192-results-overflow`. Cosmetic residue **F23**.
- **Summary verdict mix (**B193**).** The review summary no longer counts conflicts and partials as unsupported. `src/utils/verdictMix.js` is the single four-way count used by both payload builders; the prompt states that a conflict has evidence. Frontend `35cc974`; backend `27e9a47`; tag `b193-summary-verdict-mix`. Four-summary reconciliation was **B194** (shipped 2026-09-17).
...
- **Multi-value correction under the owner’s rulings.** Design A `362cb42` (contradicted evidence never calls the rewrite model; one same-kind pair → exact-quote replace, else acknowledge). Locator then `548f131`. Ruled table `4536ac7` (**B169**). A contradicted sentence is corrected in every licensed figure, or not at all. Live F18 pin: replace S3, S4 (38→35 and March→April 2025), S5, S8 (both returns, `gross` stays); acknowledge S0, S2, S7 (forecast, R5). **Annotation 2026-09-13 (B183):** R1 self-disagreement could not fire in the live product until B183. **Annotation 2026-09-15:** live production F18 after the update-governs ruling produced those replace proposals. The B173 ask-first table is the before-ruling half. Evidence `365d6a4`.
...
- **Confirming passage and card chrome.** Backend `12311c4`; frontend `de81f14`, then `5c11fa7` / `bad36cf`. A same-document confirming span is painted when it supports a figure left in the sentence; the clause *“It confirms the residential unit count, which is left unchanged.”* is emitted only then. Caption `Proposed change`. Change counts inline. Source name in full, wrapping, no tooltip. Preview tints in product blue, not yellow. **Annotation 2026-09-13 (B183):** the confirming-passage selector could not fire in the live product until B183.
...
2. **Cross-document disagreement (**B173**, shipped 2026-09-13).** Two sources that disagree are asked which document governs before a proposal is offered. Copy names both figures and both labels (`sources_disagree` / `sources_disagree_neither` / `sources_agreed_kept`). Rulings are session-scoped in the request body. **B186 (2026-09-14):** first append-only `reviewer_decisions` store (rulings and overrides; change appends). Production was migrated 2026-09-14; verified live by reading a stored row back. First rows: a reviewer changed their ruling twice and all three were preserved. Retrieval beyond the simple GET waits for accounts. **B187 (2026-09-15):** each decision is tied to the draft hash. Do not close **B172** or **Pr16**. **Annotation 2026-09-13 (B183):** detection was correct but unreachable on the live runner until B183 kept `card` on the internal entry. The live look on 2026-09-13 is what found it. **Annotation 2026-09-15:** fixture 18 run live in production (`365d6a4`) — first time since B183. The cross-document question appeared, the ruling applied across the review, and all five contradicted figures came back corrected. Pinned F18 table was not wrong, it was HALF: before-the-ruling (acknowledge, sources disagree, no proposal). After the ruling those same statements carry proposals matching the other table. Detail `scripts/diagnostic/delivery-check/live-2026-09-15-f18-run.md`.
...
Do not close **B153**, **B156**, **B157**, **B158**, **B166**, **B170**, **B174**, or **Pr16**. Leave **B153** frozen.

### Accuracy judging arc (2026-09-08). Tag: `review-conflict-wins`

Backend-only. Labelled set of 100 statements, freeze 261. Catch 9 of 11, leave-alone 67 of 74 on `runs/evidence-pass-lift-1`. Instrument and noise floor: `scripts/diagnostic/accuracy/README.md`.

Shipped:

- **Conflict-wins** (`9f8fc41`). Stage 3: any reduced pair `conflicting` wins the badge. Zero any-confirmed-wins disagreements on the labelled set. Closes **B107**. Closes the aggregation half of **B39**.
- **Intra-source reducer** (`ba57db7`). One statement-source pair takes the most serious of the single-pick and every locatable widened span. Closes the one-passage window **where the matcher returns a contradicting span**. Residual matcher miss: **B161** (F13 EBITDA 11.1%).
- **Lift-only rounding on spans** (`0dee6fe`, correcting `66351a9`). Confirmed or partial spans are never forced to conflict. A conflicting span whose figures agree within tolerance stops voting as conflicting.
- **Conflict excerpt from the span** (`3214646`). Span-only conflicts can own `conflictExcerpt`. Residual: **B158** (11 conflict cards still quote in `primaryExcerpt` only).
- **Cost meter** (`32f3b97`). Stage 2 `costUsd` / `usage` reach the runner. 2026-09-05 pair has no meter and must not be re-run to recover it.
- **Frozen statement list** (`94bb017`). Accuracy runner does not re-split; 261, zero unmatched.

Do not propose another judging change against this labelled set. Next accuracy work is a larger corpus (Ben).

### B149 silence face (2026-09-03)

Silence no longer reads `supportSpans[].classification`. Widened spans never feed Stage 3; a span-level `conflicting` mark can no longer grant ACTION on a `partial` card. Test `tests/revise-actions-silence.test.mjs` failed on the reference `STRUCTURAL_TESTS` entry, then passed after it was removed. Drawer mislabel remains **B89**. No verdict, classification, or Stage 2 pin change.

### Implement Changes (Pr17) (on main 2026-09-01–02; no action-list tag)

Per-finding accept / reject / modify / apply on Review (`/assess`). Behind two flags: backend runtime `REVISE_ACTION_LIST` (`api/revise-actions.js`, `api/revise-actions-apply.js`); frontend build-time `VITE_REVISE_ACTION_LIST` (`src/modules/drafting/actionListDisplay.js`). Production flag state is not verified here. No backend tag on this arc. Frontend tag on the Suggest-control removal only: `v8.77.0-remove-suggest-revised-draft` (`cdec139`). A tag landing is not a row closing; **B151** stays open.

Shipped by SHA, not by a fresh billed Review: action list `d7a38d2`; licensed-change (no licensed change → acknowledgement) `924b859`; card display of proposed sentence and reason (frontend `77ab6cd`); apply is deterministic, no model (`lib/revise-actions/apply.mjs`, `953ebb4` / frontend `4e856ac`); accept / reject / modify / Save / Discard, never-write until Save (`4e856ac`); default-accept retired, nothing applied unless the user chose it (`4a65922` / frontend `d9eda91`, **B152**); polish bulk state, pending line, expand/collapse all (`958c3b5` / frontend `ac32db9`); version-history readable diff coalesces adjacent word-runs (frontend `92a6e3b`). Constraints **B150** (Neon autosave) obeyed: decisions are panel state, not `REVIEW_STATE_SNAPSHOT_FIELDS`.

**Apply was proven against the recorded Brackenhill sample, not a fresh run.** Locks use `scripts/diagnostic/revise/per-finding-action-list/brackenhill-2026-09-02.json` plus `brackenhill-memo-draft.txt` (`tests/revise-actions-apply.test.mjs`). That JSON is a baseline and must not be regenerated (`924b859`; also `brackenhill-fund-iii-source.txt`, `brackenhill-2026-09-02.md`). A fresh Review or action-list call is a new sample.

**Not shipped:** step 8 preview before save (**B151**, deliberate v1 deviation). Suggest control removed (`cdec139`); the old modal save handler `handleSaveSuggestRevisionAsVersion` still calls `setDraftText` / `captureAssessVersion` with no button (**B126** landmine). Writing workspace remains dead (**B126** pile 2).

Residuals filed on this arc: **B147**, **B148**, **B150**, **B151**, **B152**, **B153**. **B149** shipped 2026-09-03 (silence no longer reads widened `supportSpans`). Next: **B155**, then wording pass (**B153**). **B154** last if a backstop is built.
...
**Unblocked-but-not-built** now that a store exists: **B9** (needs append-only finding-decision events, not an overwrite buffer; acting on a finding and acting on an edit are two different workflows, and B9 as scoped covers only the first. **B186** is the beginning of that table — production migrated and verified 2026-09-14 by reading a stored row back — and still does not record per-finding accept or reject), **B61** (needs a durable LLM-result cache, not the frontend snapshot), **Pr13** approved-text library (needs a library of past texts; restore explicitly shipped without list/picker/history UI). See BACKLOG.
...
**B34 — Remove Assess Review Settings auto-detect** (shipped 2026-07-05). Tags: backend `b34-remove-auto-detect`, frontend `v8.62.0-b34-remove-auto-detect`. Commit: `feat(B34): remove Assess Review Settings auto-detect`.
...
- **Source-style conflation pattern** (observed R6.5 F01 live regression): LLM treats source text style as authoritative over house style. Statement uses `5'500` (PG-correct apostrophe separator); source uses `5,500` (comma separator); LLM fires `thousand_separator` concern recommending changing the correct draft to match the source's incorrect form. **Believed resolved at the filter layer via R6.5.5. That claim is false as written.** The statement-scoped regex exists, but `applyDeterministicStyleFilters` **failed open**: if the flagged text cannot be located as a span, the concern is kept. The apostrophe the house rule requires is the character that defeats quote location. Diagnostic `626524d` (`scripts/diagnostic/delivery-check/house-style-inversion-findings.md`): **61** unique already-compliant house-style flags across stored batches; live exhibit **B176**. **Evidence caveat:** those 61 counts come from gitignored local `scripts/diagnostic/runs/` folders. A fresh clone cannot reproduce them. Committed accuracy runs return **zero** because editorial is disabled in all of them. **2026-09-12 B178 + B180:** source removed from the editorial payload; statement-scoped DROP filters now run without a span for `thousand_separator`, `currency_format`, `smart_quotes`, `em_dash`. **B176** closed. Correction history kept.
...
|------|---------|----------|
| **COMMENTARY CALIBRATION (Stage 5 / reviewer-prompt)** | **SHIPPED / closed (B22 chapter)** — B22 + B22.1 + B22.2. Tags: `commentary-calibration` (orig `commentary-calibration`), `b22.1-commentary-specificity`, `b22.2-editorial-excerpt-removal`. Full closeout + verification caveats: **BACKLOG B22**. | Shipped |
...
| B34 — Remove Assess Review Settings auto-detect | `b34-remove-auto-detect` (backend), `v8.62.0-b34-remove-auto-detect` (frontend) |
...
### D1 — Conflict-with-confirmation card surfacing (CLOSED 2026-09-08)

**Closed 2026-09-08.** Stage 3 is conflict-wins (`9f8fc41`). A contradicting source now outranks a confirming one, so the card verdict is `conflicting` rather than `supported_full` with a hidden `hasConflict`. Zero any-confirmed-wins disagreements on the labelled set. Closes **B107** and **B39**. Residual excerpt layout: **B158**.
...
- **`thousand_separator`** — R6.5.5 statement-scoped backstop was believed to have closed source-style conflation. **Corrected 2026-09-11:** the filter fails open when it cannot locate the flagged text; diagnostic `626524d` counts 61 already-compliant flags on gitignored local batches. Live residue **B176**. Do not treat this watch item as clean. 2026-09-12: prompt layer built. Watch item stays open until the filter fails closed. **2026-09-12 B178:** source text removed from the editorial+style payload rather than a fifth ignore-instruction or a fail-closed filter. **2026-09-12 B180:** filter now fails closed without a span for `thousand_separator`, `currency_format`, `smart_quotes`, `em_dash`. **B176** closed (F1 met). See **B178** and **B180**.
...
- **B9 — full accept / reject / refine** — **unblocked-but-not-built (2026-08-21).** A store exists (`persist-review-state`) but `review_state` overwrites; B9 needs append-only finding-decision event rows in a separate table. **Reframe (2026-08-23):** acting on a finding and acting on an edit are two different workflows. B9 as scoped covers only the first. The chapter's first question is where in the flow the human makes each decision, not how accept and reject work on a card. Per-change accept, reject or MODIFY on Review shipped as **Pr17** (2026-09-01–02; no action-list tag; residual **B151**; also the answer to **B143**). That does not close B9. **Annotation 2026-09-14 (B186):** `reviewer_decisions` is the beginning of that table (source-governance rulings and overrides). Production was migrated 2026-09-14 and verified by reading a stored row back. Per-finding accept and reject are still not recorded. **B187** sends the draft hash with each decision. Pr9 rewrite-correctness narrative also lives in Claude project docs outside this GitHub tree; in-repo record is ROADMAP Pr9 rewrite-correctness.
