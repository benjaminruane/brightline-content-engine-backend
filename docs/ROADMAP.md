...
Last updated: 2026-08-30 (B122 closed; Review B131-B133 and Suggest style carve-out B134 opened)
...
**The governing rule reversed: silence never edits.** Where a source is silent or vague on what the draft asserts, Revise now leaves the wording exactly as written and flags it. It does not soften it, drop the figure, cut the clause, or rewrite the sentence. Silence is the absence of evidence, not evidence against the claim, and the author decides what to do about it (`build-revision-prompt.mjs` rule (b)). Deterministic whole-sentence removal and the soften-and-cut branches no longer run; `api/suggest-revision.js` passes `deterministicUnsupportedRemoval: false` at both call sites. This supersedes the soften-or-cut description in **Pr9 rewrite-correctness** below.
...
**B122 CLOSED 2026-08-30.** Opened as "an editorial instruction naming an exact span is followed about seven times in ten, cause unknown". Every clause of that was wrong. Passes: instrument `0d12440`, scorer+re-score `235def4` / `4f88ad2` / `ee170f7`, residual adjudication `cf32a76`. What the 29 of 42 was made of: 3 cells the scorer could not register (`structural_integrity` followed 3 of 3, verbatim); 3 cells the scorer credited the wrong directive; 6 cells Review emitted two directives that cannot both be satisfied (**B131**); 3 cells Review emitted a wrong directive and Suggest obeyed it (**B132**); the rest silence-never-edits beating craft on a mixed card (**B134**, decided 2026-08-30, not built). Apostrophe truncation of `note_quote` spans is **B133** (cosmetic; evidence spans are a separate path). Protect from **P29**: `scripts/diagnostic/revise/author-confusion-sweep.json`, `b122-rescore.json`, `directive-follow-scorer.mjs`, `tests/directive-follow-scorer.test.mjs`.

**Open from this arc:** **B123** F02:S6 anchor tokenisation, a live false red on the client's own statements. **B98** revision-layer instability, now quantified at 8 of 10 cards varying across three runs on one unchanged Review, and confirmed as generated in the revision layer rather than inherited from Review. New Review rows from the B122 split: **B131** contradictory `suggestedDirection` pair (cross-link **Pr16**), **B132** `voice_consistency` on an already third-person sentence, **B133** `note_quote` apostrophe truncation. New Suggest row: **B134** style carve-out (silence blocks claims, not grammar or voice). New hygiene: **B124**-**B126**, **P29** (four B122 files protected), **P30**. New capability gaps: **Pr16** internal consistency in Review (Ben, must-have), **Pr17** accept and reject per change.
