...
function generateClaimsForStatement(statementText, uploadedDocs, assessment, runId = null, reqSig = null, idx = 0) {
  ...
  for (const aggClaim of cappedClaims) {
    ...
    if (canonicalClaimAnchor === "qual_valuation") {
      ...
      // A3.6.24: C) Trace checkpoint: pre_rules_emit
      if ((idx < 2 || idx === 3) && runId && reqSig) {
        ...
        diag(runId, reqSig, `[VAL_QUAL_TRACE] idx=${idx} checkpoint=pre_rules_emit len=${preRulesText.length} preview="${preview}" source=${source}`);
      }
    }
    ...
    // A3.6.24: 4) Ensure the emit/skip decision uses the final (possibly restored) claimText
    ...
    if (canonicalClaimAnchor === "qual_valuation" && (idx < 2 || idx === 3) && runId && reqSig) {
      ...
      const preview = postRulesText.length > 60 ? postRulesText.substring(0, 60) : postRulesText;
      diag(runId, reqSig, `[VAL_QUAL_TRACE] idx=${idx} checkpoint=final_emit_decision len=${postRulesText.length} preview="${preview}" source=${source} restored=${emitKeepAliveRestored}`);
    }
    
    // A3.6.24: Skip if still empty after keep-alive attempt
