...
function generateClaimsForStatement(statementText, uploadedDocs, assessment, runId = null, reqSig = null, idx = 0) {
  ...
  for (const aggClaim of cappedClaims) {
    ...
    // A3.6.26: E) Immediately BEFORE the empty-check that decides skip vs emit
    // This is the EXACT gate that currently triggers filtered_empty_after_rules
    if (canonicalClaimAnchor === "qual_valuation") {
      if (!postRulesText || !postRulesText.trim()) {
        if (preRulesOk === true) {
          // A3.6.26: E) Restore with only trailing punct/whitespace
          const restoredText = preRulesText.replace(/[,;:.\s]+$/, "").trim();
          if (isValuationSnippet(restoredText)) {
            postRulesText = restoredText;
            claimText = restoredText; // IMPORTANT: ensure the variable used for emit is restored too
            emitKeepAliveRestored = true;
            aggClaim._emitKeepAliveRestored = true;
            if (shouldTrace(statementIdx) && runId && reqSig) {
              diag(runId, reqSig, `[VAL_QUAL_KEEPALIVE] idx=${statementIdx} restored=true preLen=${preRulesText.length} postLen=0 finalLen=${restoredText.length}`);
            }
          }
        }
      }
    ...
    // A3.6.26: F) Final checkpoint must be immediately before the empty-check, using the final effective text
    if (canonicalClaimAnchor === "qual_valuation" && shouldTrace(statementIdx) && runId && reqSig) {
      let source = "unknown";
      if (aggClaim._usedBestValSnip) {
        source = "best";
      ...
      const preview = postRulesText.trim().length > 60 ? postRulesText.trim().substring(0, 60) : postRulesText.trim();
      diag(runId, reqSig, `[VAL_QUAL_TRACE] idx=${statementIdx} checkpoint=final_emit_decision len=${postRulesText.trim().length} preview="${preview}" source=${source} restored=${emitKeepAliveRestored} preRulesOk=${preRulesOk}`);
    }
    
    // A3.6.26: G) Empty-check must use the effective text used for emit
    // The gate must check the same variable that will be emitted
    if (!postRulesText || !postRulesText.trim()) {
      claimIdx++;
