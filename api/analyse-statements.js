...
function generateClaimsForStatement(statementText, uploadedDocs, assessment, runId = null, reqSig = null, idx = 0) {
  ...
  for (const aggClaim of cappedClaims) {
    ...
    // A3.6.24: A) Capture "last-known-good" claimText for qual_valuation
    ...
    if (canonicalClaimAnchor === "qual_valuation") {
      // Capture the claimText value RIGHT BEFORE any rule chain processing
      preRulesText = (claimText || "").trim();
      preRulesOk = isValuationSnippet(preRulesText);
      
      // A3.6.25: 2) Unify trace labels - ALWAYS log pre_rules_emit for qual_valuation
      // A3.6.25: 4) Validate preRulesOk uses the same string shown in the traces
      // preRulesText must be EXACTLY what is logged in pre_rules_emit preview
      if ((idx < 2 || idx === 3) && runId && reqSig) {
        ...
        // A3.6.25: Include preRulesOk in pre_rules_emit log
        diag(runId, reqSig, `[VAL_QUAL_TRACE] idx=${idx} checkpoint=pre_rules_emit len=${preRulesText.length} preview="${preview}" source=${source} preRulesOk=${preRulesOk}`);
      }
    }
    ...
    // A3.6.24: 3) Apply rule chain as-is, then keepalive-restore if it empties
    // Apply any final rule chain that could empty claimText
    // Note: Currently no explicit rule chain is applied here, but claimText might already be empty
    // or could become empty through other processing. We check after all processing.
    let postRulesText = claimText;
    
    // A3.6.24: C) Trace checkpoint: post_rules_emit (for qual_valuation)
    if (canonicalClaimAnchor === "qual_valuation" && (idx < 2 || idx === 3) && runId && reqSig) {
      ...
      const preview = postRulesText.length > 60 ? postRulesText.substring(0, 60) : postRulesText;
      diag(runId, reqSig, `[VAL_QUAL_TRACE] idx=${idx} checkpoint=post_rules_emit len=${postRulesText.length} preview="${preview}" source=${source}`);
    }
    
    // A3.6.24: B) Emit-time keep-alive restore
    // If qual_valuation AND postRulesText is empty/whitespace AND preRulesOk === true
    if (canonicalClaimAnchor === "qual_valuation" && (!postRulesText || !postRulesText.trim()) && preRulesOk) {
      // Restore claimText with ONLY trailing punctuation trimmed (no other cleaning)
      let restoredText = preRulesText.trim();
      restoredText = restoredText.replace(/[,;:.\s]+$/, "").trim();
      
      // Validate again: isValuationSnippet(restoredText) must still be true
      if (isValuationSnippet(restoredText)) {
        postRulesText = restoredText;
        emitKeepAliveRestored = true;
        
        // A3.6.24: D) Diagnostics: log restoration
        if ((idx < 2 || idx === 3) && runId && reqSig) {
    ...
    // A3.6.25: 3) Ensure keepalive restore runs immediately before the skip decision at the same gate
    // This is the EXACT gate that currently emits [CLAIMS_EMIT_SKIP] for empty claims
    // Keepalive restore must run here, right before the empty check
    
    // A3.6.25: 4) Validate preRulesOk uses the same string shown in the traces
    // Trace checkpoint: final_emit_decision (immediately before emit/skip decision)
    if (canonicalClaimAnchor === "qual_valuation" && (idx < 2 || idx === 3) && runId && reqSig) {
      let source = "unknown";
      if (aggClaim._usedBestValSnip) {
        source = "best";
      ...
      const preview = postRulesText.length > 60 ? postRulesText.substring(0, 60) : postRulesText;
      // A3.6.25: Include preRulesOk in final_emit_decision log
      diag(runId, reqSig, `[VAL_QUAL_TRACE] idx=${idx} checkpoint=final_emit_decision len=${postRulesText.length} preview="${preview}" source=${source} restored=${emitKeepAliveRestored} preRulesOk=${preRulesOk}`);
    }
    
    // A3.6.25: This is the EXACT gate that decides skip vs emit
    // Skip if still empty after keep-alive attempt
    // The subsequent decision MUST be based on the final claimText
